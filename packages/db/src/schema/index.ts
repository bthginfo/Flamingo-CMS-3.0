...
// Control-plane registry for standalone tenant databases. Connection URIs are
// encrypted by the marketing service before they reach this table and are
// intentionally kept separate from the commonly selected tenants row.
export const tenantDatabaseConnections = pgTable('tenant_database_connections', {
  tenantId: uuid('tenant_id').primaryKey().references(() => tenants.id, { onDelete: 'cascade' }),
  provider: varchar('provider', { length: 30 }).notNull().default('neon'),
...
export const customers = pgTable('customers', {
  ...
  customerType: varchar('customer_type', { length: 20 }).notNull().default('company'),
  ...
  eInvoiceRoutingId: varchar('e_invoice_routing_id', { length: 100 }),
  buyerReference: varchar('buyer_reference', { length: 100 }),
  ...
  uniqueIndex('customers_tenant_number_idx').on(t.tenantId, t.customerNumber),
  index('customers_tenant_archived_idx').on(t.tenantId, t.archivedAt),
  check('customers_type_check', sql`${t.customerType} IN ('company', 'person')`),
]);
...
export const customerCustomFieldDefinitions = pgTable('customer_custom_field_definitions', {
  ...
  uniqueIndex('customer_custom_fields_tenant_key_idx').on(t.tenantId, t.fieldKey),
  index('customer_custom_fields_tenant_sort_idx').on(t.tenantId, t.sortOrder),
  check('customer_custom_fields_type_check', sql`${t.fieldType} IN ('text', 'textarea', 'number', 'date', 'email', 'phone', 'boolean', 'select')`),
]);
...
// ─── BILLING & CUSTOMER MANAGEMENT ADDON ─────────────────────────────────────
// Shop invoices above remain a backwards-compatible order document. The billing
// module below is the immutable source of truth for manually issued invoices.
export const billingSettings = pgTable('billing_settings', {
  ...
  taxNumber: varchar('tax_number', { length: 100 }),
  vatId: varchar('vat_id', { length: 100 }),
  registerCourt: varchar('register_court', { length: 160 }),
  ...
  bic: varchar('bic', { length: 30 }),
  invoicePrefix: varchar('invoice_prefix', { length: 20 }).notNull().default('RE'),
  cancellationPrefix: varchar('cancellation_prefix', { length: 20 }).notNull().default('ST'),
  quotePrefix: varchar('quote_prefix', { length: 20 }).notNull().default('AN'),
  creditPrefix: varchar('credit_prefix', { length: 20 }).notNull().default('GS'),
  invoiceNumberFormat: varchar('invoice_number_format', { length: 120 }).notNull().default('{PREFIX}-{YYYY}-{NNNN}'),
  cancellationNumberFormat: varchar('cancellation_number_format', { length: 120 }).notNull().default('{PREFIX}-{YYYY}-{NNNN}'),
  quoteNumberFormat: varchar('quote_number_format', { length: 120 }).notNull().default('{PREFIX}-{YYYY}-{NNNN}'),
  creditNumberFormat: varchar('credit_number_format', { length: 120 }).notNull().default('{PREFIX}-{YYYY}-{NNNN}'),
  sequenceReset: varchar('sequence_reset', { length: 20 }).notNull().default('year'),
  sequencePeriod: varchar('sequence_period', { length: 10 }),
  nextInvoiceNumber: integer('next_invoice_number').notNull().default(1),
  ...
  defaultPaymentTermDays: integer('default_payment_term_days').notNull().default(14),
  defaultCashDiscountBasisPoints: integer('default_cash_discount_basis_points').notNull().default(0),
  defaultCashDiscountDays: integer('default_cash_discount_days').notNull().default(0),
  defaultReminderDays: integer('default_reminder_days').notNull().default(7),
  defaultReminderFeeCents: integer('default_reminder_fee_cents').notNull().default(0),
  paymentLinkBaseUrl: varchar('payment_link_base_url', { length: 1000 }),
  defaultIntroText: text('default_intro_text'),
  defaultClosingText: text('default_closing_text'),
  defaultFooter: text('default_footer'),
  smallBusiness: boolean('small_business').notNull().default(false),
  smallBusinessNotice: text('small_business_notice').notNull().default('Gemäß § 19 UStG wird keine Umsatzsteuer berechnet.'),
  senderName: varchar('sender_name', { length: 255 }),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
  uniqueIndex('billing_settings_tenant_idx').on(t.tenantId),
  check('billing_settings_payment_term_check', sql`${t.defaultPaymentTermDays} BETWEEN 0 AND 365`),
  check('billing_settings_counters_check', sql`${t.nextInvoiceNumber} > 0 AND ${t.nextCancellationNumber} > 0 AND ${t.nextQuoteNumber} > 0 AND ${t.nextCreditNumber} > 0 AND ${t.nextCustomerNumber} > 0`),
...
export const billingServices = pgTable('billing_services', {
  ...
  name: varchar('name', { length: 255 }).notNull(),
  description: text('description'),
  unitCode: varchar('unit_code', { length: 10 }).notNull().default('C62'),
  unitLabel: varchar('unit_label', { length: 40 }).notNull().default('Stück'),
  unitPriceNetCents: integer('unit_price_net_cents').notNull(),
  taxRateBasisPoints: integer('tax_rate_basis_points').notNull().default(1900),
  active: boolean('active').notNull().default(true),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
  uniqueIndex('billing_services_tenant_code_idx').on(t.tenantId, t.serviceCode),
  index('billing_services_tenant_active_idx').on(t.tenantId, t.active),
  check('billing_services_price_check', sql`${t.unitPriceNetCents} >= 0`),
  check('billing_services_tax_check', sql`${t.taxRateBasisPoints} BETWEEN 0 AND 10000`),
]);
...
export const billingDocuments = pgTable('billing_documents', {
  ...
  dueDate: timestamp('due_date', { withTimezone: true }),
  buyerReference: varchar('buyer_reference', { length: 100 }),
  purchaseOrderReference: varchar('purchase_order_reference', { length: 100 }),
  introText: text('intro_text'),
  closingText: text('closing_text'),
  notes: text('notes'),
  taxMode: varchar('tax_mode', { length: 30 }).notNull().default('standard'),
  ...
  discountCents: integer('discount_cents').notNull().default(0),
  cashDiscountBasisPoints: integer('cash_discount_basis_points').notNull().default(0),
  cashDiscountDays: integer('cash_discount_days').notNull().default(0),
  paymentLinkUrl: varchar('payment_link_url', { length: 1000 }),
  ...
  paymentSnapshot: jsonb('payment_snapshot').$type<Record<string, unknown>>(),
  taxBreakdown: jsonb('tax_breakdown').$type<Array<{ rateBasisPoints: number; netCents: number; taxCents: number }>>().notNull().default([]),
  subtotalNetCents: integer('subtotal_net_cents').notNull().default(0),
  ...
  uniqueIndex('billing_documents_tenant_number_idx').on(t.tenantId, t.documentNumber),
  index('billing_documents_tenant_status_idx').on(t.tenantId, t.status, t.createdAt),
  index('billing_documents_customer_idx').on(t.tenantId, t.customerId),
  index('billing_documents_original_idx').on(t.originalDocumentId),
  check('billing_documents_type_check', sql`${t.documentType} IN ('invoice', 'cancellation', 'credit_note', 'quote', 'advance_invoice', 'partial_invoice', 'final_invoice')`),
  check('billing_documents_status_check', sql`${t.status} IN ('draft', 'issued', 'finalized', 'sent', 'partially_paid', 'paid', 'cancelled', 'accepted', 'rejected', 'expired', 'converted')`),
  check('billing_documents_amount_check', sql`${t.subtotalNetCents} >= 0 AND ${t.taxCents} >= 0 AND ${t.totalGrossCents} >= 0`),
  check('billing_documents_discount_check', sql`${t.discountType} IN ('percent', 'fixed') AND ${t.discountValue} >= 0 AND ${t.discountCents} >= 0 AND ${t.cashDiscountBasisPoints} BETWEEN 0 AND 10000 AND ${t.cashDiscountDays} BETWEEN 0 AND 365`),
  check('billing_documents_tax_mode_check', sql`${t.taxMode} IN ('standard', 'small_business', 'reverse_charge', 'intra_eu', 'exempt')`),
...
export const billingDocumentItems = pgTable('billing_document_items', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  documentId: uuid('document_id').notNull().references(() => billingDocuments.id, { onDelete: 'cascade' }),
  serviceId: uuid('service_id').references(() => billingServices.id, { onDelete: 'set null' }),
  ...
  index('billing_document_items_document_idx').on(t.documentId, t.position),
  check('billing_document_items_quantity_check', sql`${t.quantity} > 0`),
  check('billing_document_items_price_check', sql`${t.unitPriceNetCents} >= 0 AND ${t.lineNetCents} >= 0`),
  check('billing_document_items_rates_check', sql`${t.discountBasisPoints} BETWEEN 0 AND 10000 AND ${t.discountType} IN ('percent', 'fixed') AND ${t.discountValue} >= 0 AND ${t.discountCents} >= 0 AND ${t.taxRateBasisPoints} BETWEEN 0 AND 10000`),
]);

export const billingPayments = pgTable('billing_payments', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  documentId: uuid('document_id').notNull().references(() => billingDocuments.id, { onDelete: 'restrict' }),
  amountCents: integer('amount_cents').notNull(),
  ...
  index('billing_payments_document_idx').on(t.tenantId, t.documentId, t.paidAt),
  check('billing_payments_amount_check', sql`${t.amountCents} > 0`),
  check('billing_payments_method_check', sql`${t.method} IN ('bank_transfer', 'cash', 'card', 'paypal', 'stripe', 'other')`),
]);
...
export const billingReminders = pgTable('billing_reminders', {
  ...
  index('billing_reminders_document_idx').on(t.tenantId, t.documentId, t.createdAt),
  check('billing_reminders_level_check', sql`${t.level} BETWEEN 1 AND 99 AND ${t.feeCents} >= 0 AND ${t.interestCents} >= 0`),
  check('billing_reminders_status_check', sql`${t.status} IN ('draft', 'sent', 'cancelled')`),
]);

export const billingRecurringSchedules = pgTable('billing_recurring_schedules', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  customerId: uuid('customer_id').notNull().references(() => customers.id, { onDelete: 'restrict' }),
  name: varchar('name', { length: 255 }).notNull(),
  status: varchar('status', { length: 20 }).notNull().default('active'),
  intervalUnit: varchar('interval_unit', { length: 20 }).notNull().default('month'),
  intervalCount: integer('interval_count').notNull().default(1),
  startAt: timestamp('start_at', { withTimezone: true }).notNull(),
  endAt: timestamp('end_at', { withTimezone: true }),
  nextRunAt: timestamp('next_run_at', { withTimezone: true }).notNull(),
  lastRunAt: timestamp('last_run_at', { withTimezone: true }),
  deliveryMode: varchar('delivery_mode', { length: 30 }).notNull().default('draft'),
  recipient: varchar('recipient', { length: 320 }),
  template: jsonb('template').$type<Record<string, unknown>>().notNull().default({}),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
  index('billing_recurring_due_idx').on(t.status, t.nextRunAt),
  index('billing_recurring_tenant_idx').on(t.tenantId, t.status, t.nextRunAt),
  check('billing_recurring_status_check', sql`${t.status} IN ('active', 'paused', 'completed')`),
  check('billing_recurring_interval_check', sql`${t.intervalUnit} IN ('day', 'week', 'month', 'year') AND ${t.intervalCount} BETWEEN 1 AND 120`),
  check('billing_recurring_delivery_check', sql`${t.deliveryMode} IN ('draft', 'finalize', 'finalize_send')`),
]);

export const billingRecurringRuns = pgTable('billing_recurring_runs', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  scheduleId: uuid('schedule_id').notNull().references(() => billingRecurringSchedules.id, { onDelete: 'restrict' }),
  documentId: uuid('document_id').references(() => billingDocuments.id, { onDelete: 'restrict' }),
  ...
  uniqueIndex('billing_recurring_runs_schedule_time_idx').on(t.scheduleId, t.scheduledFor),
  index('billing_recurring_runs_tenant_idx').on(t.tenantId, t.createdAt),
  check('billing_recurring_runs_status_check', sql`${t.status} IN ('running', 'completed', 'failed')`),
]);

export const billingPortalLinks = pgTable('billing_portal_links', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  documentId: uuid('document_id').notNull().references(() => billingDocuments.id, { onDelete: 'restrict' }),
  ...
  uniqueIndex('billing_portal_links_token_idx').on(t.tokenHash),
  index('billing_portal_links_document_idx').on(t.tenantId, t.documentId, t.createdAt),
]);

export const billingDocumentEvents = pgTable('billing_document_events', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  documentId: uuid('document_id').notNull().references(() => billingDocuments.id, { onDelete: 'restrict' }),
...
export const billingDeliveryAttempts = pgTable('billing_delivery_attempts', {
  ...
  index('billing_delivery_document_idx').on(t.documentId, t.createdAt),
  index('billing_delivery_status_idx').on(t.tenantId, t.status, t.createdAt),
  check('billing_delivery_status_check', sql`${t.status} IN ('sending', 'sent', 'failed', 'uncertain')`),
]);
