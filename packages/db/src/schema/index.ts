...
export const billingSettings = pgTable('billing_settings', {
  ...
  check('billing_settings_counters_check', sql`${t.nextInvoiceNumber} > 0 AND ${t.nextCancellationNumber} > 0 AND ${t.nextCustomerNumber} > 0`),
...
export const billingDocuments = pgTable('billing_documents', {
  ...
  check('billing_documents_type_check', sql`${t.documentType} IN ('invoice', 'cancellation')`),
  check('billing_documents_status_check', sql`${t.status} IN ('draft', 'finalized', 'sent', 'paid', 'cancelled')`),
...
export const billingDocumentItems = pgTable('billing_document_items', {
  ...
  check('billing_document_items_rates_check', sql`${t.discountBasisPoints} BETWEEN 0 AND 10000 AND ${t.taxRateBasisPoints} BETWEEN 0 AND 10000`),
]);
