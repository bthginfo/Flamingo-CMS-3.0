...
export const PUT = withApiHandler(async (req, auth) => {
  ...
  if (existing) {
    await db.update(globalSettings).set({ socialLinks: body }).where(eq(globalSettings.tenantId, auth.tenantId));
  } else {
    await db.insert(globalSettings).values({ tenantId: auth.tenantId, socialLinks: body });
  }
