...
async function getInboxUnreadCount(tenantId: string): Promise<number> {
  try {
    const db = getDb();
    const [row] = await db.select({ count: sql<number>`count(*)::int` }).from(formSubmissions)
      .where(and(eq(formSubmissions.tenantId, tenantId), eq(formSubmissions.status, 'new')));
    return row?.count ?? 0;
  } catch {
    return 0; // badge is informational — never block the admin shell
  }
}
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <div className="flex h-screen overflow-hidden bg-admin-bg text-zinc-900 antialiased" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      <Sidebar tenantId={session.tenantId} industry={(await getTenantStyle(session.tenantId)).industry} inboxUnread={await getInboxUnreadCount(session.tenantId)} />
