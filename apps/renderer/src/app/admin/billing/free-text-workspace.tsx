...
function ComposerLoaded({ detail, customers, settings, pending, startTransition, mobilePreview, setMobilePreview, confirmFinalize, setConfirmFinalize, onBack }: {
  ...
  return <section>
    <header className="mb-5 flex flex-wrap items-center gap-3 border-b border-zinc-200 pb-5">
      ...
      {!locked ? <button type="button" disabled={pending} onClick={save} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-zinc-300 px-3 text-sm font-semibold text-zinc-800 hover:bg-zinc-50 disabled:opacity-50"><Save className="size-4" /> \u00c4nderungen speichern</button> : null}
...
function A4Preview({ settings, recipient, title, subject, issueDate, content, pageCount }: { settings: Settings; recipient: Recipient; title: string; subject: string; issueDate: string; content: FreeTextDocument; pageCount?: number }) {
  ...
  return <aside aria-label="A4-Vorschau"><div className="mb-2 flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-[0.12em] text-zinc-500">A4-Vorschau</span><span className="rounded-lg bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{count} {count === 1 ? 'Seite' : 'Seiten'}{pageCount && pageCount !== count ? ` (PDF: ${pageCount})` : ''}</span></div>{!header.fits ? <p className="mb-2 rounded-xl bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700">Empf\u00e4nger oder Betreff sind zu lang f\u00fcr den Briefkopf.</p> : null}<div className="max-h-[calc(100vh-170px)] space-y-4 overflow-y-auto rounded-2xl bg-zinc-200 p-3 shadow-inner">{layout.pages.map((layoutPage, pageIndex) => <article key={pageIndex} className="relative mx-auto aspect-[210/297] w-full max-w-[600px] overflow-hidden bg-white text-zinc-900 shadow-lg">
    ...
    <div className="absolute left-[9.4%] right-[9.4%] border-t border-zinc-200" style={{ top: topAt(752) }}><span className="block w-12 border-t-2 border-blue-600" /></div>
    {pageIndex === 0 ? <><p className="absolute left-[9.4%] right-[9.4%] truncate text-[6.5pt] text-zinc-500" style={{ top: topAt(736, 6.5) }}>{settings.companyName} {'\u00b7'} {sellerAddress}</p><p className="absolute left-[9.4%] text-[6pt] font-bold uppercase tracking-[0.14em] text-blue-700" style={{ top: topAt(714, 6) }}>Empf\u00e4nger</p><p className="absolute right-[9.4%] text-right text-[6pt] font-bold uppercase tracking-[0.14em] text-blue-700" style={{ top: topAt(714, 6) }}>Datum</p>{header.recipientLines.map((line, index) => <p key={`recipient-${index}`} className={`absolute left-[9.4%] whitespace-pre text-[10pt] leading-none ${line.bold ? 'font-bold' : ''}`} style={{ top: topAt(header.recipientY - index * 14, 10) }}>{line.text}</p>)}<p className="absolute right-[9.4%] text-right text-[9pt] text-zinc-600" style={{ top: topAt(header.recipientY, 9) }}>{formattedDate(issueDate)}</p><p className="absolute left-[9.4%] text-[6pt] font-bold uppercase tracking-[0.14em] text-blue-700" style={{ top: topAt(header.subjectLabelY, 6) }}>Betreff</p>{header.subjectLines.map((line, index) => <h2 key={`subject-${index}`} className="absolute left-[9.4%] whitespace-pre text-[15pt] font-bold leading-none" style={{ top: topAt(header.subjectY - index * 19, 15) }}>{line}</h2>)}</> : null}
