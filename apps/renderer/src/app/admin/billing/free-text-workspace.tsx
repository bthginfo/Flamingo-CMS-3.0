...
import { FREE_TEXT_FONT_SIZES, freeTextRecipientFromCustomer, layoutFreeTextDocument, layoutFreeTextHeader, resolveFreeTextPreviewParties, type FreeTextDocument, type FreeTextLayoutSegment, type FreeTextNode } from '@/lib/billing-free-text-document';
import {
  createFreeTextDocumentAction, deleteFreeTextDocumentDraftAction, finalizeFreeTextDocumentAction,
  ...
  return (
    <section aria-labelledby="free-text-title">
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        {rows === null && !error ? <State icon={<Loader2 className="animate-spin" />} title="Schreiben werden geladen" text={'Ihre Entw\u00fcrfe und festgeschriebenen Dokumente werden vorbereitet.'} /> : null}
        ...
        {rows?.length === 0 ? <State icon={<FilePenLine />} title="Noch kein Schreiben" text={'Erstellen Sie Ihren ersten Gesch\u00e4ftsbrief. Empf\u00e4nger und Inhalt bleiben bis zum Festschreiben flexibel.'} action={<button onClick={() => void create()} className="text-sm font-semibold text-blue-700">Erstes Schreiben anlegen</button>} /> : null}
...
function ComposerLoaded({ detail, customers, settings, pending, startTransition, mobilePreview, setMobilePreview, confirmFinalize, setConfirmFinalize, onBack }: {
  ...
  const save = () => run(() => saveFreeTextDocumentAction(payload()), 'Entwurf gespeichert');
  const finalize = () => run(() => finalizeFreeTextDocumentAction(payload()), 'Schreiben festgeschrieben', () => window.location.reload());
  const remove = () => { if (window.confirm('Diesen Entwurf wirklich l\u00f6schen?')) run(() => deleteFreeTextDocumentDraftAction(detail.id), 'Entwurf gel\u00f6scht', onBack); };
  useEffect(() => {
  ...
  return <section>
    <header className="mb-5 flex flex-wrap items-center gap-3 border-b border-zinc-200 pb-5">
      ...
      <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="truncate text-xl font-bold text-zinc-950">{title}</h2><span className={`rounded-lg px-2 py-1 text-[11px] font-semibold ${finalized ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-800'}`}>{finalized ? 'Festgeschrieben' : locked ? 'Wird festgeschrieben' : 'Entwurf'}</span></div><p className="mt-0.5 text-xs text-zinc-500">{finalized ? `${detail.pageCount || 1} PDF-Seite${detail.pageCount === 1 ? '' : 'n'} \u00b7 Inhalt unver\u00e4nderlich` : locked ? 'Die PDF-Datei wird gerade unver\u00e4nderlich erzeugt.' : '\u00c4nderungen bleiben bis zum Festschreiben bearbeitbar.'}</p></div>
      ...
      {!locked ? <button type="button" disabled={pending} onClick={save} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-zinc-300 px-3 text-sm font-semibold text-zinc-800 hover:bg-zinc-50 disabled:opacity-50"><Save className="size-4" /> Entwurf speichern</button> : null}
      {!locked ? <button ref={finalizeTriggerRef} type="button" disabled={pending} onClick={() => setConfirmFinalize(true)} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-blue-600 px-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50"><Check className="size-4" /> Festschreiben</button> : finalized ? <a href={`/api/billing/free-text-documents/${detail.id}/pdf?download=1`} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-blue-600 px-3 text-sm font-semibold text-white hover:bg-blue-700"><Download className="size-4" /> PDF herunterladen</a> : <button type="button" disabled={pending} onClick={finalize} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-amber-50 px-3 text-sm font-semibold text-amber-800 hover:bg-amber-100 disabled:opacity-50"><Loader2 className={`size-4 ${pending ? 'animate-spin' : ''}`} /> Status pr\u00fcfen</button>}
    </header>
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(390px,0.82fr)]">
      <div className={`${mobilePreview ? 'hidden' : 'space-y-5'} lg:block`}>
        ...
        {!locked ? <button type="button" onClick={remove} className="inline-flex items-center gap-2 text-xs font-semibold text-rose-700 hover:text-rose-800"><Trash2 className="size-4" /> Entwurf l&#246;schen</button> : null}
      </div>
    ...
    {confirmFinalize ? <div ref={dialogRef} className="fixed inset-0 z-50 grid place-items-center bg-zinc-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="finalize-title" aria-describedby="finalize-description"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><span className="mb-4 grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-700"><Check /></span><h3 id="finalize-title" className="text-lg font-bold text-zinc-950">Schreiben festschreiben?</h3><p id="finalize-description" className="mt-2 text-sm leading-6 text-zinc-600">Empf&#228;nger, Absender und Inhalt werden als unver&#228;nderlicher Stand gespeichert. Danach kann das Schreiben nicht mehr bearbeitet oder gel&#246;scht werden.</p><div className="mt-6 flex justify-end gap-2"><button onClick={() => setConfirmFinalize(false)} className="min-h-10 rounded-xl border border-zinc-200 px-4 text-sm font-semibold">Abbrechen</button><button ref={confirmRef} disabled={pending} onClick={() => { setConfirmFinalize(false); finalize(); }} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white">{pending ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />} Unver&#228;nderlich festschreiben</button></div></div></div> : null}
  </section>;
}
...
function A4Preview({ settings, recipient, title, subject, issueDate, content, pageCount }: { settings: Settings; recipient: Recipient; title: string; subject: string; issueDate: string; content: FreeTextDocument; pageCount?: number }) {
  ...
  return <aside aria-label="A4-Vorschau"><div className="mb-2 flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-[0.12em] text-zinc-500">A4-Vorschau</span><span className="rounded-lg bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{count} {count === 1 ? 'Seite' : 'Seiten'}{pageCount && pageCount !== count ? ` (PDF: ${pageCount})` : ''}</span></div>{!header.fits ? <p className="mb-2 rounded-xl bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700">Empf\u00e4nger oder Betreff sind zu lang f\u00fcr den Briefkopf.</p> : null}<div className="max-h-[calc(100vh-170px)] space-y-4 overflow-y-auto rounded-2xl bg-zinc-200 p-3 shadow-inner">{layout.pages.map((layoutPage, pageIndex) => <article key={pageIndex} className="relative mx-auto aspect-[210/297] w-full max-w-[600px] overflow-hidden bg-white text-zinc-900 shadow-lg"><div className="absolute left-[9.4%] right-[9.4%] top-[5%] flex items-start justify-between border-b border-zinc-200 pb-3">{pageIndex === 0 && settings.logoUrl ? <img src={settings.logoUrl} alt="" className="max-h-10 max-w-32 object-contain" /> : <span className="max-w-[45%] truncate text-[7pt] text-zinc-500">{pageIndex > 0 ? title : ''}</span>}<strong className="max-w-[48%] text-right text-[9pt]">{settings.companyName || 'Unternehmensname'}</strong></div>{pageIndex === 0 ? <><p className="absolute left-[9.4%] right-[9.4%] top-[11.5%] truncate text-[6.5pt] text-zinc-500">{settings.companyName} {'\u00b7'} {settings.street} {'\u00b7'} {settings.postalCode} {settings.city}</p>{header.recipientLines.map((line, index) => <p key={`recipient-${index}`} className={`absolute left-[9.4%] whitespace-pre text-[10pt] ${line.bold ? 'font-bold' : ''}`} style={{ top: `${((841.89 - (724 - index * 14) - 10) / 841.89) * 100}%` }}>{line.text}</p>)}<p className="absolute right-[9.4%] top-[22%] text-right text-[8pt] text-zinc-500">{formattedDate(issueDate)}</p>{header.subjectLines.map((line, index) => <h2 key={`subject-${index}`} className="absolute left-[9.4%] whitespace-pre text-[15pt] font-bold" style={{ top: `${((841.89 - (header.subjectY - index * 19) - 15) / 841.89) * 100}%` }}>{line}</h2>)}</> : null}{layoutPage.lines.map((line, lineIndex) => <div key={lineIndex} className="absolute whitespace-pre" style={{ left: `${((56 + line.xOffset) / 595.28) * 100}%`, top: `${((841.89 - line.y - 10) / 841.89) * 100}%`, lineHeight: `${line.height}pt` }}>{line.prefix ? <span className="inline-block w-[18pt] text-[10.5pt]">{line.prefix}</span> : null}{line.segments.map((segment, segmentIndex) => <PreviewSegment key={segmentIndex} segment={segment} />)}</div>)}<footer className="absolute bottom-[4%] left-[9.4%] right-[9.4%] flex items-end justify-between border-t border-zinc-200 pt-2 text-[6pt] text-zinc-500"><span>{[settings.email, settings.phone, settings.website].filter(Boolean).join(' \u00b7 ')}</span><span>Seite {pageIndex + 1} von {count}</span></footer></article>)}</div></aside>;
}
