  ...
  return (
    <section aria-labelledby="free-text-title">
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        ...
        {rows?.length === 0 ? <State icon={<FilePenLine />} title="Noch kein Schreiben" text={'Erstellen Sie Ihren ersten Gesch\u00e4ftsbrief. Entw\u00fcrfe k\u00f6nnen Sie bearbeiten; festgeschriebene Schreiben stehen als PDF bereit.'} action={<button onClick={() => void create()} className="text-sm font-semibold text-blue-700">Erstes Schreiben anlegen</button>} /> : null}
        ...
        {filteredRows.length ? <div className="divide-y divide-zinc-100">{filteredRows.map(row => {
          ...
          return <button key={row.id} type="button" onClick={() => onOpen(row.id)} className="grid w-full gap-2 px-4 py-4 text-left transition hover:bg-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 sm:grid-cols-[minmax(0,1fr)_180px_150px_auto] sm:items-center sm:px-5">
            ...
            <span className={`w-fit rounded-lg px-2 py-1 text-[11px] font-semibold ring-1 ring-inset ${row.status === 'finalized' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-amber-50 text-amber-800 ring-amber-200'}`}>{row.status === 'finalized' ? 'Unveränderlich' : row.status === 'finalizing' ? 'In Verarbeitung' : 'Entwurf'}</span>
          </button>;
        })}</div> : null}
      </div>
    </section>
...
function ComposerLoaded({ detail, customers, settings, pending, startTransition, mobilePreview, setMobilePreview, confirmFinalize, setConfirmFinalize, onBack }: {
  ...
  return <section>
    <header className="mb-5 flex flex-wrap items-center gap-3 border-b border-zinc-200 pb-5">
      ...
      <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="truncate text-xl font-bold text-zinc-950">{title}</h2><span className={`rounded-lg px-2 py-1 text-[11px] font-semibold ${finalized ? 'bg-emerald-50 text-emerald-700' : locked ? 'bg-amber-50 text-amber-800' : 'bg-blue-50 text-blue-700'}`}>{finalized ? 'Festgeschrieben' : locked ? 'PDF wird erzeugt' : 'Entwurf'}</span></div><p className="mt-0.5 text-xs text-zinc-500">{locked ? 'Die PDF-Datei wird gerade erzeugt.' : finalized ? `${detail.pageCount || 1} PDF-Seite${detail.pageCount === 1 ? '' : 'n'} \u00b7 unver\u00e4nderlich` : 'Entwurf: bearbeiten, speichern oder als PDF festschreiben.'}</p></div>
      ...
      {finalized ? <a href={`/api/billing/free-text-documents/${detail.id}/pdf?download=1`} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-zinc-300 px-3 text-sm font-semibold text-zinc-800 hover:bg-zinc-50"><Download className="size-4" /> PDF herunterladen</a> : null}
      {editable ? <button type="button" disabled={pending} onClick={save} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-zinc-300 px-3 text-sm font-semibold text-zinc-800 hover:bg-zinc-50 disabled:opacity-50"><Save className="size-4" /> &Auml;nderungen speichern</button> : null}
      {editable ? <button ref={finalizeTriggerRef} type="button" disabled={pending} onClick={() => setConfirmFinalize(true)} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-blue-600 px-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50"><Check className="size-4" /> PDF erstellen</button> : locked ? <button type="button" disabled={pending} onClick={finalize} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-amber-50 px-3 text-sm font-semibold text-amber-800 hover:bg-amber-100 disabled:opacity-50"><Loader2 className={`size-4 ${pending ? 'animate-spin' : ''}`} /> Status pr&#252;fen</button> : null}    </header>
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(390px,0.82fr)]">
      <div className={`${mobilePreview ? 'hidden' : 'space-y-5'} lg:block`}>
        <Card title={'1. Empf\u00e4nger'} subtitle={'W\u00e4hlen Sie einen Kunden oder geben Sie eine einmalige Adresse an.'}>
          <div className="mb-4 inline-grid grid-cols-2 rounded-xl bg-zinc-100 p-1" role="group" aria-label="Empfaengerart">{(['customer', 'custom'] as const).map(value => <button key={value} type="button" disabled={!editable} aria-pressed={mode === value} onClick={() => setMode(value)} className={`min-h-9 rounded-lg px-3 text-xs font-semibold transition ${mode === value ? 'bg-white text-zinc-950 shadow-sm' : 'text-zinc-500 hover:text-zinc-800'}`}>{value === 'customer' ? 'Kunde' : 'Eigener Empf\u00e4nger'}</button>)}</div>
          {mode === 'customer' ? <label className="block text-xs font-semibold text-zinc-600">Kunde<select disabled={!editable} className={`${field} mt-1`} value={customerId} onChange={event => setCustomerId(event.target.value)}><option value="">Bitte ausw&#228;hlen</option>{customers.map(customer => <option key={customer.id} value={customer.id}>{customer.companyName || customer.name}</option>)}</select>{selectedCustomer ? <span className="mt-2 block rounded-xl bg-zinc-50 p-3 font-normal leading-5 text-zinc-600">{previewRecipient.displayName}{previewRecipient.contactLine ? <><br />{previewRecipient.contactLine}</> : null}<br />{previewRecipient.street}{previewRecipient.addressLine2 ? <><br />{previewRecipient.addressLine2}</> : null}<br />{previewRecipient.postalCode} {previewRecipient.city}<br />{previewRecipient.countryCode}</span> : null}</label> : <div className="grid gap-3 sm:grid-cols-2"><TextField label="Firma / Name" value={recipient.displayName} setValue={value => setRecipient({ ...recipient, displayName: value })} disabled={!editable} /><TextField label="Ansprechpartner (optional)" value={recipient.contactLine || ''} setValue={value => setRecipient({ ...recipient, contactLine: value })} disabled={!editable} /><TextField label={'Stra\u00dfe'} value={recipient.street} setValue={value => setRecipient({ ...recipient, street: value })} disabled={!editable} /><TextField label="Adresszusatz (optional)" value={recipient.addressLine2 || ''} setValue={value => setRecipient({ ...recipient, addressLine2: value })} disabled={!editable} /><TextField label="Postleitzahl" value={recipient.postalCode} setValue={value => setRecipient({ ...recipient, postalCode: value })} disabled={!editable} /><TextField label="Ort" value={recipient.city} setValue={value => setRecipient({ ...recipient, city: value })} disabled={!editable} /><TextField label={'L\u00e4ndercode'} value={recipient.countryCode} setValue={value => setRecipient({ ...recipient, countryCode: value.toUpperCase().slice(0, 2) })} disabled={!editable} /><TextField label="E-Mail (optional)" value={recipient.email || ''} setValue={value => setRecipient({ ...recipient, email: value })} disabled={!editable} type="email" /></div>}
        </Card>
        <Card title="2. Dokument" subtitle="Titel hilft intern beim Wiederfinden; der Betreff erscheint im Schreiben."><div className="grid gap-3 sm:grid-cols-2"><TextField label="Interner Titel" value={title} setValue={setTitle} disabled={!editable} /><TextField label="Ausstellungsdatum" value={issueDate} setValue={setIssueDate} disabled={!editable} type="date" /><div className="sm:col-span-2"><TextField label="Betreff" value={subject} setValue={setSubject} disabled={!editable} /></div></div></Card>
        <Card title="3. Schreiben" subtitle={'Nutzen Sie \u00dcberschriften und Listen sparsam \u2013 der Umbruch erfolgt automatisch.'}><DocumentEditor value={content} onChange={setContent} disabled={!editable} /></Card>
        {editable ? <button type="button" onClick={remove} className="inline-flex items-center gap-2 text-xs font-semibold text-rose-700 hover:text-rose-800"><Trash2 className="size-4" /> Schreiben l&#246;schen</button> : null}
      </div>
    ...
    {confirmFinalize ? <div ref={dialogRef} className="fixed inset-0 z-50 grid place-items-center bg-zinc-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="finalize-title" aria-describedby="finalize-description"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><span className="mb-4 grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-700"><Check /></span><h3 id="finalize-title" className="text-lg font-bold text-zinc-950">Schreiben festschreiben?</h3><p id="finalize-description" className="mt-2 text-sm leading-6 text-zinc-600">Die aktuelle Fassung wird als PDF gespeichert. Danach ist das Schreiben unver\u00e4nderlich und kann nur noch als PDF heruntergeladen werden. Pr\u00fcfen Sie Empf\u00e4nger und Inhalt vor dem Festschreiben.</p><div className="mt-6 flex justify-end gap-2"><button onClick={() => setConfirmFinalize(false)} className="min-h-10 rounded-xl border border-zinc-200 px-4 text-sm font-semibold">Abbrechen</button><button ref={confirmRef} disabled={pending} onClick={() => { setConfirmFinalize(false); finalize(); }} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white">{pending ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />} Endgültig festschreiben</button></div></div></div> : null}
  </section>;
}
