...
export async function getPageWithSectionsAction(pageId: string) {
  ...
  const previewProducts = shopAddonResult[0]?.active
    ? await db.select({ id: products.id, title: products.title, slug: products.slug, priceCents: products.priceCents, comparePriceCents: products.comparePriceCents, images: products.images }).from(products).where(and(eq(products.tenantId, session.tenantId), eq(products.status, 'active'))).limit(20)
    : [];
