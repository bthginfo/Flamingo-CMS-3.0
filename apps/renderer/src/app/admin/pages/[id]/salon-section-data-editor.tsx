...
function ServiceMenuEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Basics d={d} setD={setD} /><Repeater items={d.categories} addLabel="+ Kategorie" onAdd={() => setD({ ...d, categories: [...d.categories, cardListFromData({})] })} render={(item, index) => <div className="space-y-3">{cardFields({ item, index, d, setD, keyName: 'categories' })}<LineListField label="Leistungen" value={item.services} onChange={(v) => updateItem(d, setD, 'categories', index, { ...item, services: v })} placeholder="Leistung" addLabel="Leistung hinzufügen" /></div>} /><ButtonField label="CTA" value={d.ctaPrimary} onChange={(v) => setD({ ...d, ctaPrimary: v })} /></div>;
}
...
function CardRepeaterEditor({ data, onChange, keyName, addLabel, factory, arrayKeys = [], withCta, ctaKey = 'cta' }: { data: Record<string, unknown>; onChange: (data: Record<string, unknown>) => void; keyName: string; addLabel: string; factory: (r: Record<string, unknown>) => any; arrayKeys?: string[]; withCta?: boolean; ctaKey?: string }) {
  ...
  return <div className="space-y-3"><Basics d={d} setD={setD} /><Repeater items={d[keyName]} addLabel={addLabel} onAdd={() => setD({ ...d, [keyName]: [...d[keyName], factory({})] })} render={(item, index) => <div className="space-y-3">{cardFields({ item, index, d, setD, keyName })}{arrayKeys.map((key) => <LineListField key={key} label={listFieldLabel(key)} value={item[key]} onChange={(v) => updateItem(d, setD, keyName, index, { ...item, [key]: v })} placeholder="Eintrag" addLabel="Eintrag hinzufügen" />)}<ButtonField label="CTA" value={item[ctaKey]} onChange={(v) => updateItem(d, setD, keyName, index, { ...item, [ctaKey]: v })} /></div>} />{withCta && <ButtonField label="CTA" value={d.ctaPrimary} onChange={(v) => setD({ ...d, ctaPrimary: v })} />}</div>;
}

function listFieldLabel(key: string) {
  return ({ steps: 'Ablauf', careTips: 'Pflegehinweise', includes: 'Enthaltene Leistungen' } as Record<string, string>)[key] || key;
}
