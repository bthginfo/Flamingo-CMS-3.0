...
function FoodMenuEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} /><Repeater items={d.items} addLabel="+ Gericht" onAdd={() => setD({ ...d, items: [...d.items, { name: '', description: '', price: '', image: '', badge: '' }] })} render={(item, i) => <div className="space-y-2"><div className="grid grid-cols-2 gap-2"><Field label="Name" value={item.name} onChange={v => updateItem(d, setD, 'items', i, { ...item, name: v })} /><Field label="Preis" value={item.price} onChange={v => updateItem(d, setD, 'items', i, { ...item, price: v })} /></div><Field label="Beschreibung" value={item.description} onChange={v => updateItem(d, setD, 'items', i, { ...item, description: v })} /><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'items', i, { ...item, image: v })} /><Field label="Badge (z.B. Vegan)" value={item.badge} onChange={v => updateItem(d, setD, 'items', i, { ...item, badge: v })} /></div>} /></div>;
}
...
function DailySpecialsEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} /><Repeater items={d.specials} addLabel="+ Tagesangebot" onAdd={() => setD({ ...d, specials: [...d.specials, { title: '', description: '', price: '', day: '' }] })} render={(item, i) => <div className="space-y-2"><div className="grid grid-cols-3 gap-2"><Field label="Tag" value={item.day} onChange={v => updateItem(d, setD, 'specials', i, { ...item, day: v })} /><Field label="Titel" value={item.title} onChange={v => updateItem(d, setD, 'specials', i, { ...item, title: v })} /><Field label="Preis" value={item.price} onChange={v => updateItem(d, setD, 'specials', i, { ...item, price: v })} /></div><Field label="Beschreibung" value={item.description} onChange={v => updateItem(d, setD, 'specials', i, { ...item, description: v })} /></div>} /></div>;
}
