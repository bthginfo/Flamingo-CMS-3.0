...
function ValuationCtaEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Field label="CTA Label" value={d.ctaLabel} onChange={v => setD({ ...d, ctaLabel: v })} /><Field label="CTA Link" value={d.ctaHref} onChange={v => setD({ ...d, ctaHref: v })} /></div><ImageUploadField label="Hintergrundbild" value={d.bgImage} onChange={v => setD({ ...d, bgImage: v })} /><Repeater items={d.stats} addLabel="+ Stat" onAdd={() => setD({ ...d, stats: [...d.stats, { label: '', value: '' }] })} render={(item, i) => <div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Field label="Label" value={item.label} onChange={v => updateItem(d, setD, 'stats', i, { ...item, label: v })} /><Field label="Wert" value={item.value} onChange={v => updateItem(d, setD, 'stats', i, { ...item, value: v })} /></div>} /></div>;
}
