...
function PropertyShowcaseEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.properties} addLabel="+ Immobilie" onAdd={() => setD({ ...d, properties: [...d.properties, { title: '', image: '', price: '', size: '', rooms: '', location: '', badge: '', href: '' }] })} render={(item, i) => <div className="space-y-2"><Field label="Titel" value={item.title} onChange={v => updateItem(d, setD, 'properties', i, { ...item, title: v })} /><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'properties', i, { ...item, image: v })} /><div className="grid grid-cols-3 gap-2"><Field label="Preis" value={item.price} onChange={v => updateItem(d, setD, 'properties', i, { ...item, price: v })} /><Field label="Fläche" value={item.size} onChange={v => updateItem(d, setD, 'properties', i, { ...item, size: v })} /><Field label="Zimmer" value={item.rooms} onChange={v => updateItem(d, setD, 'properties', i, { ...item, rooms: v })} /></div><div className="grid grid-cols-2 gap-2"><Field label="Lage" value={item.location} onChange={v => updateItem(d, setD, 'properties', i, { ...item, location: v })} /><Field label="Badge" value={item.badge} onChange={v => updateItem(d, setD, 'properties', i, { ...item, badge: v })} /></div><Field label="Detail-Link" value={item.href} onChange={v => updateItem(d, setD, 'properties', i, { ...item, href: v })} /></div>} /></div>;
}
...
function MarketReportEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} /><Field label="Region" value={d.region} onChange={v => setD({ ...d, region: v })} /><Field label="Beschreibung" value={d.description} onChange={v => setD({ ...d, description: v })} multiline /><Repeater items={d.stats} addLabel="+ Statistik" onAdd={() => setD({ ...d, stats: [...d.stats, { label: '', value: '', trend: '' }] })} render={(item, i) => <div className="grid grid-cols-3 gap-2"><Field label="Label" value={item.label} onChange={v => updateItem(d, setD, 'stats', i, { ...item, label: v })} /><Field label="Wert" value={item.value} onChange={v => updateItem(d, setD, 'stats', i, { ...item, value: v })} /><Field label="Trend (↑/↓)" value={item.trend} onChange={v => updateItem(d, setD, 'stats', i, { ...item, trend: v })} /></div>} /></div>;
}
...
function AgentTeamEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.agents} addLabel="+ Makler" onAdd={() => setD({ ...d, agents: [...d.agents, { name: '', role: '', image: '', specialization: '', phone: '', email: '', soldCount: '' }] })} render={(item, i) => <div className="space-y-2"><div className="grid grid-cols-2 gap-2"><Field label="Name" value={item.name} onChange={v => updateItem(d, setD, 'agents', i, { ...item, name: v })} /><Field label="Rolle" value={item.role} onChange={v => updateItem(d, setD, 'agents', i, { ...item, role: v })} /></div><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'agents', i, { ...item, image: v })} /><Field label="Spezialisierung" value={item.specialization} onChange={v => updateItem(d, setD, 'agents', i, { ...item, specialization: v })} /><div className="grid grid-cols-3 gap-2"><Field label="Telefon" value={item.phone} onChange={v => updateItem(d, setD, 'agents', i, { ...item, phone: v })} /><Field label="E-Mail" value={item.email} onChange={v => updateItem(d, setD, 'agents', i, { ...item, email: v })} /><Field label="Vermittelt" value={item.soldCount} onChange={v => updateItem(d, setD, 'agents', i, { ...item, soldCount: v })} /></div></div>} /></div>;
}
...
function ValuationCtaEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><div className="grid grid-cols-2 gap-2"><Field label="CTA Label" value={d.ctaLabel} onChange={v => setD({ ...d, ctaLabel: v })} /><Field label="CTA Link" value={d.ctaHref} onChange={v => setD({ ...d, ctaHref: v })} /></div><ImageUploadField label="Hintergrundbild" value={d.bgImage} onChange={v => setD({ ...d, bgImage: v })} /><Repeater items={d.stats} addLabel="+ Stat" onAdd={() => setD({ ...d, stats: [...d.stats, { label: '', value: '' }] })} render={(item, i) => <div className="grid grid-cols-2 gap-2"><Field label="Label" value={item.label} onChange={v => updateItem(d, setD, 'stats', i, { ...item, label: v })} /><Field label="Wert" value={item.value} onChange={v => updateItem(d, setD, 'stats', i, { ...item, value: v })} /></div>} /></div>;
}
...
function ReferencesSoldEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} /><Field label="Gesamt vermittelt" value={d.totalSold} onChange={v => setD({ ...d, totalSold: v })} /><Repeater items={d.properties} addLabel="+ Referenz" onAdd={() => setD({ ...d, properties: [...d.properties, { title: '', image: '', location: '', soldIn: '', price: '' }] })} render={(item, i) => <div className="space-y-2"><Field label="Titel" value={item.title} onChange={v => updateItem(d, setD, 'properties', i, { ...item, title: v })} /><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'properties', i, { ...item, image: v })} /><div className="grid grid-cols-3 gap-2"><Field label="Lage" value={item.location} onChange={v => updateItem(d, setD, 'properties', i, { ...item, location: v })} /><Field label="Verkauft in" value={item.soldIn} onChange={v => updateItem(d, setD, 'properties', i, { ...item, soldIn: v })} /><Field label="Preis" value={item.price} onChange={v => updateItem(d, setD, 'properties', i, { ...item, price: v })} /></div></div>} /></div>;
}
