...
function ArtistGridEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.artists} addLabel="+ Künstler" onAdd={() => setD({ ...d, artists: [...d.artists, { name: '', image: '', styles: '', bio: '', instagram: '', href: '' }] })} render={(item, i) => <div className="space-y-2"><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Field label="Name" value={item.name} onChange={v => updateItem(d, setD, 'artists', i, { ...item, name: v })} /><Field label="Instagram" value={item.instagram} onChange={v => updateItem(d, setD, 'artists', i, { ...item, instagram: v })} /></div><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'artists', i, { ...item, image: v })} /><LineListField label="Stile" value={item.styles} onChange={v => updateItem(d, setD, 'artists', i, { ...item, styles: v })} placeholder="z. B. Fine Line" addLabel="Stil hinzufügen" /><Field label="Bio" value={item.bio} onChange={v => updateItem(d, setD, 'artists', i, { ...item, bio: v })} multiline /><DetailLinkField label="Detail-Link" value={item.href} onChange={v => updateItem(d, setD, 'artists', i, { ...item, href: v })} /></div>} /></div>;
}
...
function ArtistHeroEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Name" value={d.name} onChange={v => setD({ ...d, name: v })} /><ImageUploadField label="Bild" value={d.image} onChange={v => setD({ ...d, image: v })} /><Field label="Bio" value={d.bio} onChange={v => setD({ ...d, bio: v })} multiline /><LineListField label="Stile" value={d.styles} onChange={v => setD({ ...d, styles: v })} placeholder="z. B. Realistic" addLabel="Stil hinzufügen" /><Field label="Instagram" value={d.instagram} onChange={v => setD({ ...d, instagram: v })} /><Field label="Erfahrung" value={d.experience} onChange={v => setD({ ...d, experience: v })} /></div>;
}
...
function PricingInfoEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.items} addLabel="+ Preisposition" onAdd={() => setD({ ...d, items: [...d.items, { label: '', value: '', note: '' }] })} render={(item, i) => <div className="space-y-2"><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Field label="Bezeichnung" value={item.label} onChange={v => updateItem(d, setD, 'items', i, { ...item, label: v })} /><Field label="Preis/Wert" value={item.value} onChange={v => updateItem(d, setD, 'items', i, { ...item, value: v })} /></div><Field label="Hinweis" value={item.note} onChange={v => updateItem(d, setD, 'items', i, { ...item, note: v })} /></div>} /><LineListField label="Allgemeine Hinweise" value={d.notes} onChange={v => setD({ ...d, notes: v })} placeholder="Hinweis" addLabel="Hinweis hinzufügen" /></div>;
}
...
function TattooBookingEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><LineListField label="Künstler-Auswahl" value={d.artists} onChange={v => setD({ ...d, artists: v })} placeholder="Künstlername" addLabel="Künstler hinzufügen" /></div>;
}
