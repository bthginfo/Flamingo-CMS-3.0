...
function ArtistGridEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.artists} addLabel="+ Künstler" onAdd={() => setD({ ...d, artists: [...d.artists, { name: '', image: '', styles: '', bio: '', instagram: '', href: '' }] })} render={(item, i) => <div className="space-y-2"><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Field label="Name" value={item.name} onChange={v => updateItem(d, setD, 'artists', i, { ...item, name: v })} /><Field label="Instagram" value={item.instagram} onChange={v => updateItem(d, setD, 'artists', i, { ...item, instagram: v })} /></div><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'artists', i, { ...item, image: v })} /><Field label="Stile (eine pro Zeile)" value={item.styles} onChange={v => updateItem(d, setD, 'artists', i, { ...item, styles: v })} multiline /><Field label="Bio" value={item.bio} onChange={v => updateItem(d, setD, 'artists', i, { ...item, bio: v })} multiline /><Field label="Detail-Link" value={item.href} onChange={v => updateItem(d, setD, 'artists', i, { ...item, href: v })} /></div>} /></div>;
}
...
function TattooBookingCtaEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Field label="CTA Label" value={d.ctaLabel} onChange={v => setD({ ...d, ctaLabel: v })} /><Field label="CTA Link" value={d.ctaHref} onChange={v => setD({ ...d, ctaHref: v })} /></div><Field label="Hinweise (eine pro Zeile)" value={d.hints} onChange={v => setD({ ...d, hints: v })} multiline /></div>;
}
...
function FlashDayBannerEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Datum" value={d.date} onChange={v => setD({ ...d, date: v })} /><Field label="Beschreibung" value={d.description} onChange={v => setD({ ...d, description: v })} multiline /><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Field label="CTA Label" value={d.ctaLabel} onChange={v => setD({ ...d, ctaLabel: v })} /><Field label="CTA Link" value={d.ctaHref} onChange={v => setD({ ...d, ctaHref: v })} /></div><div className="flex items-center gap-2"><label className="text-xs font-medium text-zinc-600">Hintergrundfarbe</label><input type="color" className="w-8 h-8 rounded border cursor-pointer" value={d.bgColor} onChange={e => setD({ ...d, bgColor: e.target.value })} /></div></div>;
}
