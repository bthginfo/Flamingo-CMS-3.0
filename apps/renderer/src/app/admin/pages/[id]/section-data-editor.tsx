...
const FIELD_LABELS: Record<string, string> = {
  ...
  nextLabel: 'Weiter-Button',
  backLabel: 'Zurück-Button',
  trustNote: 'Vertrauenshinweis',
  goals: 'Besucherziele',
  scopeLabel: 'Beschriftung Umfang',
  scopeOptions: 'Optionen Umfang',
  timingLabel: 'Beschriftung Zeitraum',
  timingOptions: 'Optionen Zeitraum',
  budgetLabel: 'Beschriftung Budget',
  budgetOptions: 'Optionen Budget',
  summaryTitle: 'Titel des Live-Briefs',
      ...
      {stats.map((stat, i) => (
        <div key={i} className="border rounded p-3 space-y-2 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <Field label="Nachzeichen" value={stat.suffix} onChange={(v) => setStats(stats.map((s, idx) => idx === i ? { ...s, suffix: v } : s))} placeholder="z.B. +, %, Jahre" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
...
function GalleryGridEditor({ data, onChange }: EditorProps) {
  ...
  async function handleBulkUpload(files: FileList) {
    ...
    const { buildDeterministicUploadPath, resizeImage } = await import('@/components/image-upload-field');
...
function CollectionListEditor({ data, onChange }: EditorProps) {
  ...
  return (
    <div className="space-y-3">
      <Field label={fieldLabel('headline')} value={d.headline} onChange={(v) => setD({ ...d, headline: v })} placeholder="z.B. Alle Beiträge" />
      <Field label={fieldLabel('subline')} value={d.subline} onChange={(v) => setD({ ...d, subline: v })} />
      <CollectionKeySelect value={d.collectionKey} onChange={(v) => setD({ ...d, collectionKey: v })} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <label className="block">
...
const EDITORS: Record<string, React.FC<EditorProps>> = {
  ...
  kineticIdentity: AdvancedSectionEditor,
  signaturePath: AdvancedSectionEditor,
  layeredAnatomy: AdvancedSectionEditor,
  guidedChoice: AdvancedSectionEditor,
  dayToNight: AdvancedSectionEditor,
  livingBlueprint: AdvancedSectionEditor,
  editorialCardMorph: AdvancedSectionEditor,
  materialAtelier: AdvancedSectionEditor,
  verticalReelShowcase: AdvancedSectionEditor,
  aiWorkflowReel: AdvancedSectionEditor,
  cameraExplodeScroll: AdvancedSectionEditor,
  hero: HeroEditor,
  ...
  availabilityCalendar: SchemaSectionEditor,
  ...
  bookingCtaPro: SchemaSectionEditor,
  bookingDateRange: SchemaSectionEditor,
  bookingStrip: SchemaSectionEditor,
  bookingSlotPicker: SchemaSectionEditor,
  bookingWidget: SchemaSectionEditor,
  ...
  resourceBookingShowcase: SchemaSectionEditor,
