...
function AccommodationGridEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Basics d={d} setD={setD} /><Repeater items={d.items} addLabel="+ Unterkunft" onAdd={() => setD({ ...d, items: [...d.items, accommodationFromData({})] })} render={(item, index) => <div className="space-y-3">{imageCardFields({ item, index, d, setD, keyName: 'items' })}<div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Field label="Typ" value={item.typeLabel} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, typeLabel: v })} /><Field label="Preis" value={item.priceLabel} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, priceLabel: v })} /></div><Field label="Ausstattung" value={item.amenities} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, amenities: v })} multiline /></div>} /></div>;
}
