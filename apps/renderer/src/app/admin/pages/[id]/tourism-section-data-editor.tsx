...
function PlacesMapEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Basics d={d} setD={setD} /><Field label="Karten-Embed-URL" value={d.mapEmbedUrl} onChange={(v) => setD({ ...d, mapEmbedUrl: v })} multiline /><Field label="Fallback-Text ohne Karte" value={d.mapFallbackText} onChange={(v) => setD({ ...d, mapFallbackText: v })} /><Repeater items={d.places} addLabel="+ Ort" onAdd={() => setD({ ...d, places: [...d.places, placeFromData({})] })} render={(item, index) => <div className="space-y-3"><Field label="Titel" value={item.title} onChange={(v) => updateItem(d, setD, 'places', index, { ...item, title: v })} /><Field label="Beschreibung" value={item.text} onChange={(v) => updateItem(d, setD, 'places', index, { ...item, text: v })} multiline /><ImageUploadField label="Bild" value={item.image} onChange={(v) => updateItem(d, setD, 'places', index, { ...item, image: v })} /><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3"><Field label="Kategorie" value={item.category} onChange={(v) => updateItem(d, setD, 'places', index, { ...item, category: v })} /><Field label="Distanz" value={item.distanceLabel} onChange={(v) => updateItem(d, setD, 'places', index, { ...item, distanceLabel: v })} /><Field label="Adresse" value={item.address} onChange={(v) => updateItem(d, setD, 'places', index, { ...item, address: v })} /></div><ButtonField label="Ort-CTA" value={item.cta} onChange={(v) => updateItem(d, setD, 'places', index, { ...item, cta: v })} /></div>} /><ButtonField label="Sektion-CTA" value={d.ctaPrimary} onChange={(v) => setD({ ...d, ctaPrimary: v })} /></div>;
}
...
function SightseeingListEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Basics d={d} setD={setD} /><Repeater items={d.items} addLabel="+ Sehenswuerdigkeit" onAdd={() => setD({ ...d, items: [...d.items, sightFromData({})] })} render={(item, index) => <div className="space-y-3">{imageCardFields({ item, index, d, setD, keyName: 'items' })}<div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Field label="Oeffnungszeiten" value={item.openingText} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, openingText: v })} /><Field label="Kategorie" value={item.category} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, category: v })} /></div></div>} /></div>;
}
...
function AccommodationGridEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Basics d={d} setD={setD} /><Repeater items={d.items} addLabel="+ Unterkunft" onAdd={() => setD({ ...d, items: [...d.items, accommodationFromData({})] })} render={(item, index) => <div className="space-y-3">{imageCardFields({ item, index, d, setD, keyName: 'items' })}<div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Field label="Typ" value={item.typeLabel} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, typeLabel: v })} /><Field label="Preis" value={item.priceLabel} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, priceLabel: v })} /></div><LineListField label="Ausstattung" value={item.amenities} onChange={(v) => updateItem(d, setD, 'items', index, { ...item, amenities: v })} placeholder="Ausstattungsmerkmal" addLabel="Merkmal hinzufügen" /></div>} /></div>;
}
