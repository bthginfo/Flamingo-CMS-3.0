...
function HeroEditor({ data, onChange }: EditorProps) {
  const [d, setD] = useState({ coupleName: str(data.coupleName), date: str(data.date), venue: str(data.venue), bgImage: str(data.bgImage), overlayColor: str(data.overlayColor), overlayOpacity: str(data.overlayOpacity), tagline: str(data.tagline) });
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Paarnamen (z.B. Anna & Max)" value={d.coupleName} onChange={v => setD({ ...d, coupleName: v })} /><Field label="Datum (YYYY-MM-DD)" value={d.date} onChange={v => setD({ ...d, date: v })} /><Field label="Location" value={d.venue} onChange={v => setD({ ...d, venue: v })} /><Field label="Tagline" value={d.tagline} onChange={v => setD({ ...d, tagline: v })} /><ImageUploadField label="Hintergrundbild" value={d.bgImage} onChange={v => setD({ ...d, bgImage: v })} /><div className="grid grid-cols-2 gap-3"><Field label="Overlay-Farbe" value={d.overlayColor} onChange={v => setD({ ...d, overlayColor: v })} /><Field label="Overlay-Opacity (0-1)" value={d.overlayOpacity} onChange={v => setD({ ...d, overlayOpacity: v })} /></div></div>;
}
