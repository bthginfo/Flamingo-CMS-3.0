...
function HeroEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Paarnamen (z.B. Anna & Max)" value={d.coupleName} onChange={v => setD({ ...d, coupleName: v })} /><Field label="Datum (YYYY-MM-DD)" value={d.date} onChange={v => setD({ ...d, date: v })} /><Field label="Location" value={d.venue} onChange={v => setD({ ...d, venue: v })} /><Field label="Tagline" value={d.tagline} onChange={v => setD({ ...d, tagline: v })} /><ImageUploadField label="Hintergrundbild" value={d.bgImage} onChange={v => setD({ ...d, bgImage: v })} /><div className="grid grid-cols-2 gap-3"><Field label="Overlay-Farbe" value={d.overlayColor} onChange={v => setD({ ...d, overlayColor: v })} /><Field label="Overlay-Opacity (0-1)" value={d.overlayOpacity} onChange={v => setD({ ...d, overlayOpacity: v })} /></div><div><label className="text-xs font-medium text-zinc-600 mb-1 block">Bild-Effekt</label><select className="admin-input" value={d.imageEffect} onChange={(e) => setD({ ...d, imageEffect: e.target.value })}><option value="none">Kein Effekt</option><option value="parallax">Parallax</option><option value="kenBurns">Ken Burns (Zoom)</option><option value="mouseGlow">Mouse Glow</option></select>{d.imageEffect !== 'none' && (<select className="admin-input mt-2" value={d.imageEffectIntensity} onChange={(e) => setD({ ...d, imageEffectIntensity: e.target.value })}><option value="subtle">Dezent</option><option value="medium">Mittel</option><option value="strong">Stark</option></select>)}</div></div>;
}
...
function CoupleStoryEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.milestones} addLabel="+ Meilenstein" onAdd={() => setD({ ...d, milestones: [...d.milestones, milestoneFromData({})] })} render={(item, index) => <div className="space-y-3"><div className="grid grid-cols-2 gap-3"><Field label="Jahr" value={item.year} onChange={v => updateItem(d, setD, 'milestones', index, { ...item, year: v })} /><Field label="Titel" value={item.title} onChange={v => updateItem(d, setD, 'milestones', index, { ...item, title: v })} /></div><Field label="Text" value={item.text} onChange={v => updateItem(d, setD, 'milestones', index, { ...item, text: v })} multiline /><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'milestones', index, { ...item, image: v })} /></div>} /></div>;
}
...
function EventScheduleEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.events} addLabel="+ Programmpunkt" onAdd={() => setD({ ...d, events: [...d.events, eventFromData({})] })} render={(item, index) => <div className="space-y-3"><div className="grid grid-cols-2 gap-3"><Field label="Uhrzeit" value={item.time} onChange={v => updateItem(d, setD, 'events', index, { ...item, time: v })} /><Field label="Titel" value={item.title} onChange={v => updateItem(d, setD, 'events', index, { ...item, title: v })} /></div><Field label="Beschreibung" value={item.description} onChange={v => updateItem(d, setD, 'events', index, { ...item, description: v })} /><div className="grid grid-cols-2 gap-3"><IconPickerField label="Icon" value={item.icon} onChange={v => updateItem(d, setD, 'events', index, { ...item, icon: v })} /><Field label="Ort" value={item.location} onChange={v => updateItem(d, setD, 'events', index, { ...item, location: v })} /></div></div>} /></div>;
}
...
function TravelInfoEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><h4 className="text-xs font-medium text-gray-500 mt-4">Anreise-Optionen</h4><Repeater items={d.sections} addLabel="+ Option" onAdd={() => setD({ ...d, sections: [...d.sections, travelSectionFromData({})] })} render={(item, index) => <div className="space-y-3"><IconPickerField label="Icon" value={item.icon} onChange={v => updateItem(d, setD, 'sections', index, { ...item, icon: v })} /><Field label="Titel" value={item.title} onChange={v => updateItem(d, setD, 'sections', index, { ...item, title: v })} /><Field label="Inhalt" value={item.content} onChange={v => updateItem(d, setD, 'sections', index, { ...item, content: v })} multiline /></div>} /><h4 className="text-xs font-medium text-gray-500 mt-4">Hotels</h4><Repeater items={d.hotels} addLabel="+ Hotel" onAdd={() => setD({ ...d, hotels: [...d.hotels, hotelFromData({})] })} render={(item, index) => <div className="space-y-3"><Field label="Name" value={item.name} onChange={v => updateItem(d, setD, 'hotels', index, { ...item, name: v })} /><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'hotels', index, { ...item, image: v })} /><div className="grid grid-cols-2 gap-3"><Field label="Link" value={item.link} onChange={v => updateItem(d, setD, 'hotels', index, { ...item, link: v })} /><Field label="Entfernung" value={item.distance} onChange={v => updateItem(d, setD, 'hotels', index, { ...item, distance: v })} /></div><div className="grid grid-cols-2 gap-3"><Field label="Sonderkonditionen" value={item.specialRate} onChange={v => updateItem(d, setD, 'hotels', index, { ...item, specialRate: v })} /><Field label="Sterne" value={item.stars} onChange={v => updateItem(d, setD, 'hotels', index, { ...item, stars: v })} /></div></div>} /></div>;
}
...
function WeddingPartyEditor({ data, onChange }: EditorProps) {
  ...
  useReport(d, onChange);
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Repeater items={d.members} addLabel="+ Person" onAdd={() => setD({ ...d, members: [...d.members, memberFromData({})] })} render={(item, index) => <div className="space-y-3"><div className="grid grid-cols-2 gap-3"><Field label="Name" value={item.name} onChange={v => updateItem(d, setD, 'members', index, { ...item, name: v })} /><Field label="Rolle" value={item.role} onChange={v => updateItem(d, setD, 'members', index, { ...item, role: v })} /></div><Field label="Beziehung" value={item.relationship} onChange={v => updateItem(d, setD, 'members', index, { ...item, relationship: v })} /><Field label="Text" value={item.text} onChange={v => updateItem(d, setD, 'members', index, { ...item, text: v })} multiline /><ImageUploadField label="Bild" value={item.image} onChange={v => updateItem(d, setD, 'members', index, { ...item, image: v })} /></div>} /></div>;
}
...
function GiftRegistryEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><Field label="Freitext" value={d.freeText} onChange={v => setD({ ...d, freeText: v })} multiline /><Repeater items={d.gifts} addLabel="+ Geschenk" onAdd={() => setD({ ...d, gifts: [...d.gifts, giftFromData({})] })} render={(item, index) => <div className="space-y-3"><div className="grid grid-cols-2 gap-3"><Field label="Titel" value={item.title} onChange={v => updateItem(d, setD, 'gifts', index, { ...item, title: v })} /><Field label="Preis" value={item.price} onChange={v => updateItem(d, setD, 'gifts', index, { ...item, price: v })} /></div><Field label="Beschreibung" value={item.description} onChange={v => updateItem(d, setD, 'gifts', index, { ...item, description: v })} /><DetailLinkField label="Link" value={item.link} onChange={v => updateItem(d, setD, 'gifts', index, { ...item, link: v })} /><Checkbox label="Bereits reserviert" checked={item.claimed} onChange={v => updateItem(d, setD, 'gifts', index, { ...item, claimed: v })} /></div>} /><h4 className="text-xs font-medium text-gray-500 mt-4">Bankverbindung</h4><div className="grid grid-cols-2 gap-3"><Field label="Inhaber" value={d.bankHolder} onChange={v => setD({ ...d, bankHolder: v })} /><Field label="IBAN" value={d.bankIban} onChange={v => setD({ ...d, bankIban: v })} /></div><div className="grid grid-cols-2 gap-3"><Field label="BIC" value={d.bankBic} onChange={v => setD({ ...d, bankBic: v })} /><Field label="Hinweis" value={d.bankNote} onChange={v => setD({ ...d, bankNote: v })} /></div></div>;
}
...
function RsvpEditor({ data, onChange }: EditorProps) {
  ...
  return <div className="space-y-3"><Field label="Headline" value={d.headline} onChange={v => setD({ ...d, headline: v })} /><Field label="Subline" value={d.subline} onChange={v => setD({ ...d, subline: v })} multiline /><div className="grid grid-cols-2 gap-3"><Field label="Anmeldeschluss" value={d.deadline} onChange={v => setD({ ...d, deadline: v })} /><Field label="Max. Gäste pro Anmeldung" value={d.maxGuests} onChange={v => setD({ ...d, maxGuests: v })} /></div><Checkbox label="Musikwunsch anzeigen" checked={d.showSongWish} onChange={v => setD({ ...d, showSongWish: v })} /><Checkbox label="Ernährung anzeigen" checked={d.showDietary} onChange={v => setD({ ...d, showDietary: v })} /><Checkbox label="Allergien anzeigen" checked={d.showAllergies} onChange={v => setD({ ...d, showAllergies: v })} /></div>;
}
