...
import { getSeoPageAction, saveSeoPageAction } from '../../seo/actions';
import { Search } from 'lucide-react';

export type PageSeoPanelHandle = { save: () => Promise<void>; isDirty: () => boolean };

export const PageSeoPanel = forwardRef<PageSeoPanelHandle, { pageId: string; onDirty?: () => void }>(function PageSeoPanel({ pageId, onDirty }, ref) {
  ...
  useEffect(() => {
    getSeoPageAction(pageId).then(row => {
      if (row) {
        const d = {
  ...
  useImperativeHandle(ref, () => ({
    save: async () => {
      ...
      await saveSeoPageAction(pageId, data);
  ...
  return (
    <div className="admin-card mb-4">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium hover:bg-gray-50">
        <span className="flex items-center gap-2"><Search size={16} /> SEO-Einstellungen</span>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t pt-3">
          <div>
            ...
            <input className="admin-input" value={data.metaTitle} onChange={e => update({ metaTitle: e.target.value })} placeholder="Seitentitel für Suchmaschinen" maxLength={70} />
