...
export function ProductForm({ categories, initial }: { categories: Category[]; initial?: ProductData }) {
  ...
  const { isOpen: previewOpen, open: openPreview, close: closePreview, setUrl: setPreviewUrl } = usePreview();
  const [data, setData] = useState<ProductData>(initial || defaultProduct);
  const [saving, setSaving] = useState(false);
  const isEdit = Boolean(initial?.id);
  const productPreviewUrl = isEdit && data.slug ? `/shop/${data.slug}` : '/live-preview';

  useEffect(() => {
    setPreviewUrl(productPreviewUrl);
    return () => {
      setPreviewUrl('/live-preview');
    };
  }, [productPreviewUrl, setPreviewUrl]);
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-5 space-y-4">
        ...
        <p className="text-xs text-zinc-400">Das erste Bild wird als Hauptbild verwendet.</p>
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-5 space-y-4">
        ...
        <p className="text-xs text-zinc-400">Werden auf der Produktdetailseite als Auflistung angezeigt.</p>
        {data.highlights.map((h, i) => (
          <div key={i} className="flex gap-2">
            ...
            <button
              type="button"
              onClick={() => set('highlights', data.highlights.filter((_, j) => j !== i))}
        ...
        <button
          type="button"
          onClick={() => set('highlights', [...data.highlights, ''])}
        ...
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-zinc-200 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300"
              placeholder="Falls leer, wird der Produkttitel verwendet"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 rounded-lg border border-zinc-200 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300 resize-y"
              placeholder="Empfohlen: ca. 156 Zeichen"
            />
          </div>
        </div>
      </div>
