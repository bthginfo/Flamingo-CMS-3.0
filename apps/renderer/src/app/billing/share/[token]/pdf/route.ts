...
export async function GET(_: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const [row] = await getDb().select({ documentNumber: billingDocuments.documentNumber, pdfBase64: billingDocuments.pdfBase64, pdfBlobUrl: billingDocuments.pdfBlobUrl, status: billingDocuments.status })
