    ...
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {plans.map((plan, index) => (
          <motion.article key={index} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.06 }} className={`relative flex min-h-[460px] flex-col rounded-3xl border p-6 shadow-sm ${plan.highlighted ? 'border-[var(--token-card-border,var(--style-accent-color,var(--token-icon, var(--brand-primary))))] bg-[#070707] text-[color:var(--token-on-dark-heading,#ffffff)] shadow-2xl' : 'border-[var(--token-card-border,var(--style-border-color,rgba(0,0,0,0.08)))] bg-[var(--token-card-bg,var(--style-card-bg,#fff))] text-[var(--token-body,var(--style-body-color,#27272a))]'}`}>
            {plan.highlighted && <div className="absolute right-5 top-5 rounded-full bg-[var(--token-card-bg,#ffffff)] px-3 py-1 text-xs font-black uppercase text-[color:var(--token-heading,#09090b)]">Empfohlen</div>}
            <div className={`text-sm font-bold uppercase tracking-[0.18em] ${plan.highlighted ? 'text-[color:var(--token-on-dark-heading,#ffffff)/58]' : 'text-[var(--token-eyebrow,var(--style-accent-color,var(--token-icon, var(--brand-primary))))]'}`}>{plan.name}</div>
