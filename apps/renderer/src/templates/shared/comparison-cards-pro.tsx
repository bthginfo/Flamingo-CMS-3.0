    ...
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {plans.map((plan, index) => (
          <motion.article key={index} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.06 }} className={`relative flex min-h-[460px] flex-col overflow-hidden rounded-3xl border bg-[var(--token-card-bg)] p-6 text-[color:var(--token-card-body,var(--token-body))] shadow-sm transition-all duration-300 motion-safe:hover:-translate-y-1 hover:shadow-xl ${plan.highlighted ? 'border-[var(--token-btn-bg)] shadow-2xl ring-2 ring-[color-mix(in_srgb,var(--token-btn-bg)_18%,transparent)]' : 'border-[var(--token-card-border)]'}`} data-card data-edit-collection="plans" data-edit-index={index}>
