...
export function PremiumComparisonSection({ data }: Props) {
  ...
  return (
    <div>
      ...
      <div className="overflow-x-auto rounded-[var(--token-card-radius)] border border-[var(--token-card-border)] bg-[var(--token-card-bg)] shadow-xl" role="region" aria-label={headline || 'Angebotsvergleich'} tabIndex={0}>
        <div className="grid min-w-[720px]" style={{ gridTemplateColumns: `minmax(180px,1.2fr) repeat(${columns.length}, minmax(140px,1fr))` }}>
          ...
          {columns.map((col, i) => (
            <div key={i} className={`border-b border-l border-[var(--token-card-border)] p-5 text-center ${i === highlightCol ? 'bg-[var(--token-btn-bg)] text-[color:var(--token-btn-text)]' : ''}`} data-card data-edit-collection="columns" data-edit-index={i}>
