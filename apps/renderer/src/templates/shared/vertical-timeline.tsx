...
export function VerticalTimelineSection({ data }: Props) {
  ...
  const accentColor = (data.accentColor as string) || '#18181b';
  const lineColor = (data.lineColor as string) || '#e4e4e7';
    ...
    >
      ...
      <div className="pb-2">
        <div className="flex items-baseline gap-3 mb-1">
          ...
          {step.timeLabel && <span className="text-xs text-zinc-400">{step.timeLabel}</span>}
        </div>
        <h3 className="text-xl md:text-2xl font-bold mt-1">{step.title}</h3>
        {step.text && <p className="mt-2 text-zinc-500 leading-relaxed">{step.text}</p>}
