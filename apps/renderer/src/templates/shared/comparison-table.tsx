        ...
        <table className="w-full min-w-[820px] border-collapse text-sm">
          <thead>
            <tr>
              ...
              {columns.map((col, i) => (
                <th key={i} className={`border-b border-r last:border-r-0 border-[color:var(--token-card-border)] px-7 py-6 text-center text-sm font-extrabold md:px-8 ${i === highlightCol ? 'bg-[color:color-mix(in_srgb,var(--token-accent,#f59e0b)_15%,var(--token-card-bg,#fff))] text-[color:var(--token-accent)]' : 'bg-[color:var(--token-card-bg)] text-[color:var(--token-heading)]'}`} data-edit-collection="columns" data-edit-index={i}>
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr key={ri} className="group transition-colors" data-edit-collection="rows" data-edit-index={ri}>
                <td className="border-b border-r border-[color:var(--token-card-border)] bg-[color:var(--token-card-bg)] px-7 py-6 font-bold leading-relaxed text-[color:var(--token-heading)] transition group-last:border-b-0 group-hover:bg-[color:color-mix(in_srgb,var(--token-section-bg-alt,#f8fafc)_58%,var(--token-card-bg,#fff))] md:px-8">{row.feature || `Kriterium ${ri + 1}`}</td>
