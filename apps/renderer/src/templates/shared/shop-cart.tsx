  ...
  if (items.length === 0) {
    return (
      <section className="py-16 text-center">
        ...
        <Link href="/shop" className="inline-block px-6 py-3 bg-zinc-900 text-white rounded-xl font-medium hover:bg-zinc-800 transition">
          {continueLabel}
        </Link>
      </section>
    );
    ...
    <section className="py-12 md:py-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map(item => (
            <div key={`${item.productId}-${item.variantId || ''}`} className="flex gap-4 p-4 bg-white rounded-xl border border-zinc-100">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/shop/${item.slug}`} className="font-medium text-sm hover:underline">{item.title}</Link>
        ...
        <div className="bg-zinc-50 rounded-2xl p-6 h-fit sticky top-4">
          ...
          <Link href="/checkout" className="block w-full text-center py-3 mt-4 bg-zinc-900 text-white font-medium rounded-xl hover:bg-zinc-800 transition">
            {checkoutLabel}
          </Link>
          <Link href="/shop" className="block w-full text-center py-2 mt-2 text-sm text-zinc-500 hover:text-zinc-700">
            {continueLabel}
          </Link>
        </div>
      </div>
