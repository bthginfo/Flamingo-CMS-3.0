  ...
  if (items.length === 0) {
    return (
      <section className="py-16 text-center">
        <ShoppingBag size={48} className="mx-auto mb-4 text-[color:var(--token-body)]" />
        ...
        <Link href={shopBase} className="inline-block px-6 py-3 bg-[var(--token-section-bg-alt)] text-[color:var(--token-on-dark-heading)] rounded-xl font-medium hover:bg-[var(--token-section-bg-alt)] transition">
          {continueLabel}
        </Link>
      </section>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map(item => (
            <div key={`${item.productId}-${item.variantId || ''}`} className="flex gap-4 p-4 bg-[var(--token-card-bg)] rounded-xl border border-[color:var(--token-card-border)]">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.variantName && <p className="text-xs text-[color:var(--token-body)]">{item.variantName}</p>}
                <p className="font-semibold mt-1">{formatPrice(item.priceCents)}</p>
              </div>
              <div className="flex flex-col items-end gap-2">
                <button onClick={() => removeItem(item.productId, item.variantId)} className="p-1 text-[color:var(--token-body)] hover:text-red-500">
                  <Trash2 size={16} />
                </button>
        ...
        <div className="bg-[var(--token-section-bg-alt)] rounded-2xl p-6 h-fit sticky top-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between text-[color:var(--token-body)]">
              <span>Versand</span>
          ...
          {shopBase.startsWith('/demo/') ? (
            <div className="mt-4 text-center">
              <span className="block w-full py-3 bg-zinc-200 text-[color:var(--token-muted)] font-medium rounded-xl cursor-not-allowed">Zur Kasse</span>
              <p className="text-xs text-[color:var(--token-body)] mt-2">Checkout ist in der Demo nicht verfügbar</p>
            </div>
          ) : (
          ...
          <Link href={shopBase} className="block w-full text-center py-2 mt-2 text-sm text-[color:var(--token-muted)] hover:text-[color:var(--token-muted)]">
            {continueLabel}
          </Link>
        </div>
      </div>
