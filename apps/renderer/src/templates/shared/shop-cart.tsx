      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map(item => (
            <div key={`${item.productId}-${item.variantId || ''}`} className="flex gap-4 p-4 bg-[var(--token-card-bg,#ffffff)] rounded-xl border border-[color:var(--token-card-border,#f4f4f5)]">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.variantName && <p className="text-xs text-[color:var(--token-body,#a1a1aa)]">{item.variantName}</p>}
                <p className="font-semibold mt-1">{formatPrice(item.priceCents)}</p>
              </div>
              <div className="flex flex-col items-end gap-2">
                <button onClick={() => removeItem(item.productId, item.variantId)} className="p-1 text-[color:var(--token-body,#a1a1aa)] hover:text-red-500">
                  <Trash2 size={16} />
                </button>
        ...
        <div className="bg-[var(--token-section-bg-alt,#fafafa)] rounded-2xl p-6 h-fit sticky top-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between text-[color:var(--token-body,#a1a1aa)]">
              <span>Versand</span>
          ...
          {shopBase.startsWith('/demo/') ? (
            <div className="mt-4 text-center">
              <span className="block w-full py-3 bg-zinc-200 text-[color:var(--token-muted,#71717a)] font-medium rounded-xl cursor-not-allowed">Zur Kasse</span>
              <p className="text-xs text-[color:var(--token-body,#a1a1aa)] mt-2">Checkout ist in der Demo nicht verfügbar</p>
            </div>
          ) : (
          ...
          <Link href={shopBase} className="block w-full text-center py-2 mt-2 text-sm text-[color:var(--token-muted,#71717a)] hover:text-[color:var(--token-muted,#3f3f46)]">
            {continueLabel}
          </Link>
        </div>
      </div>
