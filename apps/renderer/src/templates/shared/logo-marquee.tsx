    ...
    <div ref={ref} className="overflow-hidden -mx-6 md:-mx-12 lg:-mx-20 py-8">
      {(headline || subline) && (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4 }} className="text-center mb-8 px-6">
          {headline && <p className="text-sm font-medium uppercase tracking-widest text-[color:var(--token-body,#a1a1aa)]" data-edit-path="headline">{headline}</p>}
          {subline && <p className="text-xs text-[color:var(--token-body,#a1a1aa)] mt-1" data-edit-path="subline">{plain(subline)}</p>}
        </motion.div>
      )}
            ...
            {doubled.map((item, i) => (
              <div key={i} className="flex-shrink-0 flex items-center justify-center h-12 opacity-50 hover:opacity-100 transition-opacity duration-300 grayscale hover:grayscale-0" data-edit-collection="doubled" data-edit-index={i}>
                {item.image ? (
                  ...
                  <span className="text-lg font-bold text-[color:var(--token-body,#a1a1aa)] whitespace-nowrap" data-edit-path="name">{item.name}</span>
                )}
              </div>
            ))}
          </div>
