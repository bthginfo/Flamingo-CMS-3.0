...
export function ShopProductGridSection({ data }: Props) {
  ...
  return (
    <section className="py-16 md:py-24 px-4 md:px-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8">
        {showSearch && (
          <div className="relative flex-1">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[color:var(--token-body)]" />
            <input
              type="text"
              value={search}
        ...
        <div className="flex items-center gap-2">
          <SlidersHorizontal size={14} className="text-[color:var(--token-body)]" />
      ...
      {/* Product grid */}
      {loading ? (
        <div className="text-center py-16 text-[color:var(--token-body)]">Produkte werden geladen…</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 text-[color:var(--token-body)]">
                ...
                <div className="aspect-[4/5] bg-[var(--token-section-bg-alt)] relative overflow-hidden">
                  {product.images?.[0] ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <ShoppingBag size={40} className="text-[color:var(--token-body)]" />
                    </div>
                  )}
                  {product.stock === 0 && (
                  ...
                  {product.comparePriceCents && (
                    <div className="absolute top-3 right-3 bg-green-500 text-[color:var(--token-on-dark-heading)] text-xs font-bold px-2.5 py-1 rounded-full">Sale</div>
                  )}
                </div>
                <div className="p-5">
                  {product.categoryName && <p className="text-xs text-[color:var(--token-body)] uppercase tracking-wide mb-1">{product.categoryName}</p>}
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    {product.comparePriceCents && (
                      <span className="text-sm text-[color:var(--token-body)] line-through">{formatPrice(product.comparePriceCents)}</span>
                    )}
                  </div>
                </div>
              </div>
