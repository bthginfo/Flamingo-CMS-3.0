...
export function CinematicHeroSection({ data }: Props) {
  ...
  const mediaContent = (
    <>
      {videoUrl ? (
        ...
        <div className="h-full w-full bg-[var(--style-section-bg-alt,var(--brand-dark,#09090b))]" />
      )}
  ...
  return (
    <section ref={ref} className="relative min-h-[100svh] overflow-hidden bg-[var(--style-section-bg,#000)]" style={{ color: heroBody }}>
      ...
      <motion.div style={{ y: copyY, opacity }} className={`relative z-10 mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-end px-6 pb-16 pt-32 md:pb-24 ${align === 'center' ? 'items-center text-center' : 'items-start text-left'}`}>
        {eyebrow && <div className="mb-5 inline-flex rounded-full border border-white/20 bg-[var(--style-badge-bg,rgba(255,255,255,0.14))] px-4 py-2 text-xs font-semibold uppercase text-[var(--style-badge-text,#fff)] backdrop-blur">{eyebrow}</div>}
        ...
        <div className="mt-9 flex flex-wrap items-center gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full bg-[var(--brand-btn-bg,var(--style-accent-color,#fff))] px-6 py-3 text-sm font-bold text-[var(--brand-btn-text,#111)] shadow-xl transition hover:brightness-110">{primaryCta.label}<ArrowRight size={16} /></a>}
