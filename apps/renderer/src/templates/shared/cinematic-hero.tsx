  ...
  return (
    <section ref={ref} className="relative min-h-[100svh] overflow-hidden bg-[var(--token-section-bg)]" style={{ color: heroBody }}>
      ...
      <motion.div style={{ y: copyY, opacity }} className={`relative z-10 mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-center px-6 pb-14 pt-28 md:pb-16 md:pt-32 ${align === 'center' ? 'items-center text-center' : 'items-start text-left'}`}>
        {eyebrow && <div className="mb-5 inline-flex rounded-full border border-[var(--token-badge-border)] bg-[var(--token-badge-bg)] px-4 py-2 text-xs font-semibold uppercase text-[color:var(--token-eyebrow,var(--token-badge-text))] backdrop-blur" data-edit-path="eyebrow">{eyebrow}</div>}
        ...
        <div className="mt-9 flex flex-wrap items-center gap-3">
          ...
          {secondaryCta.label && <a data-edit-link="secondaryCta" href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full border border-[var(--token-btn-secondary-border,var(--token-card-border))] bg-white/10 px-6 py-3 text-sm font-bold backdrop-blur transition hover:bg-white/15" style={{ background: 'var(--token-badge-bg)', color: 'var(--token-badge-text,var(--token-on-dark-heading))' }}><Play size={15} /><span data-edit-path="label">{secondaryCta.label}</span></a>}
        </div>

        {facts.length > 0 && (
          <div className="mt-10 grid w-full max-w-3xl grid-cols-2 gap-3 md:mt-12 md:grid-cols-4 md:gap-4">
            {facts.map((fact, i) => (
              <div key={i} className="rounded-2xl border border-[color:var(--token-card-border,rgba(255,255,255,0.22))] bg-transparent p-4 md:p-5" style={{ backgroundColor: 'transparent' }} data-edit-collection="facts" data-edit-index={i}>
