      ...
      <div className="relative z-10 mx-auto grid min-h-[620px] max-w-7xl items-end gap-10 px-6 py-16 md:grid-cols-[1.1fr_0.9fr] md:py-24">
        <div>
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            ...
            {secondaryCta.label && <a data-edit-link="secondaryCta" href={secondaryCta.href || '#'} className="btn-secondary inline-flex items-center gap-2 rounded-full border px-6 py-3 text-sm font-bold backdrop-blur transition hover:brightness-110" style={{ background: 'var(--token-btn-secondary-bg,color-mix(in_srgb,var(--token-on-dark-heading)_10%,transparent))', color: 'var(--token-btn-secondary-text,var(--token-on-dark-heading))', borderColor: 'var(--token-btn-secondary-border,color-mix(in_srgb,var(--token-on-dark-heading)_28%,transparent))' }} data-edit-path="label">{secondaryCta.label}</a>}
          </div>
        </div>
