          ...
          {steps.map((step, i) => (
            <motion.div
             ...
             data-edit-collection="steps" data-edit-index={i}>
              {/* Step circle */}
              <div className="relative mx-auto w-24 h-24 rounded-full bg-[color-mix(in_srgb,var(--token-card-icon)_5%,transparent)] border-2 border-[color-mix(in_srgb,var(--token-card-icon)_20%,transparent)] flex flex-col items-center justify-center mb-5">
                {step.icon ? (
                  <DynamicIcon editPath="icon" name={step.icon} size={28} className="text-[color:var(--token-card-icon)]" />
                ) : (
                  <span className="text-2xl font-bold text-[color:var(--token-card-icon)]">{step.number || i + 1}</span>
                )}
              </div>

              {/* Arrow between steps (desktop) */}
              {i < steps.length - 1 && (
                <div className="hidden lg:block absolute top-12 -right-2 text-[color-mix(in_srgb,var(--token-card-icon)_30%,transparent)] text-xl">→</div>
              )}
