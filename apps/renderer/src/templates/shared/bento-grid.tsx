      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-[minmax(180px,auto)]">
        {items.map((item, i) => (
          <motion.div
            ...
            className={`group relative overflow-hidden rounded-2xl bg-[var(--token-card-bg, var(--style-card-bg,#fafafa))] border border-[var(--token-card-border, var(--style-border-color,rgba(228,228,231,0.6)))] p-6 cursor-default transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${getSpanClass(item, i)}`}
          >
            {/* Hover spotlight */}
            ...
            {item.image && (
              <div className="absolute inset-0 z-0">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[var(--token-card-bg, var(--style-card-bg,#ffffff))] via-[color-mix(in_srgb,var(--token-card-bg, var(--style-card-bg,#ffffff))_80%,transparent)] to-transparent" />
              </div>
            )}

            <div className="relative z-10 flex flex-col h-full justify-end">
              {item.icon && <div className="text-3xl mb-3 text-[var(--token-icon, var(--style-icon-color,var(--color-primary)))]"><DynamicIcon name={item.icon} size={32} /></div>}
              <h3 className="text-lg font-semibold text-[var(--token-heading, var(--style-heading-color,var(--style-text-primary,#18181b)))] mb-1" data-edit-path="title">{item.title}</h3>
              {item.description && <p className="text-sm text-[var(--token-body, var(--style-body-color,var(--style-text-secondary,#52525b)))] leading-relaxed" data-edit-path="description">{plain(item.description)}</p>}
            </div>
          </motion.div>
        ))}
      </div>
