...
export function StatsCounterSection({ data }: Props) {
  ...
  return (
    <div ref={ref} className="relative overflow-hidden rounded-2xl bg-[var(--token-section-bg-alt, var(--token-section-bg-alt, var(--style-section-bg-alt,#18181b)))] py-20 px-6 text-[var(--style-text-primary,#ffffff)]">
      ...
      <div className="relative z-10">
        {(headline || badge) && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-16">
            {badge && <span className="inline-block text-xs font-semibold tracking-widest uppercase text-[var(--token-badge-text, var(--token-badge-text, var(--style-badge-text,var(--color-primary))))] bg-[var(--token-badge-bg, var(--token-badge-bg, var(--style-badge-bg,rgba(255,255,255,0.10))))] rounded-full px-4 py-1.5 mb-4">{badge}</span>}
            {headline && <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--token-heading, var(--token-heading, var(--style-heading-color,var(--style-text-primary,#ffffff))))]">{headline}</h2>}
            {subline && <p className="mt-3 text-lg text-[var(--token-body, var(--token-body, var(--style-body-color,var(--style-text-secondary,#a1a1aa))))] max-w-xl mx-auto">{plain(subline)}</p>}
          </motion.div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4 max-w-5xl mx-auto">
          {stats.map((stat, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: i * 0.15 }} className="text-center">
              ...
              <div className="mt-2 text-sm md:text-base text-[var(--style-text-secondary,#a1a1aa)] font-medium">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
