      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-[var(--token-card-radius)] border border-[var(--token-card-border)] bg-[var(--token-card-bg)] p-6 text-[color:var(--token-body)] shadow-[0_30px_120px_rgba(0,0,0,0.35)] md:p-8">
        ...
        {(primaryCta.label || secondaryCta.label) && (
          <div className="relative mt-7 flex flex-col gap-3 sm:flex-row">
            ...
            {secondaryCta.label && (
              <a data-edit-link="secondaryCta" href={secondaryCta.href || '#'} onClick={close} className="inline-flex items-center justify-center rounded-[var(--token-button-radius)] border border-[var(--token-btn-secondary-border)] bg-[color-mix(in_srgb,var(--token-card-bg)_70%,transparent)] px-5 py-3 text-sm font-bold text-[color:var(--token-btn-secondary-text)] transition hover:bg-[var(--token-btn-secondary-bg)]" data-edit-path="label">
                {secondaryCta.label}
              </a>
            )}
          </div>
