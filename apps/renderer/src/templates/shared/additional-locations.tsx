    ...
    <section className="overflow-hidden bg-[var(--token-section-bg)] py-16 text-[var(--token-body)] md:py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
          <div>
            {badge && (
              <div className="mb-4 inline-flex rounded-full bg-[var(--token-badge-bg)] px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[var(--token-badge-text)]" data-edit-path="badge">
                {badge}
              </div>
            )}
            {headline && <h2 className="max-w-3xl text-4xl font-black leading-tight text-[var(--token-heading)] md:text-6xl" data-edit-path="headline">{headline}</h2>}
          </div>
          {subline && <p className="max-w-2xl text-base leading-8 text-[var(--token-subheading)] md:text-lg" data-edit-path="subline">{plain(subline)}</p>}
        </div>
              ...
              >
                ...
                <div className="p-6">
                  <div className="mb-5 flex items-start gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[var(--token-card-badge-bg, var(--token-badge-bg))] text-[var(--token-icon)]">
                      <MapPin size={22} />
                    </div>
                    <div>
                      {location.name && <h3 className="text-xl font-black leading-tight text-[var(--token-card-heading, var(--token-heading))]" data-edit-path="name">{location.name}</h3>}
                      {location.address && <p className="mt-2 text-sm leading-6 text-[var(--token-muted)]" data-edit-path="address">{location.address}</p>}
                    </div>
                  </div>

                  <div className="space-y-3 text-sm">
                    {location.openingHours && (
                      <div className="flex gap-3 text-[var(--token-card-body, var(--token-body))]">
                        <Clock size={17} className="mt-0.5 shrink-0 text-[var(--token-icon)]" />
                        <span className="whitespace-pre-line leading-6">{location.openingHours}</span>
                      </div>
                    )}
                    {location.phone && (
                      <a href={`tel:$<span data-edit-path="phone">{location.phone}</span>`} className="flex gap-3 text-[var(--token-card-body, var(--token-body))] transition hover:text-[var(--token-accent)]">
                        <Phone size={17} className="mt-0.5 shrink-0 text-[var(--token-icon)]" />
                        <span data-edit-path="phone">{location.phone}</span>
                      </a>
                    )}
                    {email && (
                      <a href={`mailto:$<span data-edit-path="email">{email}</span>`} className="flex gap-3 break-all text-[var(--token-card-body, var(--token-body))] transition hover:text-[var(--token-accent)]">
                  ...
                  {(location.ctaLabel || location.address) && (
                    <a
                      ...
                      className="mt-6 inline-flex items-center gap-2 rounded-full bg-[var(--token-btn-bg)] px-5 py-3 text-sm font-bold text-[var(--token-btn-text)] transition hover:brightness-110"
                    >
