    ...
    <section className="overflow-hidden bg-[var(--token-section-bg, var(--style-section-bg,#f8fafc))] py-16 text-[var(--token-body, var(--style-body-color,var(--style-text-primary,#111827)))] md:py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
          <div>
            {badge && (
              <div className="mb-4 inline-flex rounded-full bg-[var(--token-badge-bg, var(--style-badge-bg,rgba(0,0,0,0.06)))] px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[var(--token-badge-text, var(--style-badge-text,var(--brand-primary,#111827)))]">
                {badge}
              </div>
            )}
              ...
              >
                ...
                <div className="p-6">
                  <div className="mb-5 flex items-start gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[var(--token-badge-bg, var(--style-badge-bg,rgba(0,0,0,0.06)))] text-[var(--token-icon, var(--style-icon-color,var(--brand-primary,#111827)))]">
                      <MapPin size={22} />
                    </div>
                    <div>
                  ...
                  <div className="space-y-3 text-sm">
                    {location.openingHours && (
                      <div className="flex gap-3 text-[var(--token-body, var(--style-body-color,var(--style-text-primary,#111827)))]">
                        <Clock size={17} className="mt-0.5 shrink-0 text-[var(--token-icon, var(--style-icon-color,var(--brand-primary,#111827)))]" />
                        <span className="whitespace-pre-line leading-6">{location.openingHours}</span>
                      </div>
                    )}
                    {location.phone && (
                      <a href={`tel:${location.phone}`} className="flex gap-3 text-[var(--token-body, var(--style-body-color,var(--style-text-primary,#111827)))] transition hover:text-[var(--style-accent-color,var(--brand-primary,#111827))]">
                        <Phone size={17} className="mt-0.5 shrink-0 text-[var(--token-icon, var(--style-icon-color,var(--brand-primary,#111827)))]" />
                        <span>{location.phone}</span>
                      </a>
                    )}
                    {email && (
                      <a href={`mailto:${email}`} className="flex gap-3 break-all text-[var(--token-body, var(--style-body-color,var(--style-text-primary,#111827)))] transition hover:text-[var(--style-accent-color,var(--brand-primary,#111827))]">
                        <Mail size={17} className="mt-0.5 shrink-0 text-[var(--token-icon, var(--style-icon-color,var(--brand-primary,#111827)))]" />
                        <span>{email}</span>
                      </a>
                    )}
                  </div>
                  ...
                  {(location.ctaLabel || location.address) && (
                    <a
                      ...
                      className="mt-6 inline-flex items-center gap-2 rounded-full bg-[var(--token-btn-bg, var(--brand-btn-bg,var(--brand-primary,#111827)))] px-5 py-3 text-sm font-bold text-[var(--token-btn-text, var(--brand-btn-text,#ffffff))] transition hover:brightness-110"
                    >
