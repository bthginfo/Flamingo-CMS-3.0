...
const FORM_CONTROL_CLASS = 'rounded-xl border border-[var(--booking-border-color,rgba(9,9,11,.16))] bg-[var(--booking-card-bg,#fff)] px-4 py-3 text-sm text-[var(--booking-text-secondary,#09090b)] outline-none placeholder:text-[var(--booking-muted-color,rgba(9,9,11,.45))] focus:border-[var(--token-btn-bg)]';
  ...
  return (
    ...
    >
      ...
      <div className="relative grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        ...
        <div className="rounded-[var(--token-card-radius)] border p-4 shadow-xl sm:p-6" style={{ borderColor: 'var(--booking-border-color, rgba(255,255,255,.1))', background: 'var(--booking-card-bg, #ffffff)', color: 'var(--booking-text-secondary, #09090b)' }}>
...
export function BookingSlotPickerSection({ data }: SectionProps) {
  ...
  return (
    <section id="booking" className="relative overflow-hidden rounded-[var(--token-card-radius)] px-5 py-10 shadow-2xl sm:px-8 md:px-12" style={{ background: 'var(--booking-section-bg, #09090b)', color: 'var(--booking-text-primary, #ffffff)' }}>
      ...
      <div className="relative grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          ...
          <div className="mt-6 grid gap-3 text-sm sm:grid-cols-3">
            {['Datum wählen', 'Slots prüfen', config?.mode === 'instant' ? 'Direkt buchen' : 'Anfrage senden'].map((item, index) => (
              <div key={item} className="rounded-2xl border p-3" style={{ borderColor: 'var(--booking-border-color, rgba(255,255,255,.14))', background: 'color-mix(in srgb, var(--booking-card-bg, #ffffff) 8%, transparent)', color: 'var(--booking-body-color, rgba(255,255,255,.72))' }} data-edit-collection="senden" data-edit-index={index}>
                <span className="mb-2 grid h-7 w-7 place-items-center rounded-full text-xs font-black" style={{ background: 'var(--booking-accent-color, #f43f5e)', color: 'var(--token-btn-text)' }}>{index + 1}</span>
                {item}
              </div>
            ))}
          </div>
        ...
        <form onSubmit={submit} className="rounded-[var(--token-card-radius)] border p-4 shadow-xl sm:p-6" style={{ borderColor: 'var(--booking-border-color, rgba(255,255,255,.1))', background: 'var(--booking-card-bg, #ffffff)', color: 'var(--booking-text-secondary, #09090b)' }}>
  ...
  return (
    <section id="booking" className="relative overflow-hidden rounded-[var(--token-card-radius)] px-5 py-10 shadow-2xl sm:px-8 md:px-12" style={{ background: 'var(--booking-section-bg, #09090b)', color: 'var(--booking-text-primary, #ffffff)' }}>
      ...
      <div className="relative grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-start">
        ...
        <form onSubmit={submit} className="grid gap-4 rounded-[var(--token-card-radius)] border p-4 shadow-xl sm:p-6" style={{ borderColor: 'var(--booking-border-color, rgba(255,255,255,.1))', background: 'var(--booking-card-bg, #ffffff)', color: 'var(--booking-text-secondary, #09090b)' }}>
          {status === 'success' ? (
            <div className="flex min-h-80 flex-col items-center justify-center gap-3 text-center">
              <CheckCircle style={{ color: 'var(--booking-accent-color, #10b981)' }} size={44} />
      ...
      <div className="mx-auto w-full max-w-sm rounded-2xl border p-3 shadow-xl" style={{ borderColor: 'var(--booking-border-color, rgba(255,255,255,.14))', background: 'color-mix(in srgb, var(--booking-card-bg, #ffffff) 10%, transparent)' }}>
        ...
        <div className="grid grid-cols-7 gap-1">
          {['Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa', 'So'].map((label) => (
            <div key={label} className="py-1 text-center text-[10px] font-bold uppercase tracking-[0.14em]" style={{ color: 'var(--booking-muted-color, rgba(255,255,255,.52))' }} data-edit-path="label">{label}</div>
          ))}
          {days.map((day) => (
          <button key={day.date} type="button" onClick={() => setSelectedDate(day.date)} className="relative grid aspect-square place-items-center rounded-lg border text-sm font-bold transition hover:brightness-110" style={{ borderColor: selectedDate === day.date ? 'var(--token-btn-bg)' : 'var(--booking-border-color, rgba(255,255,255,.14))', background: selectedDate === day.date ? 'var(--token-btn-bg)' : day.inMonth ? 'transparent' : 'rgba(255,255,255,.03)', color: selectedDate === day.date ? 'var(--token-btn-text)' : 'var(--booking-heading-color, #ffffff)', opacity: day.inMonth ? 1 : 0.42 }} aria-label={`${day.label}, ${day.day}. ${counts[day.date] ?? 0} freie Slots`}>
            {day.day}
            {day.isToday ? <span className="absolute left-1 top-1 h-1.5 w-1.5 rounded-full" style={{ background: selectedDate === day.date ? 'var(--token-btn-text)' : 'var(--booking-accent-color, #f43f5e)' }} /> : null}
            {(counts[day.date] || 0) > 0 ? <span className="absolute bottom-1 h-1.5 w-1.5 rounded-full" style={{ background: selectedDate === day.date ? 'var(--token-btn-text)' : 'var(--booking-accent-color, #f43f5e)' }} /> : null}
          </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-3">
        {['Ressource wählen', 'Zeitraum prüfen', 'Anfrage senden'].map((label, index) => (
          <div key={label} className="flex items-center gap-3 rounded-2xl border p-4" style={{ borderColor: 'var(--booking-border-color, rgba(255,255,255,.14))', background: 'color-mix(in srgb, var(--booking-card-bg, #ffffff) 10%, transparent)' }} data-edit-collection="senden" data-edit-index={index}>
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full font-black" style={{ background: 'var(--booking-accent-color, #f43f5e)', color: 'var(--token-btn-text)' }}>{index + 1}</span>
