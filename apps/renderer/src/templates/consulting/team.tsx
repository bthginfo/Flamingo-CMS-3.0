    ...
    <div ref={ref}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {members.map((member, i) => (
          ...
          >
            {member.image && (
              <div className="w-48 h-48 mx-auto mb-5 rounded-full overflow-hidden border-4 border-slate-100 shadow-md group-hover:border-[var(--token-card-border,var(--brand-primary,#1a5276))/30] transition-colors">
            ...
            <div className="flex items-center justify-center gap-4 mt-4">
              {member.phone && (
                <a href={`tel:${member.phone}`} className="text-slate-400 hover:text-[color:var(--token-icon,var(--brand-primary,#1a5276))] transition-colors">
                  <DynamicIcon name="phone" size={16} />
                </a>
              )}
              {member.email && (
                <a href={`mailto:${member.email}`} className="text-slate-400 hover:text-[color:var(--token-icon,var(--brand-primary,#1a5276))] transition-colors">
