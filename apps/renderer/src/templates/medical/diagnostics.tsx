      ...
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="grid gap-6 md:grid-cols-3">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden rounded-xl bg-[var(--token-card-bg, var(--token-card-bg, var(--style-card-bg,#fff)))] shadow-lg">
            ...
            <div className="p-5">
              {(item.methodLabel || item.benefitLabel) && <p className="text-xs font-bold uppercase tracking-widest text-[var(--token-badge-text, var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,var(--token-icon, var(--brand-primary))))))]">{[item.methodLabel, item.benefitLabel].filter(Boolean).join(' / ')}</p>}
              ...
              {item.cta?.label && <a href={item.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 rounded-full bg-[var(--token-btn-bg, var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--token-icon, var(--brand-primary))))))] px-5 py-2.5 text-sm font-semibold text-[var(--token-btn-text, var(--token-btn-text, var(--brand-btn-text,#fff)))]">{item.cta.label}<ArrowRight size={14} /></a>}
            </div>
          </article>
        ))}
      </motion.div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden border border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,rgba(0,0,0,.1))))] bg-[var(--token-card-bg, var(--token-card-bg, var(--style-card-bg,#fff)))]">
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.title || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-5">
              {(item.methodLabel || item.benefitLabel) && <p className="text-xs font-light uppercase tracking-widest text-[var(--token-badge-text, var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,var(--token-icon, var(--brand-primary))))))]">{[item.methodLabel, item.benefitLabel].filter(Boolean).join(' / ')}</p>}
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden border-2 border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,var(--style-text-primary,#111827))))] bg-[var(--token-card-bg, var(--token-card-bg, var(--style-card-bg,#fff)))] shadow-[4px_4px_0_var(--token-card-border, var(--token-card-border, var(--style-border-color,var(--style-text-primary,#111827))))]">
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.title || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-5">
              {(item.methodLabel || item.benefitLabel) && <p className="text-xs font-black uppercase tracking-widest text-[var(--token-badge-text, var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,var(--token-icon, var(--brand-primary))))))]">{[item.methodLabel, item.benefitLabel].filter(Boolean).join(' / ')}</p>}
