        ...
        {days.map((day, index) => (
          <div key={`${day.label}-${index}`} className="flex items-center justify-between gap-4 border-b border-[var(--token-card-border, var(--style-border-color,rgba(0,0,0,.1)))] px-5 py-4 last:border-b-0">
            <div className="flex items-center gap-3">
              <Clock size={17} className="text-[var(--token-icon, var(--style-icon-color,var(--style-accent-color,var(--brand-primary))))]" />
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        ...
        {ctaPrimary.label && <div className="mt-6"><a href={ctaPrimary.href || '#'} className="inline-flex items-center gap-2 rounded-lg border border-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary))))] bg-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary))))] px-5 py-3 font-semibold text-[var(--token-btn-text, var(--brand-btn-text,#fff))]">{ctaPrimary.label}<ArrowRight size={16} /></a></div>}
      </div>
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        ...
        {acuteCareText && <p className="text-sm leading-6 text-[var(--token-body, var(--style-body-color,var(--style-text-secondary,#4b5563)))]">{acuteCareText}</p>}
        {holidayNote && <p className="mt-3 text-xs text-[var(--token-muted, var(--style-text-muted,var(--style-text-secondary,#4b5563)))]">{holidayNote}</p>}
        {ctaPrimary.label && <div className="mt-6"><a href={ctaPrimary.href || '#'} className="inline-flex items-center gap-2 border-2 border-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary))))] bg-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary))))] px-5 py-3 font-black uppercase text-[var(--token-btn-text, var(--brand-btn-text,#fff))]">{ctaPrimary.label}<ArrowRight size={16} /></a></div>}
      </div>
