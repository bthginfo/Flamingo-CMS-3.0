  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        ...
        {acuteCareText && <p className="text-sm leading-6 text-[var(--style-text-secondary)]">{acuteCareText}</p>}
        ...
        {days.map((day, index) => (
          <div key={`${day.label}-${index}`} className="flex items-center justify-between gap-4 border-b border-black/10 px-5 py-4 last:border-b-0">
            <div className="flex items-center gap-3">
              ...
              <div><p className="font-semibold text-[var(--style-text-primary)]">{day.label || ''}</p>{day.note && <p className="text-xs text-[var(--style-text-secondary)]">{day.note}</p>}</div>
            </div>
...
function Modern({ header, days, acuteCareText, holidayNote, ctaPrimary }: Props) {
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        <SectionHeader {...header} />
        {acuteCareText && <p className="text-sm font-light leading-6 text-[var(--style-text-secondary)]">{acuteCareText}</p>}
        {holidayNote && <p className="mt-3 text-xs font-light text-[var(--style-text-secondary)]">{holidayNote}</p>}
        {ctaPrimary.label && <div className="mt-6"><a href={ctaPrimary.href || '#'} className="inline-flex items-center gap-2 rounded-[var(--style-button-radius)] border border-blue-600 bg-blue-600 px-5 py-3 font-semibold text-white">{ctaPrimary.label}<ArrowRight size={16} /></a></div>}
      </div>
      <div className="border border-black/10 bg-[var(--style-card-bg)]">
        {days.map((day, index) => (
          <div key={`${day.label}-${index}`} className="flex items-center justify-between gap-4 border-b border-black/10 px-5 py-4 last:border-b-0">
            <div className="flex items-center gap-3">
              <Clock size={17} className="text-blue-500" />
              <div><p className="font-light text-[var(--style-text-primary)]">{day.label || ''}</p>{day.note && <p className="text-xs font-light text-[var(--style-text-secondary)]">{day.note}</p>}</div>
            </div>
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="border-2 border-[var(--style-text-primary)] bg-[var(--style-card-bg)] shadow-[4px_4px_0_var(--style-text-primary)]">
        {days.map((day, index) => (
          <div key={`${day.label}-${index}`} className="flex items-center justify-between gap-4 border-b-2 border-[var(--style-text-primary)] px-5 py-4 last:border-b-0">
            <div className="flex items-center gap-3">
              <Clock size={17} className="text-teal-400" />
              <div><p className="font-black uppercase text-[var(--style-text-primary)]">{day.label || ''}</p>{day.note && <p className="text-xs text-[var(--style-text-secondary)]">{day.note}</p>}</div>
            </div>
            <p className="text-sm font-bold text-[var(--style-text-primary)]">{day.closed ? (day.note || '') : day.hours}</p>
          </div>
        ))}
      </div>
    </div>
