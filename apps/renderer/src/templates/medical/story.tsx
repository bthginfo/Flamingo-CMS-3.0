    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          {p.storyText && <p className="whitespace-pre-line leading-7 text-[color:var(--token-on-dark-muted,#52525b)]">{plain(p.storyText)}</p>}
          {p.founderQuote && (
  ...
  return (
    <div>
      <div className="grid gap-16 lg:grid-cols-2 lg:items-start">
        <div>
          <SectionHeader headline={p.headline} subline={plain(p.subline)} badgeText={p.badgeText} />
          {p.storyText && <p className="whitespace-pre-line font-light leading-7 text-[color:var(--token-on-dark-muted,#52525b)]">{plain(p.storyText)}</p>}
          {p.founderQuote && (
        ...
        <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {p.values.map((v, i) => (
            <div key={`${v.title}-${i}`} className="border-l border-black/20 pl-5">
              <DynamicIcon name={v.icon || 'stethoscope'} size={18} className="text-[color:var(--token-on-dark-muted,#52525b)]" />
