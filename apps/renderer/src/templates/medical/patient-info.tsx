...
function Modern({ header, introText, cards }: Props) {
  return (
    <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {cards.map((card, index) => (
          <article key={`${card.title}-${index}`} className="border border-black/10 bg-[var(--style-card-bg)] p-5">
            <IconRows items={[card]} />
            <div className="mt-4 flex flex-wrap gap-2">{asList<string>(card.items).map((item) => <span key={item} className="border border-black/10 px-3 py-1 text-xs text-[var(--style-text-secondary)]">{item}</span>)}</div>
          </article>
        ))}
      </div>
    </div>
...
function Bold({ header, introText, cards }: Props) {
  return (
    <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {cards.map((card, index) => (
          <article key={`${card.title}-${index}`} className="border-2 border-[var(--style-text-primary)] bg-[var(--style-card-bg)] p-5 shadow-[4px_4px_0_var(--style-text-primary)]">
            <IconRows items={[card]} />
