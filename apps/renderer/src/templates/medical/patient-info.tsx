    ...
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {cards.map((card, index) => (
          <article key={`${card.title}-${index}`} className="rounded-xl bg-[var(--token-card-bg,#ffffff)] p-5 shadow-lg">
            <IconRows items={[card]} />
...
function Modern({ header, introText, cards }: Props) {
  return (
    <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {cards.map((card, index) => (
          <article key={`${card.title}-${index}`} className="border border-black/10 bg-[var(--token-card-bg,#ffffff)] p-5">
            <IconRows items={[card]} />
            <div className="mt-4 flex flex-wrap gap-2">{asList<string>(card.items).map((item) => <span key={item} className="border border-black/10 px-3 py-1 text-xs text-[color:var(--token-on-dark-muted,#52525b)]">{item}</span>)}</div>
          </article>
        ))}
      </div>
    </div>
...
function Bold({ header, introText, cards }: Props) {
  return (
    <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {cards.map((card, index) => (
          <article key={`${card.title}-${index}`} className="border-2 border-[#111827] bg-[var(--token-card-bg,#ffffff)] p-5 shadow-[4px_4px_0_#111827]">
            <IconRows items={[card]} />
