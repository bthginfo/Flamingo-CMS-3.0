...
function HeroClassic({ headline, subline, badgeText, badgeIcon, bgImage, bgImageMobile, bgColor, bgMode, specialtyLabel, emergencyHint, trustItems, primaryCta, emergencyCta, secondaryCta, overlayColor, overlayOpacity, bgPosition, bgPositionMobile, imageEffect, imageEffectIntensity}: HeroProps) {
  return (
    <section className="relative min-h-screen overflow-hidden -mt-[112px] pt-[112px] bg-[var(--token-section-bg, var(--style-section-bg,#111827))]">
      ...
      <svg className="absolute bottom-0 left-0 w-full text-[var(--token-card-bg, var(--style-card-bg,#ffffff))]" viewBox="0 0 1440 120" preserveAspectRatio="none" aria-hidden="true">
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-112px)] max-w-7xl flex-col items-center justify-center px-6 py-12 md:py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-4 flex items-center gap-2">
          <DynamicIcon name={badgeIcon} className="text-[var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,#67e8f9)))]" size={20} />
          {badgeText && <p className="text-xs font-bold uppercase tracking-widest text-[var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,#67e8f9)))]" data-edit-path="badgeText">{badgeText}</p>}
          <Stethoscope className="text-[var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,#67e8f9)))]" size={20} />
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }} className="max-w-5xl text-3xl md:text-5xl font-[700] leading-[0.95] text-[var(--token-heading, var(--style-heading-color,#fff))] sm:text-6xl lg:text-8xl" style={{ textShadow: '0 2px 20px rgba(0,0,0,0.5)' }} data-edit-path="headline">{headline}</motion.h1>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.45 }} className="mt-8 flex flex-wrap justify-center gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full bg-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,#22d3ee)))] px-6 py-3 font-semibold text-[var(--token-btn-text, var(--brand-btn-text,#134e4a))]"><span data-edit-path="label">{primaryCta.label}</span>{primaryCta.icon && <DynamicIcon name={primaryCta.icon} size={17} />}</a>}
          {emergencyCta.label && <a href={emergencyCta.href || '#'} className="inline-flex items-center gap-2 rounded-full bg-red-600 px-6 py-3 font-semibold text-[color:var(--token-on-dark-heading,#ffffff)]">{emergencyCta.icon && <DynamicIcon name={emergencyCta.icon} size={17} />}<span data-edit-path="label">{emergencyCta.label}</span></a>}
          {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full border border-[var(--token-card-border, var(--style-border-color,rgba(255,255,255,.35)))] px-6 py-3 font-semibold text-[var(--token-heading, var(--style-heading-color,#fff))]" data-edit-path="label">{secondaryCta.label}</a>}
        </motion.div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.6 }} className="mt-10 flex flex-wrap justify-center gap-3 text-sm text-[var(--token-on-dark-body,var(--token-body,#ffffff))]">
          {specialtyLabel && <span className="inline-flex items-center gap-2 rounded-full bg-[var(--token-badge-bg, var(--style-badge-bg,rgba(0,0,0,.25)))] px-4 py-2"><Heart size={15} className="text-[var(--token-icon, var(--style-icon-color,var(--style-accent-color,#67e8f9)))]" />{specialtyLabel}</span>}
          {emergencyHint && <span className="rounded-full bg-red-900/40 px-4 py-2">{emergencyHint}</span>}
          {trustItems.map((item) => <span key={item} className="inline-flex items-center gap-1.5 rounded-full bg-[var(--token-badge-bg, var(--style-badge-bg,rgba(0,0,0,.25)))] px-4 py-2"><CheckCircle size={14} className="text-[var(--token-icon, var(--style-icon-color,var(--style-accent-color,#67e8f9)))]" />{item}</span>)}
        </motion.div>
      </div>
    </section>
  );
    ...
    <section className="relative min-h-screen overflow-hidden -mt-[112px] pt-[112px] bg-[var(--token-section-bg, var(--style-section-bg,#fff))]">
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-112px)] max-w-7xl items-center gap-10 px-6 py-12 md:py-20 lg:grid-cols-2">
        <div className="max-w-xl">
          {badgeText && <p className="text-xs font-light uppercase tracking-widest text-[var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,var(--token-icon, var(--brand-primary)))))]" data-edit-path="badgeText">{badgeText}</p>}
          <h1 className="mt-5 text-3xl md:text-5xl font-light leading-[0.95] text-[var(--token-heading, var(--style-heading-color,var(--style-text-primary,#111827)))] sm:text-6xl lg:text-7xl" data-edit-path="headline">{headline}</h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-lg border border-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--token-icon, var(--brand-primary)))))] bg-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--token-icon, var(--brand-primary)))))] px-5 py-3 font-semibold text-[var(--token-btn-text, var(--brand-btn-text,#fff))]"><span data-edit-path="label">{primaryCta.label}</span>{primaryCta.icon && <DynamicIcon name={primaryCta.icon} size={17} />}</a>}
            {emergencyCta.label && <a href={emergencyCta.href || '#'} className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-5 py-3 font-semibold text-[color:var(--token-on-dark-heading,#ffffff)]">{emergencyCta.icon && <DynamicIcon name={emergencyCta.icon} size={17} />}<span data-edit-path="label">{emergencyCta.label}</span></a>}
            {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-lg border border-[var(--token-card-border, var(--style-border-color,rgba(0,0,0,.15)))] px-5 py-3 font-semibold text-[var(--token-heading, var(--style-heading-color,var(--style-text-primary,#111827)))]" data-edit-path="label">{secondaryCta.label}</a>}
          </div>
          <div className="mt-10 flex flex-wrap gap-3 text-sm text-[var(--token-body, var(--style-body-color,var(--style-text-secondary,#4b5563)))]">
            {specialtyLabel && <span className="inline-flex items-center gap-2 rounded-full border border-[var(--token-card-border, var(--style-border-color,rgba(0,0,0,.1)))] px-4 py-2"><Heart size={15} />{specialtyLabel}</span>}
        ...
        <div className="relative min-h-[500px] overflow-hidden rounded-xl border border-[var(--token-card-border, var(--style-border-color,rgba(0,0,0,.1)))] bg-[var(--token-card-bg, var(--style-card-bg,#fff))]">
          {(bgMode === 'image' && bgImage) ? (
            <>
...
function HeroBold({ headline, subline, badgeText, badgeIcon, bgImage, bgImageMobile, bgColor, bgMode, specialtyLabel, emergencyHint, trustItems, primaryCta, emergencyCta, secondaryCta , bgPosition, bgPositionMobile, imageEffect, imageEffectIntensity}: HeroProps) {
  return (
    <section className="relative min-h-screen overflow-hidden -mt-[112px] pt-[112px] bg-[var(--token-section-bg, var(--style-section-bg,#030712))]">
      {(bgMode === 'image' && bgImage) ? (
        <>
          ...
          <div className="absolute inset-0 bg-[color-mix(in_srgb,var(--token-section-bg, var(--style-section-bg,#030712))_60%,transparent)]" />
        </>
      ) : (bgMode === 'color' && bgColor) ? (
        <div className="absolute inset-0" style={{ backgroundColor: bgColor }} />
      ) : null}
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-112px)] max-w-7xl flex-col justify-center px-6 py-12 md:py-20">
        <div className="flex items-center gap-3">
          ...
          {badgeText && <p className="text-xs font-black uppercase tracking-widest text-[var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,var(--token-icon, var(--brand-primary)))))]" data-edit-path="badgeText">{badgeText}</p>}
        </div>
        <h1 className="mt-5 max-w-5xl text-3xl md:text-5xl font-black uppercase leading-[0.95] text-[var(--token-heading, var(--style-heading-color,#fff))] sm:text-6xl lg:text-8xl" data-edit-path="headline">{headline}</h1>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 border-2 border-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--token-icon, var(--brand-primary)))))] bg-[var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,var(--token-icon, var(--brand-primary)))))] px-6 py-3 font-black uppercase text-[var(--token-btn-text, var(--brand-btn-text,#fff))]"><span data-edit-path="label">{primaryCta.label}</span>{primaryCta.icon && <DynamicIcon name={primaryCta.icon} size={17} />}</a>}
          ...
          {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 border-2 border-[var(--token-card-border, var(--style-border-color,rgba(255,255,255,.4)))] px-6 py-3 font-black uppercase text-[var(--token-heading, var(--style-heading-color,#fff))]" data-edit-path="label">{secondaryCta.label}</a>}
        </div>
