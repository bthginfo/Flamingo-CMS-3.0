      ...
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="grid gap-4 md:grid-cols-3">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="rounded-xl bg-[var(--token-card-bg)] p-5 shadow-lg" data-edit-collection="items" data-edit-index={index}>
            <Download size={22} className="text-[var(--token-success,theme(colors.teal.700))]" />
            {item.metaLabel && <p className="mt-4 text-xs font-bold uppercase tracking-widest text-[var(--token-success,theme(colors.teal.700))]">{item.metaLabel}</p>}
            ...
            {item.fileLabel && <a href={item.fileHref || '#'} className="mt-5 inline-flex font-semibold text-[var(--token-success,theme(colors.teal.700))]">{item.fileLabel}</a>}
          </article>
        ))}
      </motion.div>
    </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="border border-black/10 bg-[var(--token-card-bg)] p-5" data-edit-collection="items" data-edit-index={index}>
            <Download size={22} className="text-[var(--token-accent,theme(colors.blue.500))]" />
            {item.metaLabel && <p className="mt-4 text-xs font-light uppercase tracking-widest text-[var(--token-accent,theme(colors.blue.500))]">{item.metaLabel}</p>}
            ...
            {item.fileLabel && <a href={item.fileHref || '#'} className="mt-5 inline-flex font-semibold text-[var(--token-accent,theme(colors.blue.600))]">{item.fileLabel}</a>}
          </article>
        ))}
      </div>
    </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {items.map((item, index) => (
          <article key={`${item.title || 'item'}-${index}`} className="border-2 border-[var(--token-card-border)] bg-[var(--token-card-bg)] p-5 shadow-[4px_4px_0_var(--token-card-border)]" data-edit-collection="items" data-edit-index={index}>
            <Download size={22} className="text-[var(--token-success,theme(colors.teal.400))]" />
            {item.metaLabel && <p className="mt-4 text-xs font-black uppercase tracking-widest text-[var(--token-success,theme(colors.teal.500))]">{item.metaLabel}</p>}
            <h3 className="mt-2 text-xl font-black uppercase text-[color:var(--token-heading)]" data-edit-path="title">{item.title || ''}</h3>
            {item.text && <div className="mt-3 text-sm leading-6 text-[color:var(--token-muted)] rt-content" data-edit-rich="text" dangerouslySetInnerHTML={{ __html: item.text }} />}
            {item.fileLabel && <a href={item.fileHref || '#'} className="mt-5 inline-flex font-black uppercase text-[var(--token-success,theme(colors.teal.400))]">{item.fileLabel}</a>}
          </article>
        ))}
      </div>
    </div>
