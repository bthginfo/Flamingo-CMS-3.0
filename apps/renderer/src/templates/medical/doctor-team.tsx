        ...
        {doctors.map((item, index) => (
          <article key={`${item.name}-${index}`} className="group overflow-hidden rounded-xl bg-white shadow-lg">
            ...
            <div className="p-5">
              ...
              <h3 className="mt-2 text-xl font-bold text-gray-900">{[item.title, item.name].filter(Boolean).join(' ')}</h3>
              {item.bio && <div className="mt-3 whitespace-pre-line text-sm leading-6 text-gray-600 rt-content" dangerouslySetInnerHTML={{ __html: item.bio }} />}
              {item.languages && item.languages.length > 0 && <p className="mt-2 text-xs text-gray-600">{item.languages.join(' / ')}</p>}
        ...
        {doctors.map((item, index) => (
          <article key={`${item.name}-${index}`} className="group overflow-hidden border border-black/10 bg-white">
            ...
            <div className="p-5">
              ...
              <h3 className="mt-2 text-xl font-light text-gray-900">{[item.title, item.name].filter(Boolean).join(' ')}</h3>
              {item.bio && <div className="mt-3 whitespace-pre-line text-sm font-light leading-6 text-gray-600 rt-content" dangerouslySetInnerHTML={{ __html: item.bio }} />}
              {item.languages && item.languages.length > 0 && <p className="mt-2 text-xs font-light text-gray-600">{item.languages.join(' / ')}</p>}
              {item.appointmentCta?.label && <a href={item.appointmentCta.href || '#'} className="mt-4 inline-flex items-center gap-2 rounded-lg border border-blue-600 bg-blue-600 px-4 py-2 text-sm font-semibold text-white">{item.appointmentCta.label}<ArrowRight size={14} /></a>}
            </div>
          </article>
        ))}
      </div>
        ...
        {doctors.map((item, index) => (
          <article key={`${item.name}-${index}`} className="group overflow-hidden border-2 border-[#111827] bg-white shadow-[4px_4px_0_#111827]">
            ...
            <div className="p-5">
              ...
              <h3 className="mt-2 text-xl font-black uppercase text-gray-900">{[item.title, item.name].filter(Boolean).join(' ')}</h3>
              {item.bio && <div className="mt-3 whitespace-pre-line text-sm leading-6 text-gray-600 rt-content" dangerouslySetInnerHTML={{ __html: item.bio }} />}
              {item.languages && item.languages.length > 0 && <p className="mt-2 text-xs text-gray-600">{item.languages.join(' / ')}</p>}
