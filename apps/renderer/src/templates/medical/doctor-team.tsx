        ...
        {doctors.map((item, index) => (
          <article key={`${item.name}-${index}`} className="group overflow-hidden rounded-xl bg-[var(--token-card-bg)] shadow-lg" data-edit-collection="doctors" data-edit-index={index}>
            ...
            <div className="p-5">
              ...
              {item.bio && <div className="mt-3 whitespace-pre-line text-sm leading-6 text-[var(--token-body)] rt-content" dangerouslySetInnerHTML={{ __html: item.bio }} />}
        ...
        {doctors.map((item, index) => (
          <article key={`${item.name}-${index}`} className="group overflow-hidden border border-[var(--token-card-border)] bg-[var(--token-card-bg)]" data-edit-collection="doctors" data-edit-index={index}>
            ...
            <div className="p-5">
              ...
              {item.bio && <div className="mt-3 whitespace-pre-line text-sm font-light leading-6 text-[var(--token-body)] rt-content" dangerouslySetInnerHTML={{ __html: item.bio }} />}
...
function Bold({ header, doctors }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {doctors.map((item, index) => (
          <article key={`${item.name}-${index}`} className="group overflow-hidden border-2 border-[var(--token-card-border)] bg-[var(--token-card-bg)] shadow-[4px_4px_0_var(--token-card-border)]" data-edit-collection="doctors" data-edit-index={index}>
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.name || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-5">
