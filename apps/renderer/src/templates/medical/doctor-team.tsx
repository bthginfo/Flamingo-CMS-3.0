      ...
      <div className="grid gap-6 md:grid-cols-3">
        {doctors.map((item, index) => (
          <article key={`${item.name}-${index}`} className="group overflow-hidden border border-[var(--token-card-border)] bg-[var(--token-card-bg)]" data-edit-collection="doctors" data-edit-index={index}>
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image data-edit-image="image" src={item.image} alt={item.name || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-5">
              {item.specialty && <p className="text-xs font-light uppercase tracking-widest text-[color:var(--token-badge-text)]">{item.specialty}</p>}
              <h3 className="mt-2 text-xl font-light text-[color:var(--token-heading)]">{[item.title, item.name].filter(Boolean).join(' ')}</h3>
              {item.bio && <div className="mt-3 whitespace-pre-line text-sm font-light leading-6 text-[color:var(--token-body)] rt-content" data-edit-rich="bio" dangerouslySetInnerHTML={{ __html: item.bio }} />}
              {item.languages && item.languages.length > 0 && <p className="mt-2 text-xs font-light text-[color:var(--token-muted)]">{item.languages.join(' / ')}</p>}
              {item.appointmentCta?.label && <a href={item.appointmentCta.href || '#'} className="mt-4 inline-flex items-center gap-2 rounded-lg border border-[var(--token-btn-bg)] bg-[var(--token-btn-bg)] px-4 py-2 text-sm font-semibold text-[color:var(--token-btn-text)]"><span data-edit-path="label">{item.appointmentCta.label}</span><ArrowRight size={14} /></a>}
            </div>
          </article>
        ))}
      </div>
...
function Bold({ header, doctors }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {doctors.map((item, index) => (
          <article key={`${item.name || 'item'}-${index}`} className="group overflow-hidden border-2 border-[var(--token-card-border)] bg-[var(--token-card-bg)] shadow-[4px_4px_0_var(--token-card-border)]" data-edit-collection="doctors" data-edit-index={index}>
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image data-edit-image="image" src={item.image} alt={item.name || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-5">
              {item.specialty && <p className="text-xs font-black uppercase tracking-widest text-[color:var(--token-badge-text)]">{item.specialty}</p>}
              <h3 className="mt-2 text-xl font-black uppercase text-[color:var(--token-heading)]">{[item.title, item.name].filter(Boolean).join(' ')}</h3>
              {item.bio && <div className="mt-3 whitespace-pre-line text-sm leading-6 text-[color:var(--token-body)] rt-content" data-edit-rich="bio" dangerouslySetInnerHTML={{ __html: item.bio }} />}
              {item.languages && item.languages.length > 0 && <p className="mt-2 text-xs text-[color:var(--token-muted)]">{item.languages.join(' / ')}</p>}
              {item.appointmentCta?.label && <a href={item.appointmentCta.href || '#'} className="mt-4 inline-flex items-center gap-2 border-2 border-[var(--token-btn-bg)] bg-[var(--token-btn-bg)] px-4 py-2 text-sm font-black uppercase text-[color:var(--token-btn-text)]"><span data-edit-path="label">{item.appointmentCta.label}</span><ArrowRight size={14} /></a>}
            </div>
          </article>
        ))}
      </div>
