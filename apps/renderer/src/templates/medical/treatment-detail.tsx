      ...
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="grid gap-6 md:grid-cols-2">
        {treatments.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden rounded-xl bg-[var(--style-card-bg,#fff)] shadow-lg">
            ...
            <div className="p-5">
              {(item.durationLabel || item.requirementLabel) && <p className="text-xs font-bold uppercase tracking-widest text-[var(--style-badge-text,var(--style-accent-color,var(--brand-primary)))]">{[item.durationLabel, item.requirementLabel].filter(Boolean).join(' / ')}</p>}
              ...
              {item.steps && item.steps.length > 0 && <p className="mt-2 text-xs text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{item.steps.join(' / ')}</p>}
              {item.noticeText && <p className="mt-2 text-xs italic text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{item.noticeText}</p>}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {treatments.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden border border-[var(--style-border-color,rgba(0,0,0,.1))] bg-[var(--style-card-bg,#fff)]">
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.title || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="50vw" /></div>}
            <div className="p-5">
              {(item.durationLabel || item.requirementLabel) && <p className="text-xs font-light uppercase tracking-widest text-[var(--style-badge-text,var(--style-accent-color,var(--brand-primary)))]">{[item.durationLabel, item.requirementLabel].filter(Boolean).join(' / ')}</p>}
              ...
              {item.steps && item.steps.length > 0 && <p className="mt-2 text-xs font-light text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{item.steps.join(' / ')}</p>}
              {item.noticeText && <p className="mt-2 text-xs font-light italic text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{item.noticeText}</p>}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {treatments.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden border-2 border-[var(--style-border-color,var(--style-text-primary,#111827))] bg-[var(--style-card-bg,#fff)] shadow-[4px_4px_0_var(--style-border-color,var(--style-text-primary,#111827))]">
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.title || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="50vw" /></div>}
            <div className="p-5">
              {(item.durationLabel || item.requirementLabel) && <p className="text-xs font-black uppercase tracking-widest text-[var(--style-badge-text,var(--style-accent-color,var(--brand-primary)))]">{[item.durationLabel, item.requirementLabel].filter(Boolean).join(' / ')}</p>}
              <h3 className="mt-2 text-xl font-black uppercase text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{item.title || ''}</h3>
              {item.text && <div className="mt-3 whitespace-pre-line text-sm leading-6 text-[var(--style-body-color,var(--style-text-secondary,#4b5563))] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
              {item.steps && item.steps.length > 0 && <p className="mt-2 text-xs text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{item.steps.join(' / ')}</p>}
              {item.noticeText && <p className="mt-2 text-xs text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{item.noticeText}</p>}
