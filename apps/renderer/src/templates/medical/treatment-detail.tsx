        ...
        {treatments.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden rounded-2xl bg-[var(--style-card-bg)] shadow-lg">
            ...
            <div className="p-5">
              ...
              {item.steps && item.steps.length > 0 && <p className="mt-2 text-xs text-[var(--style-text-secondary)]">{item.steps.join(' / ')}</p>}
              {item.noticeText && <p className="mt-2 text-xs italic text-[var(--style-text-secondary)]">{item.noticeText}</p>}
        ...
        {treatments.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden border border-black/10 bg-[var(--style-card-bg)]">
            ...
            <div className="p-5">
              ...
              {item.steps && item.steps.length > 0 && <p className="mt-2 text-xs font-light text-[var(--style-text-secondary)]">{item.steps.join(' / ')}</p>}
              {item.noticeText && <p className="mt-2 text-xs font-light italic text-[var(--style-text-secondary)]">{item.noticeText}</p>}
        ...
        {treatments.map((item, index) => (
          <article key={`${item.title}-${index}`} className="group overflow-hidden border-2 border-[var(--style-text-primary)] bg-[var(--style-card-bg)] shadow-[4px_4px_0_var(--style-text-primary)]">
            ...
            <div className="p-5">
              ...
              {item.text && <p className="mt-3 whitespace-pre-line text-sm leading-6 text-[var(--style-text-secondary)]">{item.text}</p>}
              {item.steps && item.steps.length > 0 && <p className="mt-2 text-xs text-[var(--style-text-secondary)]">{item.steps.join(' / ')}</p>}
              {item.noticeText && <p className="mt-2 text-xs text-[var(--style-text-secondary)]">{item.noticeText}</p>}
