    ...
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((img, index) => (
          <article key={`${img.src}-${index}`} className="group overflow-hidden rounded-2xl bg-[var(--style-card-bg)] shadow-lg">
            ...
            <div className="p-4">
              ...
              {(img.caption || img.alt) && <h3 className="mt-1 font-semibold text-[var(--style-text-primary)]">{img.caption || img.alt}</h3>}
            </div>
          </article>
        ))}
      </motion.div>
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((img, index) => (
          <article key={`${img.src}-${index}`} className="group overflow-hidden border border-black/10 bg-[var(--style-card-bg)]">
            {img.src && <div className="relative aspect-[4/3] overflow-hidden"><Image src={img.src} alt={img.alt || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-4">
              {img.category && <p className="text-xs font-light uppercase tracking-widest text-blue-500">{img.category}</p>}
              {(img.caption || img.alt) && <h3 className="mt-1 font-light text-[var(--style-text-primary)]">{img.caption || img.alt}</h3>}
            </div>
          </article>
        ))}
      </div>
...
function Bold({ header, images }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((img, index) => (
          <article key={`${img.src}-${index}`} className="group overflow-hidden border-2 border-[var(--style-text-primary)] bg-[var(--style-card-bg)] shadow-[4px_4px_0_var(--style-text-primary)]">
            {img.src && <div className="relative aspect-[4/3] overflow-hidden"><Image src={img.src} alt={img.alt || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-4">
              {img.category && <p className="text-xs font-black uppercase tracking-widest text-teal-500">{img.category}</p>}
              {(img.caption || img.alt) && <h3 className="mt-1 font-black uppercase text-[var(--style-text-primary)]">{img.caption || img.alt}</h3>}
            </div>
          </article>
        ))}
      </div>
