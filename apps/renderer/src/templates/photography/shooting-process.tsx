              ...
              return (
                <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="flex gap-8 border-t border-[color:var(--token-card-border,#e4e4e7)] py-8">
                  <div className="flex flex-col items-center gap-2 shrink-0">
                    <span className="text-[10px] uppercase tracking-[0.2em] text-[color:var(--token-body,#a1a1aa)]">{String(i + 1).padStart(2, '0')}</span>
                    <Icon className="w-4 h-4 text-[color:var(--token-body,#a1a1aa)]" />
                  </div>
                  <div>
                    <h3 className="font-light text-[color:var(--token-heading,#18181b)] text-lg" data-edit-path="title">{step.title}</h3>
                    <div className="text-[color:var(--token-muted,#71717a)] text-sm mt-2 rt-content" dangerouslySetInnerHTML={{ __html: step.text }} />
                  </div>
                </motion.div>
              );
            })}
            ...
            {steps.map((step, i) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="border border-[color:var(--token-card-border,#ffffff)/10] p-6">
                  ...
                  <h3 className="font-bold text-[color:var(--token-on-dark-heading,#ffffff)] mb-2" data-edit-path="title">{step.title}</h3>
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="relative md:grid md:grid-cols-2 md:gap-12 items-center pl-16 md:pl-0">
                  ...
                  <div className={`${isLeft ? 'md:text-right md:pr-12' : 'md:col-start-2 md:pl-12'}`}>
                    ...
                    <h3 className="text-lg md:text-xl font-semibold text-[color:var(--token-heading,#18181b)]" data-edit-path="title">{step.title}</h3>
                    <div className="text-[color:var(--token-muted,#52525b)] mt-1 md:mt-2 text-sm md:text-base rt-content" dangerouslySetInnerHTML={{ __html: step.text }} />
                  </div>
