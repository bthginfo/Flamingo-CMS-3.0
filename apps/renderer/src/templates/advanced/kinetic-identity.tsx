...
function StatementLine({ item, index, compact = false }: { item: Statement; index: number; compact?: boolean }) {
  return <h3 className={`hyphens-auto [overflow-wrap:anywhere] font-black text-[color:var(--token-heading)] ${compact ? 'text-[clamp(2rem,4vw,4.6rem)] leading-[.92] tracking-[-.055em]' : 'text-[clamp(2.25rem,6vw,6.8rem)] leading-[.88] tracking-[-.065em]'}`} data-edit-collection="statements" data-edit-index={index}><span data-edit-path="prefix">{item.prefix}</span>{' '}<span className="text-[color:var(--token-accent)]" data-edit-path="highlight">{item.highlight}</span>{' '}<span data-edit-path="suffix">{item.suffix}</span></h3>;
}
...
export function KineticIdentitySection({ data }: Props) {
  ...
  return <>
    <section className="advanced-static-fallback bg-[var(--token-section-bg)] px-5 py-16 lg:hidden">
      <div className="mx-auto max-w-3xl">{intro}<div className="mt-10 space-y-12">{items.map((item, index) => <article key={index} className="border-t border-[var(--token-divider)] pt-5"><StatementLine item={item} index={index} />{item.text && <p className="mt-4 max-w-xl text-base leading-7 text-[color:var(--token-body)]" data-edit-path="text">{plain(item.text)}</p>}{item.image && <img src={item.image} alt="" loading="lazy" className="mt-5 aspect-[4/3] w-full rounded-[var(--token-card-radius)] object-cover" data-edit-image="image" />}</article>)}</div><AdvancedLink cta={data.cta as AdvancedCta} className="mt-10" /></div>
    </section>
