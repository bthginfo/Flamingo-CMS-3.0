    ...
    <section className="py-12 px-6" style={{ backgroundColor: bgColor }}>
      <div className="max-w-4xl mx-auto text-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
          {date && <p className="text-[color:var(--token-on-dark-heading,#ffffff)/70] text-sm font-mono uppercase tracking-widest mb-2" data-edit-path="date">{date}</p>}
          ...
          <a href={ctaHref} className="inline-flex items-center justify-center mt-6 px-8 py-3 bg-[var(--token-section-bg-alt,#000000)] text-[color:var(--token-on-dark-heading,#ffffff)] font-bold uppercase tracking-wider text-sm hover:bg-[var(--token-section-bg-alt,#000000)/80] transition-colors" data-edit-path="ctaLabel">
            {ctaLabel}
          </a>
        </motion.div>
      </div>
