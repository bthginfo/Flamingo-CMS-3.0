        ...
        <div className="space-y-6">
          {steps.map((step, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="shrink-0 w-10 h-10 rounded-full bg-[var(--token-card-bg)/10] border border-[color:var(--token-card-border)/20] flex items-center justify-center text-[color:var(--token-on-dark-heading)] font-bold text-sm">
                {i + 1}
              </div>
              <div>
                <h3 className="text-[color:var(--token-on-dark-heading)] font-semibold" data-edit-path="title">{step.title}</h3>
                <p className="text-[color:var(--token-on-dark-heading)/50] text-sm mt-1" data-edit-path="description">{plain(step.description)}</p>
              </div>
            </motion.div>
          ))}
        </div>
