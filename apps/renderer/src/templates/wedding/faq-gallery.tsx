...
function FaqItem({ question, answer, index, variant }: { question: string; answer: string; index: number; variant: string }) {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="border-t border-[color:var(--token-card-border,#e4e4e7)]">
        <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between py-5 text-left">
          <span className="font-light text-[color:var(--token-heading,#18181b)]">{question}</span>
          <ChevronDown className={`w-4 h-4 text-[color:var(--token-on-dark-body,#a1a1aa)] transition-transform ${open ? 'rotate-180' : ''}`} />
        </button>
        {open && <div className="pb-5 text-[color:var(--token-on-dark-muted,#71717a)] text-sm leading-relaxed">{plain(answer)}</div>}
      </motion.div>
    );
  }
  ...
  if (variant === 'bold') {
    return (
      <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="border-2 border-gray-900">
        ...
        {open && <div className="px-5 pb-5 text-[color:var(--token-on-dark-muted,#71717a)] text-sm leading-relaxed">{plain(answer)}</div>}
      </motion.div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="bg-[var(--token-card-bg,#ffffff)] rounded-xl border border-[color:var(--token-card-border,#f4f4f5)] overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between p-5 text-left">
        <span className="font-medium text-[color:var(--token-heading,#18181b)]">{question}</span>
        <ChevronDown className={`w-5 h-5 text-[color:var(--token-on-dark-body,#a1a1aa)] transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && <div className="px-5 pb-5 text-[color:var(--token-on-dark-muted,#52525b)] text-sm leading-relaxed">{plain(answer)}</div>}
    </motion.div>
  );
}
