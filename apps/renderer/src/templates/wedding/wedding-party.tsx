        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {members.map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center" data-edit-collection="members" data-edit-index={i}>
              ...
              <h3 className="text-lg font-semibold text-[color:var(--token-heading,#18181b)]" data-edit-path="name">{m.name}</h3>
              <p className="text-[color:var(--token-icon,var(--brand-primary,#1a5276))] text-sm font-medium mt-1" data-edit-path="role">{m.role}</p>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          {members.map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex gap-6 items-start border-t border-[color:var(--token-card-border,#e4e4e7)] pt-6" data-edit-collection="members" data-edit-index={i}>
              ...
              <div>
                <h3 className="text-base font-medium text-[color:var(--token-heading,#18181b)]" data-edit-path="name">{m.name}</h3>
                <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--token-body,#a1a1aa)] mt-1" data-edit-path="role">{m.role}</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {members.map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="border-2 border-[color:var(--token-card-border,#18181b)] p-6 text-center hover:border-[var(--token-card-border,var(--brand-accent,#f39c12))/50] transition-colors" data-edit-collection="members" data-edit-index={i}>
              ...
              <h3 className="text-lg font-bold text-[color:var(--token-heading,#18181b)]" data-edit-path="name">{m.name}</h3>
              <p className="text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))] text-xs font-bold uppercase tracking-widest mt-1" data-edit-path="role">{m.role}</p>
              {m.text && <div className="text-[color:var(--token-muted,#71717a)] text-sm mt-3 rt-content" dangerouslySetInnerHTML={{ __html: m.text }} />}
            </motion.div>
          ))}
        </div>
