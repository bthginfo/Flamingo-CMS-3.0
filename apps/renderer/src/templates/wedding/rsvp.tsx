    ...
    return (
      <section className="py-16 md:py-24 px-4 md:px-6">
        <div className="max-w-lg mx-auto text-center">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className={`w-16 h-16 mx-auto mb-6 rounded-full flex items-center justify-center ${isBold ? 'bg-[color-mix(in_srgb,var(--token-badge-bg)_20%,transparent)]' : 'bg-[var(--token-success-bg,theme(colors.green.100))]'}`}>
            <Check className={`w-8 h-8 ${isBold ? 'text-[color:var(--token-eyebrow)]' : 'text-[var(--token-success,theme(colors.green.600))]'}`} />
          </motion.div>
      ...
      <section className="py-24 md:py-36 px-4 md:px-6">
        <div className="max-w-md mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {status === 'error' && <p className="text-sm text-[var(--token-danger,theme(colors.red.500))] flex items-center gap-1"><AlertCircle size={14} />{errorMsg}</p>}
            <div>
