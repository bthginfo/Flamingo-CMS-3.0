  ...
  return (
    <div>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {items.map((item, i) => (
          <article key={`${item.title || 'item'}-${i}`} className="border-t border-black/10 pt-6" data-edit-collection="items" data-edit-index={i}>
...
function ExpertiseBold({ headline, subline, badgeText, items }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {items.map((item, i) => (
          <article key={`${item.title || 'item'}-${i}`} className="border-2 border-[var(--token-card-border)] bg-[#111] p-6 shadow-[4px_4px_0_var(--token-eyebrow)]" data-edit-collection="items" data-edit-index={i}>
