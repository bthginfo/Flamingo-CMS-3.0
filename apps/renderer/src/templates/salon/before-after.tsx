        ...
        {items.map((item, i) => (
          <motion.article key={`$<span data-edit-path="title">{item.title}</span>-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="overflow-hidden rounded-xl border border-[var(--token-icon)]/20 bg-[var(--token-card-bg)] shadow-md" data-edit-collection="items" data-edit-index={i}>
            ...
            <div className="p-5">
              ...
              {item.text && <div className="mt-3 text-sm text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
              {item.caption && <p className="mt-2 text-xs text-[color:var(--token-muted)]" data-edit-path="caption">{item.caption}</p>}
              {item.cta?.label && <a href={item.cta.href || '#'} className="mt-5 inline-flex rounded-full bg-[#111827] px-5 py-2 text-sm font-semibold text-[color:var(--token-on-dark-heading)]" data-edit-path="label">{item.cta.label}</a>}
            </div>
          </motion.article>
        ))}
      </div>
        ...
        {items.map((item, i) => (
          <article key={`$<span data-edit-path="title">{item.title}</span>-${i}`} className="border-b border-black/10 pb-8" data-edit-collection="items" data-edit-index={i}>
            ...
            <div className="mt-5">
              ...
              {item.text && <div className="mt-3 text-sm font-light text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
        ...
        {items.map((item, i) => (
          <article key={`$<span data-edit-path="title">{item.title}</span>-${i}`} className="overflow-hidden border-2 border-[#111827] bg-[#111] shadow-[4px_4px_0_var(--token-eyebrow)]" data-edit-collection="items" data-edit-index={i}>
            ...
            <div className="p-5">
              ...
              {item.text && <div className="mt-3 text-sm text-[color:var(--token-on-dark-heading)/70] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
