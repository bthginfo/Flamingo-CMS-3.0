  ...
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {items.map((item, i) => (
          <motion.article key={`${item.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md">
            ...
            <div className="p-5">
              ...
              {item.text && <div className="mt-3 text-sm text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
              {item.caption && <p className="mt-2 text-xs text-[color:var(--token-on-dark-muted,#52525b)]">{item.caption}</p>}
              {item.cta?.label && <a href={item.cta.href || '#'} className="mt-5 inline-flex rounded-full bg-[#111827] px-5 py-2 text-sm font-semibold text-[color:var(--token-on-dark-heading,#ffffff)]">{item.cta.label}</a>}
            </div>
          </motion.article>
        ))}
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-2">
        {items.map((item, i) => (
          <article key={`${item.title}-${i}`} className="border-b border-black/10 pb-8">
            ...
            <div className="mt-5">
              {item.category && <p className="text-xs font-light uppercase tracking-[0.3em] text-[color:var(--token-on-dark-muted,#52525b)]">{item.category}</p>}
              <h3 className="mt-2 text-xl font-light text-[color:var(--token-heading,#18181b)]">{item.title || ''}</h3>
              {item.text && <div className="mt-3 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
...
function BeforeAfterBold({ headline, subline, badgeText, items }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {items.map((item, i) => (
          <article key={`${item.title}-${i}`} className="overflow-hidden border-2 border-[#111827] bg-[#111] shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]">
            <div className="grid grid-cols-2">
              {item.beforeImage && <div className="relative aspect-square"><Image src={item.beforeImage} alt={item.title || ''} fill className="object-cover" sizes="25vw" /></div>}
              {item.afterImage && <div className="relative aspect-square"><Image src={item.afterImage} alt={item.title || ''} fill className="object-cover" sizes="25vw" /></div>}
            </div>
