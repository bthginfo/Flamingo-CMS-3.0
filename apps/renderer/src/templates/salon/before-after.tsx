  ...
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {items.map((item, i) => (
          <motion.article key={`${item.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-white shadow-md">
            ...
            <div className="p-5">
              ...
              {item.text && <div className="mt-3 text-sm text-gray-600 rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
              {item.caption && <p className="mt-2 text-xs text-gray-600">{item.caption}</p>}
              {item.cta?.label && <a href={item.cta.href || '#'} className="mt-5 inline-flex rounded-full bg-[#111827] px-5 py-2 text-sm font-semibold text-white">{item.cta.label}</a>}
            </div>
          </motion.article>
        ))}
      </div>
        ...
        {items.map((item, i) => (
          <article key={`${item.title}-${i}`} className="border-b border-black/10 pb-8">
            ...
            <div className="mt-5">
              ...
              {item.text && <div className="mt-3 text-sm font-light text-gray-600 rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {items.map((item, i) => (
          <article key={`${item.title}-${i}`} className="overflow-hidden border-2 border-[#111827] bg-[#111] shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]">
            ...
            <div className="p-5">
              {item.category && <span className="inline-block bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-3 py-1 text-xs font-black uppercase text-white">{item.category}</span>}
              <h3 className="mt-2 text-xl font-black uppercase text-white">{item.title || ''}</h3>
              {item.text && <div className="mt-3 text-sm text-white/70 rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
