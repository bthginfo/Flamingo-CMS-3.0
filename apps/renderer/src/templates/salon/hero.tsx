    ...
    <section className="relative min-h-screen flex items-center overflow-hidden -mt-[112px] pt-[112px]">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-12 md:py-20 text-center">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }} className="mt-8 flex flex-wrap justify-center gap-3">
          {primaryCta.label && <a data-edit-link="primaryCta" href={primaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full border border-[color:var(--token-btn-bg)] bg-[var(--token-btn-bg)] px-7 py-3 font-semibold text-[color:var(--token-btn-text)] shadow-lg transition-[filter,transform] hover:brightness-105 motion-safe:hover:-translate-y-0.5"><span data-edit-path="label">{primaryCta.label}</span>{primaryCta.icon && <DynamicIcon editPath="primaryCta.icon" name={primaryCta.icon} size={17} />}</a>}
          {secondaryCta.label && <a data-edit-link="secondaryCta" href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full border border-[color:var(--token-btn-secondary-border)] bg-[var(--token-btn-secondary-bg)] px-7 py-3 font-semibold text-[color:var(--token-btn-secondary-text)] backdrop-blur-sm transition-[filter,transform] hover:brightness-105 motion-safe:hover:-translate-y-0.5" data-edit-path="label">{secondaryCta.label}</a>}
        </motion.div>
