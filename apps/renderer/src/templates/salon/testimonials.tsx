...
function TestimonialsClassic({ headline, subline, badgeText, ratingValue, ratingCount, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-3 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">{ratingValue && <span>{ratingValue}</span>}{ratingCount && <span>{ratingCount}</span>}</div>
...
function TestimonialsModern({ headline, subline, badgeText, ratingValue, ratingCount, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-3 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">{ratingValue && <span>{ratingValue}</span>}{ratingCount && <span>{ratingCount}</span>}</div>
...
function TestimonialsBold({ headline, subline, badgeText, ratingValue, ratingCount, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-3 text-sm font-bold uppercase text-[color:var(--token-on-dark-muted,#52525b)]">{ratingValue && <span>{ratingValue}</span>}{ratingCount && <span>{ratingCount}</span>}</div>
      <div className="grid gap-5 md:grid-cols-3">
        {items.map((item, i) => (
