  ...
  return (
    <div>
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {items.map((item, i) => (
          <motion.article key={`${item.name}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-white p-5 shadow-md">
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {items.map((item, i) => (
          <article key={`${item.name}-${i}`} className="border-t border-black/10 pt-6">
            ...
            {item.quote && <div className="mt-4 text-sm font-light leading-6 text-gray-900 rt-content" dangerouslySetInnerHTML={{ __html: item.quote }} />}
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {items.map((item, i) => (
          <article key={`${item.name}-${i}`} className="border-2 border-[#111827] bg-[#111] p-5 shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]">
            ...
            {item.quote && <div className="mt-4 text-sm leading-6 text-white rt-content" dangerouslySetInnerHTML={{ __html: item.quote }} />}
