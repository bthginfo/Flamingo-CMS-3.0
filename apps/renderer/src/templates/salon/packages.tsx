      ...
      <div className="grid gap-6 md:grid-cols-3">
        {packages.map((item, i) => (
          <motion.article key={`${item.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group overflow-hidden rounded-2xl border border-[var(--style-badge-bg)]/20 bg-[var(--style-card-bg)] shadow-md">
            ...
            <div className="p-5">
              {(item.priceLabel || item.validUntilLabel) && <p className="text-xs font-bold uppercase tracking-widest text-[var(--style-text-secondary)]">{[item.priceLabel, item.validUntilLabel].filter(Boolean).join(' / ')}</p>}
              ...
              {asList<string>(item.includes).length > 0 && <p className="mt-2 text-sm text-[var(--style-text-secondary)]">{asList<string>(item.includes).join(' / ')}</p>}
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {packages.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group">
            ...
            <div className="mt-4">
              {(item.priceLabel || item.validUntilLabel) && <p className="text-xs font-light uppercase tracking-[0.3em] text-[var(--style-text-secondary)]">{[item.priceLabel, item.validUntilLabel].filter(Boolean).join(' / ')}</p>}
              ...
              {asList<string>(item.includes).length > 0 && <p className="mt-2 text-sm font-light text-[var(--style-text-secondary)]">{asList<string>(item.includes).join(' / ')}</p>}
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {packages.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group overflow-hidden border-2 border-[var(--style-text-primary)] bg-[#111] shadow-[4px_4px_0_var(--style-accent)]">
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.title || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-5">
              {(item.priceLabel || item.validUntilLabel) && <span className="inline-block bg-[var(--style-accent)] px-3 py-1 text-xs font-black uppercase text-white">{[item.priceLabel, item.validUntilLabel].filter(Boolean).join(' / ')}</span>}
