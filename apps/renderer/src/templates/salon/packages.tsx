      ...
      <div className="grid gap-6 md:grid-cols-3">
        {packages.map((item, i) => (
          <motion.article key={`${item.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md">
            ...
            <div className="p-5">
              {(item.priceLabel || item.validUntilLabel) && <p className="text-xs font-bold uppercase tracking-widest text-[color:var(--token-on-dark-muted,#52525b)]">{[item.priceLabel, item.validUntilLabel].filter(Boolean).join(' / ')}</p>}
              <h3 className="mt-2 text-xl font-bold text-[color:var(--token-heading,#18181b)]">{item.title || ''}</h3>
              {item.text && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
              {asList<string>(item.includes).length > 0 && <p className="mt-2 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.includes).join(' / ')}</p>}
              {item.cta?.label && <a href={item.cta.href || '#'} className="mt-5 inline-flex rounded-full bg-[#111827] px-5 py-2 text-sm font-semibold text-[color:var(--token-on-dark-heading,#ffffff)] shadow-md">{item.cta.label}</a>}
            </div>
          </motion.article>
        ))}
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {packages.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group">
            ...
            <div className="mt-4">
              {(item.priceLabel || item.validUntilLabel) && <p className="text-xs font-light uppercase tracking-[0.3em] text-[color:var(--token-on-dark-muted,#52525b)]">{[item.priceLabel, item.validUntilLabel].filter(Boolean).join(' / ')}</p>}
              <h3 className="mt-2 text-xl font-light text-[color:var(--token-heading,#18181b)]">{item.title || ''}</h3>
              {item.text && <div className="mt-3 text-sm font-light leading-6 text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
              {asList<string>(item.includes).length > 0 && <p className="mt-2 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.includes).join(' / ')}</p>}
              {item.cta?.label && <a href={item.cta.href || '#'} className="mt-4 inline-flex border-b border-[var(--token-card-border,var(--brand-accent,#f39c12))] pb-1 text-sm font-light text-[color:var(--token-heading,#18181b)]">{item.cta.label}</a>}
            </div>
          </article>
        ))}
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {packages.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group overflow-hidden border-2 border-[#111827] bg-[#111] shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]">
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.title || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-5">
              {(item.priceLabel || item.validUntilLabel) && <span className="inline-block bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-3 py-1 text-xs font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)]">{[item.priceLabel, item.validUntilLabel].filter(Boolean).join(' / ')}</span>}
              ...
              {asList<string>(item.includes).length > 0 && <p className="mt-2 text-sm text-[color:var(--token-on-dark-heading,#ffffff)/60]">{asList<string>(item.includes).join(' / ')}</p>}
              {item.cta?.label && <a href={item.cta.href || '#'} className="mt-5 inline-flex bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-5 py-2 text-sm font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)] shadow-[4px_4px_0_rgba(0,0,0,0.8)]">{item.cta.label}</a>}
            </div>
          </article>
        ))}
      </div>
