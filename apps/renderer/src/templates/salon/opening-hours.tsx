  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        ...
        {bookingNote && <p className="text-sm leading-6 text-[color:var(--token-on-dark-muted,#52525b)]">{bookingNote}</p>}
        {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-6 inline-flex rounded-full bg-[#111827] px-5 py-3 font-semibold text-[color:var(--token-on-dark-heading,#ffffff)] shadow-md">{ctaPrimary.label}</a>}
      </div>
      <div className="rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md">
        {days.map((day, i) => (
          <motion.div key={`${day.label}-${i}`} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="flex items-center justify-between gap-4 border-b border-[var(--token-icon, var(--brand-primary))]/20 px-5 py-4 last:border-b-0">
            <div className="flex items-center gap-3"><Clock size={17} className="text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]" /><div><p className="font-semibold text-[color:var(--token-heading,#18181b)]">{day.label || ''}</p>{day.note && <p className="text-xs text-[color:var(--token-on-dark-muted,#52525b)]">{day.note}</p>}</div></div>
  ...
  return (
    <div className="grid gap-16 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        ...
        {bookingNote && <p className="text-sm font-light leading-6 text-[color:var(--token-on-dark-muted,#52525b)]">{bookingNote}</p>}
        {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-6 inline-flex border border-[#111827] px-6 py-3 font-light text-[color:var(--token-heading,#18181b)]">{ctaPrimary.label}</a>}
      </div>
      <div className="border-y border-black/10">
        {days.map((day, i) => (
          <div key={`${day.label}-${i}`} className="flex items-center justify-between gap-4 border-b border-black/10 px-2 py-5 last:border-b-0">
            <div className="flex items-center gap-3"><Clock size={16} className="text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]" /><div><p className="font-light text-[color:var(--token-heading,#18181b)]">{day.label || ''}</p>{day.note && <p className="text-xs font-light text-[color:var(--token-on-dark-muted,#52525b)]">{day.note}</p>}</div></div>
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        ...
        {bookingNote && <p className="text-sm font-bold leading-6 text-[color:var(--token-on-dark-muted,#52525b)]">{bookingNote}</p>}
        {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-6 inline-flex bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-6 py-3 font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)] shadow-[4px_4px_0_rgba(0,0,0,0.8)]">{ctaPrimary.label}</a>}
      </div>
