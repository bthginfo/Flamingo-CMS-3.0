...
function HoursClassic({ headline, subline, badgeText, days, bookingNote, ctaPrimary }: Props) {
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="rounded-xl border border-[var(--brand-primary)]/20 bg-white shadow-md">
        {days.map((day, i) => (
          <motion.div key={`${day.label}-${i}`} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="flex items-center justify-between gap-4 border-b border-[var(--brand-primary)]/20 px-5 py-4 last:border-b-0">
            <div className="flex items-center gap-3"><Clock size={17} className="text-brand-accent" /><div><p className="font-semibold text-gray-900">{day.label || ''}</p>{day.note && <p className="text-xs text-gray-600">{day.note}</p>}</div></div>
      ...
      <div className="border-y border-black/10">
        {days.map((day, i) => (
          <div key={`${day.label}-${i}`} className="flex items-center justify-between gap-4 border-b border-black/10 px-2 py-5 last:border-b-0">
            <div className="flex items-center gap-3"><Clock size={16} className="text-brand-accent" /><div><p className="font-light text-gray-900">{day.label || ''}</p>{day.note && <p className="text-xs font-light text-gray-600">{day.note}</p>}</div></div>
...
function HoursBold({ headline, subline, badgeText, days, bookingNote, ctaPrimary }: Props) {
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="border-2 border-[#111827] bg-[#111] shadow-[4px_4px_0_var(--brand-accent)]">
        {days.map((day, i) => (
          <div key={`${day.label}-${i}`} className="flex items-center justify-between gap-4 border-b-2 border-[#111827] px-5 py-4 last:border-b-0">
            <div className="flex items-center gap-3"><Clock size={17} className="text-brand-accent" /><div><p className="font-black uppercase text-white">{day.label || ''}</p>{day.note && <p className="text-xs text-white/60">{day.note}</p>}</div></div>
