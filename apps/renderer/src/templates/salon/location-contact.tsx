    ...
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex rounded-full border border-[var(--token-icon, var(--brand-primary))]/30 px-5 py-3 font-semibold text-gray-900">{secondaryCta.label}</a>}
        </div>
      </div>
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-6 grid gap-3">{infoCards.map((card, i) => <div key={`${card.label}-${i}`} className="flex gap-4 border-t-2 border-[#111827] pt-4"><DynamicIcon name={card.icon || 'mail'} size={20} className="text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]" /><div><p className="text-xs font-bold uppercase text-gray-600">{card.label || ''}</p><p className="font-black text-gray-900">{card.value || ''}</p></div></div>)}</div>
        <div className="mt-8 flex flex-wrap gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-6 py-3 font-black uppercase text-white shadow-[4px_4px_0_rgba(0,0,0,0.8)]">{primaryCta.label}</a>}
          {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex border-2 border-[#111827] px-6 py-3 font-black uppercase text-gray-900 shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]">{secondaryCta.label}</a>}
        </div>
      </div>
