...
export function LocationContactSection({ data }: SectionProps) {
  const header = baseHeader(data, 'Kontakt', 'Salon');
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div><SectionHeader {...header} />{introText && <p className="text-[var(--style-text-secondary)]">{introText}</p>}<div className="mt-6 grid gap-3">{infoCards.map((card, index) => <div key={`${card.label}-${index}`} className="flex gap-4 border-t border-black/10 pt-4"><DynamicIcon name={card.icon || 'mail'} size={20} /><div><p className="text-xs text-[var(--style-text-secondary)]">{card.label || ''}</p><p className="font-semibold text-[var(--style-text-primary)]">{card.value || ''}</p></div></div>)}</div><div className="mt-8 flex flex-wrap gap-3"><CtaButton cta={primaryCta} /><CtaButton cta={secondaryCta} /></div></div>
      <div className="rounded-[var(--style-card-radius)] border border-black/10 bg-[var(--style-card-bg)] p-5 shadow-[var(--style-card-shadow)]">
        ...
        {mapEmbedUrl && <iframe src={mapEmbedUrl} className="mb-5 h-56 w-full rounded-[var(--style-radius-md)]" loading="lazy" />}
