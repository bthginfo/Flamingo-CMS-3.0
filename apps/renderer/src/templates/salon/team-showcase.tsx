  ...
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {members.map((m, i) => (
          <motion.article key={`${m.name}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md" data-edit-collection="members" data-edit-index={i}>
...
function TeamBold({ headline, subline, badgeText, members }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {members.map((m, i) => (
          <article key={`${m.name}-${i}`} className="group overflow-hidden border-2 border-[#111827] bg-[#111] shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]" data-edit-collection="members" data-edit-index={i}>
            {m.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={m.image} alt={m.name || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-5">
