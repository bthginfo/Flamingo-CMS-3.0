      ...
      <div className="grid gap-6 md:grid-cols-2">
        {treatments.map((item, i) => (
          <motion.article key={`${item.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md">
            ...
            <div className="p-5">
              {(item.resultLabel || item.durationLabel || item.priceLabel) && <p className="text-xs font-bold uppercase tracking-widest text-[color:var(--token-on-dark-muted,#52525b)]">{[item.resultLabel, item.durationLabel, item.priceLabel].filter(Boolean).join(' / ')}</p>}
              ...
              {asList<string>(item.steps).length > 0 && <p className="mt-2 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.steps).join(' / ')}</p>}
              {asList<string>(item.careTips).length > 0 && <p className="mt-2 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.careTips).join(' / ')}</p>}
      ...
      <div className="grid gap-8 md:grid-cols-2">
        {treatments.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group">
            ...
            <div className="mt-4">
              {(item.resultLabel || item.durationLabel || item.priceLabel) && <p className="text-xs font-light uppercase tracking-[0.3em] text-[color:var(--token-on-dark-muted,#52525b)]">{[item.resultLabel, item.durationLabel, item.priceLabel].filter(Boolean).join(' / ')}</p>}
              ...
              {asList<string>(item.steps).length > 0 && <p className="mt-2 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.steps).join(' / ')}</p>}
              {asList<string>(item.careTips).length > 0 && <p className="mt-2 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.careTips).join(' / ')}</p>}
