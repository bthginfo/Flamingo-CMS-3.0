      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((img, i) => (
          <article key={`${img.src}-${i}`} className="group">
            ...
            <div className="mt-3">
              {img.category && <p className="text-xs font-light uppercase tracking-[0.3em] text-gray-600">{img.category}</p>}
              {(img.caption || img.alt) && <p className="mt-1 text-sm font-light text-gray-900">{img.caption || img.alt}</p>}
            </div>
          </article>
        ))}
      </div>
