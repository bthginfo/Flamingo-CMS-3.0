  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] p-6 shadow-md sm:p-10">
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        {onlineCta.label && <a href={onlineCta.href || '#'} className="inline-flex items-center gap-2 rounded-full bg-[#111827] px-5 py-3 font-semibold text-[color:var(--token-on-dark-heading,#ffffff)] shadow-md"><CalendarDays size={17} /><span data-edit-path="label">{onlineCta.label}</span></a>}
        {phoneCta.label && <a href={phoneCta.href || '#'} className="inline-flex items-center gap-2 rounded-full border border-[var(--token-icon, var(--brand-primary))]/30 px-5 py-3 font-semibold text-[color:var(--token-heading,#18181b)]"><Phone size={17} /><span data-edit-path="label">{phoneCta.label}</span></a>}
      ...
      <div className="mt-6 flex flex-wrap gap-2">{notes.map((note) => <span key={note} className="rounded-full bg-[var(--token-btn-bg,var(--brand-primary,#1a5276))/10] px-3 py-1 text-xs text-[var(--token-eyebrow, var(--brand-accent))]" data-edit-path="note">{note}</span>)}</div>
    </motion.div>
  );
}
...
function BookingModern({ headline, subline, badgeText, introText, onlineCta, phoneCta, whatsappCta, notes }: Props) {
  return (
    <div className="border-y border-black/10 py-14">
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        {onlineCta.label && <a href={onlineCta.href || '#'} className="inline-flex items-center gap-2 border border-[#111827] px-6 py-3 font-light text-[color:var(--token-heading,#18181b)]"><CalendarDays size={16} /><span data-edit-path="label">{onlineCta.label}</span></a>}
        {phoneCta.label && <a href={phoneCta.href || '#'} className="inline-flex items-center gap-2 px-6 py-3 font-light text-[color:var(--token-muted,#52525b)]"><Phone size={16} /><span data-edit-path="label">{phoneCta.label}</span></a>}
      ...
      <div className="mt-6 flex flex-wrap gap-3">{notes.map((note) => <span key={note} className="border-b border-[var(--token-card-border,var(--brand-accent,#f39c12))] pb-1 text-xs font-light text-[color:var(--token-muted,#52525b)]" data-edit-path="note">{note}</span>)}</div>
    </div>
  );
}
...
function BookingBold({ headline, subline, badgeText, introText, onlineCta, phoneCta, whatsappCta, notes }: Props) {
  return (
    <div className="border-2 border-[#111827] bg-[#111] p-6 shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))] sm:p-10">
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        {onlineCta.label && <a href={onlineCta.href || '#'} className="inline-flex items-center gap-2 bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-6 py-3 font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)] shadow-[4px_4px_0_rgba(0,0,0,0.8)]"><CalendarDays size={17} /><span data-edit-path="label">{onlineCta.label}</span></a>}
        {phoneCta.label && <a href={phoneCta.href || '#'} className="inline-flex items-center gap-2 border-2 border-[color:var(--token-card-border,#ffffff)] px-6 py-3 font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)] shadow-[4px_4px_0_rgba(0,0,0,0.8)]"><Phone size={17} /><span data-edit-path="label">{phoneCta.label}</span></a>}
      ...
      <div className="mt-6 flex flex-wrap gap-2">{notes.map((note) => <span key={note} className="bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-3 py-1 text-xs font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)]" data-edit-path="note">{note}</span>)}</div>
    </div>
  );
}
