        ...
        {categories.map((item, i) => (
          <motion.article key={`${item.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md">
            ...
            <div className="p-5">
              ...
              {asList<string>(item.services).length > 0 && <p className="mt-2 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.services).join(' / ')}</p>}
        ...
        {categories.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group">
            ...
            <div className="mt-4">
              ...
              {asList<string>(item.services).length > 0 && <p className="mt-2 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(item.services).join(' / ')}</p>}
