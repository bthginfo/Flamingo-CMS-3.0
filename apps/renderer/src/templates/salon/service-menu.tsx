    ...
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {categories.map((item, i) => (
          <motion.article key={`${item.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md" data-edit-collection="categories" data-edit-index={i}>
            ...
            <div className="p-5">
              {item.category && <span className="inline-block rounded-full bg-[var(--token-btn-bg,var(--brand-primary,#1a5276))/10] px-3 py-1 text-xs font-bold uppercase text-[var(--token-eyebrow, var(--brand-accent))]" data-edit-path="category">{item.category}</span>}
              ...
              {asList<string>(item.services).length > 0 && <p className="mt-2 text-sm text-[color:var(--token-muted,#52525b)]">{asList<string>(item.services).join(' / ')}</p>}
  ...
  return (
    <div>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {categories.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group" data-edit-collection="categories" data-edit-index={i}>
            ...
            <div className="mt-4">
              {item.category && <p className="text-xs font-light uppercase tracking-[0.3em] text-[color:var(--token-muted,#52525b)]" data-edit-path="category">{item.category}</p>}
              ...
              {asList<string>(item.services).length > 0 && <p className="mt-2 text-sm font-light text-[color:var(--token-muted,#52525b)]">{asList<string>(item.services).join(' / ')}</p>}
...
function ServiceBold({ headline, subline, badgeText, categories, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {categories.map((item, i) => (
          <article key={`${item.title}-${i}`} className="group overflow-hidden border-2 border-[#111827] bg-[#111] shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]" data-edit-collection="categories" data-edit-index={i}>
            {item.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={item.title || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-5">
              {item.category && <span className="inline-block bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))] px-3 py-1 text-xs font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)]" data-edit-path="category">{item.category}</span>}
              ...
              {asList<string>(item.services).length > 0 && <p className="mt-2 text-sm text-[color:var(--token-on-dark-heading,#ffffff)/60]">{asList<string>(item.services).join(' / ')}</p>}
