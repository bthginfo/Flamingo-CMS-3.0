    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {categories.map((cat, ci) => (
          <motion.article key={`${cat.title}-${ci}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: ci * 0.1 }} className="rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-white p-6 shadow-md">
            <h3 className="text-2xl font-bold text-gray-900">{cat.title || ''}</h3>
            {cat.text && <div className="mt-2 text-sm text-gray-600 rt-content" dangerouslySetInnerHTML={{ __html: cat.text }} />}
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        {categories.map((cat, ci) => (
          <article key={`${cat.title}-${ci}`} className="border-t border-black/10 pt-6">
            ...
            {cat.text && <div className="mt-2 text-sm font-light text-gray-600 rt-content" dangerouslySetInnerHTML={{ __html: cat.text }} />}
            <div className="mt-5 divide-y divide-black/10">
              {asList<PriceItem>(cat.items).map((item, ii) => (
                <div key={`${item.name}-${ii}`} className="py-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-3 text-xs font-light text-gray-600">{item.durationLabel && <span>{item.durationLabel}</span>}{item.note && <span>{item.note}</span>}{item.cta?.label && <a href={item.cta.href || '#'} className="border-b border-[var(--token-card-border,var(--brand-accent,#f39c12))] pb-1 text-gray-900">{item.cta.label}</a>}</div>
                </div>
              ))}
            </div>
          </article>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {categories.map((cat, ci) => (
          <article key={`${cat.title}-${ci}`} className="border-2 border-[#111827] bg-[#111] p-6 shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]">
            ...
            {cat.text && <div className="mt-2 text-sm text-white/60 rt-content" dangerouslySetInnerHTML={{ __html: cat.text }} />}
            <div className="mt-5 divide-y-2 divide-[#111827]">
              {asList<PriceItem>(cat.items).map((item, ii) => (
                <div key={`${item.name}-${ii}`} className="py-4">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 sm:gap-4"><div className="min-w-0"><p className="font-black uppercase text-white">{item.name || ''}</p>{item.description && <div className="mt-1 text-sm text-white/60 rt-content" dangerouslySetInnerHTML={{ __html: item.description }} />}</div>{item.priceLabel && <p className="shrink-0 whitespace-nowrap font-black text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]">{item.priceLabel}</p>}</div>
                  <div className="mt-2 flex flex-wrap gap-3 text-xs font-bold text-white/50">{item.durationLabel && <span>{item.durationLabel}</span>}{item.note && <span>{item.note}</span>}{item.cta?.label && <a href={item.cta.href || '#'} className="font-black uppercase text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]">{item.cta.label}</a>}</div>
                </div>
              ))}
            </div>
          </article>
