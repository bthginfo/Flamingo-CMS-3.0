...
function HeroClassic({ headline, subline, badgeText, bgImage, bgImageMobile, bgColor, bgMode, locationLabel, seasonLabel, trustItems, primaryCta, secondaryCta, overlayColor, overlayOpacity, bgPosition, bgPositionMobile, imageEffect, imageEffectIntensity}: HeroProps) {
  return (
    <section className="relative min-h-screen overflow-hidden -mt-[112px] pt-[112px] bg-[var(--token-section-bg, var(--token-section-bg, var(--style-section-bg,var(--style-text-primary,#111827))))]">
      ...
      <svg className="absolute bottom-0 left-0 w-full text-[var(--token-section-bg-alt, var(--token-section-bg-alt, var(--style-section-bg-alt,#ffffff)))]" viewBox="0 0 1440 120" preserveAspectRatio="none" aria-hidden="true">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.45 }} className="mt-8 flex flex-wrap justify-center gap-3">
          {primaryCta.label && (
            <a
              ...
              style={{
                backgroundColor: 'var(--token-btn-bg,var(--token-btn-bg, var(--brand-btn-bg,var(--style-accent-color,#D99A3D))))',
                color: 'var(--token-btn-text,var(--token-btn-text, var(--brand-btn-text,#0C211B)))',
              }}
            >
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-112px)] max-w-7xl items-center gap-10 px-6 py-12 md:py-20 lg:grid-cols-2">
        <div className="max-w-xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-lg border border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,rgba(0,0,0,.15))))] px-5 py-3 font-semibold text-[var(--style-text-primary,#111827)]">{secondaryCta.label}</a>}
          </div>
          <div className="mt-10 flex flex-wrap gap-3 text-sm text-[var(--style-text-secondary,#4b5563)]">
            {locationLabel && <span className="inline-flex items-center gap-2 rounded-full border border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,rgba(0,0,0,.1))))] px-4 py-2"><MapPin size={15} />{locationLabel}</span>}
            {seasonLabel && <span className="rounded-full border border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,rgba(0,0,0,.1))))] px-4 py-2">{seasonLabel}</span>}
        ...
        <div className="relative min-h-[500px] overflow-hidden rounded-xl border border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,rgba(0,0,0,.1))))]">
          {(bgMode === 'image' && bgImage) ? (
            <>
...
function HeroBold({ headline, subline, badgeText, bgImage, bgImageMobile, bgColor, bgMode, locationLabel, seasonLabel, trustItems, primaryCta, secondaryCta , bgPosition, bgPositionMobile, imageEffect, imageEffectIntensity}: HeroProps) {
  return (
    <section className="relative min-h-screen overflow-hidden -mt-[112px] pt-[112px] bg-[var(--token-section-bg, var(--token-section-bg, var(--style-section-bg,var(--style-text-primary,#030712))))]">
      {(bgMode === 'image' && bgImage) ? (
        <>
          ...
          <div className="absolute inset-0 bg-[var(--token-section-bg, var(--token-section-bg, var(--style-section-bg,var(--style-text-primary,#030712))))] opacity-60" />
        </>
      ) : (bgMode === 'color' && bgColor) ? (
        <div className="absolute inset-0" style={{ backgroundColor: bgColor }} />
      ) : null}
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-112px)] max-w-7xl flex-col justify-center px-6 py-12 md:py-20">
        <div className="flex items-center gap-3">
          <Compass className="text-[var(--token-icon, var(--token-icon, var(--style-icon-color,var(--style-accent-color,var(--token-icon, var(--brand-primary))))))]" size={28} />
        ...
        <div className="mt-10 flex flex-wrap gap-3 text-sm text-[var(--token-body, var(--token-body, var(--style-body-color,rgba(255,255,255,.7))))]">
          {locationLabel && <span className="inline-flex items-center gap-2 border border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,rgba(255,255,255,.2))))] px-4 py-2 font-bold uppercase"><MapPin size={15} />{locationLabel}</span>}
          {seasonLabel && <span className="border border-[var(--token-card-border, var(--token-card-border, var(--style-border-color,rgba(255,255,255,.2))))] px-4 py-2 font-bold uppercase">{seasonLabel}</span>}
