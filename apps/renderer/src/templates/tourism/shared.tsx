  ...
  return (
    <article className="group overflow-hidden rounded-xl border border-[var(--token-card-border)] bg-[var(--token-card-bg)] shadow-sm">
      ...
      <div className="p-5">
        ...
        {text && <div className="mt-3 text-sm leading-6 text-[color:var(--token-body)] rt-content" data-edit-rich="text" dangerouslySetInnerHTML={{ __html: text }} />}
    ...
    <div className="grid gap-4">
      {asList<{ icon?: string; title?: string; text?: string }>(items).map((item, index) => (
        <div key={`${item.title}-${index}`} className="flex gap-4 border-t border-[var(--token-card-border)] pt-4" data-edit-collection="items" data-edit-index={index}>
          <DynamicIcon editPath="icon" name={item.icon || 'map-pin'} size={20} className="text-[color:var(--token-icon)]" />
          <div>
