...
function Classic({ headline, subline, badgeText, ratingValue, ratingCount, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      {(ratingValue || ratingCount) && <p className="mb-6 text-sm text-[color:var(--token-muted,#52525b)]">{[ratingValue, ratingCount].filter(Boolean).join(' · ')}</p>}
      ...
      {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-8 inline-flex items-center gap-2 rounded-lg bg-[#111827] px-5 py-3 font-semibold text-[color:var(--token-on-dark-heading,#ffffff)]" data-edit-path="label">{ctaPrimary.label}</a>}
    </div>
  );
}
...
function Mod({ headline, subline, badgeText, ratingValue, ratingCount, items, ctaPrimary }: Props) {
  return (
    <div>
      <SectionHeader headline={headline} subline={plain(subline)} badgeText={badgeText} />
      {(ratingValue || ratingCount) && <p className="mb-8 text-sm font-light text-[color:var(--token-muted,#52525b)]">{[ratingValue, ratingCount].filter(Boolean).join(' · ')}</p>}
      ...
      {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-10 inline-flex items-center gap-2 rounded-lg border border-[#111827] px-5 py-3 font-medium text-[color:var(--token-heading,#18181b)]" data-edit-path="label">{ctaPrimary.label}</a>}
    </div>
  );
}
...
function Bold({ headline, subline, badgeText, ratingValue, ratingCount, items, ctaPrimary }: Props) {
  return (
    <div>
      <SectionHeader headline={headline} subline={plain(subline)} badgeText={badgeText} />
      {(ratingValue || ratingCount) && <p className="mb-6 text-sm font-bold uppercase text-[color:var(--token-muted,#52525b)]">{[ratingValue, ratingCount].filter(Boolean).join(' · ')}</p>}
      <div className="grid gap-5 md:grid-cols-3">
        {items.map((item, i) => (
