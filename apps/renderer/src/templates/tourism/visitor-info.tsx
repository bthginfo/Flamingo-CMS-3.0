        ...
        {blocks.map((block, index) => (
          <motion.article key={`${block.title}-${index}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} className="rounded-xl bg-[var(--token-card-bg, var(--style-card-bg,#fff))] p-5 shadow-lg">
            <div className="flex gap-4">
              <DynamicIcon name={block.icon || 'map-pin'} size={20} className="text-[var(--token-icon, var(--style-icon-color,var(--style-accent-color,var(--brand-primary))))]" />
              <div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">{asList<string>(block.items).map((item) => <span key={item} className="rounded-full bg-[var(--token-badge-bg, var(--style-badge-bg,color-mix(in_srgb,var(--style-accent-color,var(--brand-primary))_10%,#fff)))] px-3 py-1 text-xs text-[var(--token-badge-text, var(--style-badge-text,var(--style-accent-color,var(--brand-primary))))]">{item}</span>)}</div>
          </motion.article>
        ))}
      </div>
    </div>
