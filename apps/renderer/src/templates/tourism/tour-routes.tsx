...
function RouteMeta({ route, className }: { route: TourRoute; className?: string }) {
  return (
    <div className={`flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-widest ${className || 'text-[var(--style-text-secondary)]'}`}>
...
function Modern({ header, routes, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {routes.map((route, index) => (
          <article key={`${route.title}-${index}`} className="overflow-hidden border border-black/10 bg-[var(--style-card-bg)]">
            {route.image && <div className="relative aspect-[16/9]"><Image src={route.image} alt={route.title || ''} fill className="object-cover" sizes="50vw" /></div>}
            <div className="p-6">
              <RouteMeta route={route} className="text-teal-600 font-light" />
              <h3 className="mt-3 text-2xl font-light text-[var(--style-text-primary)]">{route.title || ''}</h3>
              {route.text && <p className="mt-3 text-sm font-light leading-6 text-[var(--style-text-secondary)]">{route.text}</p>}
              <div className="mt-4 flex flex-wrap gap-2">{asList<string>(route.highlights).map((item) => <span key={item} className="border border-black/10 px-3 py-1 text-xs text-[var(--style-text-secondary)]">{item}</span>)}</div>
...
function Bold({ header, routes, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {routes.map((route, index) => (
          <article key={`${route.title}-${index}`} className="overflow-hidden border-2 border-[var(--style-text-primary)] bg-[var(--style-card-bg)] shadow-[4px_4px_0_var(--style-text-primary)]">
            {route.image && <div className="relative aspect-[16/9]"><Image src={route.image} alt={route.title || ''} fill className="object-cover" sizes="50vw" /></div>}
            <div className="p-6">
              <RouteMeta route={route} className="text-orange-500 font-black" />
              <h3 className="mt-3 text-2xl font-black uppercase text-[var(--style-text-primary)]">{route.title || ''}</h3>
              {route.text && <p className="mt-3 text-sm leading-6 text-[var(--style-text-secondary)]">{route.text}</p>}
