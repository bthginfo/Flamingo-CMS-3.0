...
function Modern({ header, routes, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {routes.map((route, index) => (
          <article key={`${route.title || 'item'}-${index}`} className="overflow-hidden border border-[var(--token-card-border)] bg-[var(--token-card-bg)]" data-edit-collection="routes" data-edit-index={index}>
            {route.image && <div className="relative aspect-[16/9]"><Image data-edit-image="image" src={route.image} alt={route.title || ''} fill className="object-cover" sizes="50vw" /></div>}
            <div className="p-6">
              <RouteMeta route={route} className="text-[color:var(--token-badge-text)] font-light" />
              <h3 className="mt-3 text-2xl font-light text-[color:var(--token-heading)]" data-edit-path="title">{route.title || ''}</h3>
              {route.text && <div className="mt-3 text-sm font-light leading-6 text-[color:var(--token-body)] rt-content" data-edit-rich="text" dangerouslySetInnerHTML={{ __html: route.text }} />}
              <div className="mt-4 flex flex-wrap gap-2">{asList<string>(route.highlights).map((item) => <span key={item} className="border border-[var(--token-card-border)] px-3 py-1 text-xs text-[color:var(--token-muted)]">{item}</span>)}</div>
              {route.cta?.label && <div className="mt-5"><a href={route.cta.href || '#'} className="inline-flex items-center gap-2 font-semibold text-[color:var(--token-accent)]"><span data-edit-path="label">{route.cta.label}</span><ArrowRight size={14} /></a></div>}
            </div>
          </article>
        ))}
      </div>
...
function Bold({ header, routes, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {routes.map((route, index) => (
          <article key={`${route.title || 'item'}-${index}`} className="overflow-hidden border-2 border-[var(--token-card-border)] bg-[var(--token-card-bg)] shadow-[4px_4px_0_var(--token-card-border)]" data-edit-collection="routes" data-edit-index={index}>
            {route.image && <div className="relative aspect-[16/9]"><Image data-edit-image="image" src={route.image} alt={route.title || ''} fill className="object-cover" sizes="50vw" /></div>}
            <div className="p-6">
              <RouteMeta route={route} className="text-[color:var(--token-badge-text)] font-black" />
              <h3 className="mt-3 text-2xl font-black uppercase text-[color:var(--token-heading)]" data-edit-path="title">{route.title || ''}</h3>
              {route.text && <div className="mt-3 text-sm leading-6 text-[color:var(--token-body)] rt-content" data-edit-rich="text" dangerouslySetInnerHTML={{ __html: route.text }} />}
              <div className="mt-4 flex flex-wrap gap-2">{asList<string>(route.highlights).map((item) => <span key={item} className="border border-[var(--token-accent)] px-3 py-1 text-xs font-bold uppercase text-[color:var(--token-accent)]">{item}</span>)}</div>
              {route.cta?.label && <div className="mt-5"><a href={route.cta.href || '#'} className="inline-flex items-center gap-2 border-2 border-[var(--token-btn-bg)] bg-[var(--token-btn-bg)] px-5 py-2.5 text-sm font-black uppercase text-[color:var(--token-btn-text)]"><span data-edit-path="label">{route.cta.label}</span><ArrowRight size={14} /></a></div>}
            </div>
          </article>
        ))}
      </div>
