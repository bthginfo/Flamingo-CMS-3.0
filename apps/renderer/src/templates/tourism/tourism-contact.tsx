  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-[var(--style-button-radius)] border border-teal-600 bg-teal-600 px-5 py-3 font-semibold text-white">{primaryCta.label}<ArrowRight size={16} /></a>}
      ...
      <div className="border border-black/10 bg-[var(--style-card-bg)] p-5">
        ...
        {formEnabled && <form className="grid gap-3"><input className="admin-input" placeholder={namePlaceholder} readOnly /><input className="admin-input" placeholder={emailPlaceholder} readOnly /><textarea className="admin-input" placeholder={messagePlaceholder} readOnly /><button type="button" className="rounded-[var(--style-button-radius)] border border-teal-600 bg-teal-600 px-5 py-3 font-semibold text-white">{submitLabel}</button></form>}
      </div>
    </div>
  );
}
