      ...
      <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
        ...
        <div className="mt-6 grid gap-3">
          {infoCards.map((card, index) => (
            <div key={`${card.label}-${index}`} className="flex gap-4 border-t border-[var(--style-border-color,rgba(0,0,0,.1))] pt-4">
              <DynamicIcon name={card.icon || 'mail'} size={20} className="text-[var(--style-icon-color,var(--style-accent-color,var(--brand-primary)))]" />
              <div><p className="text-xs text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{card.label || ''}</p><p className="font-semibold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{card.value || ''}</p></div>
            </div>
          ))}
        </div>
        <div className="mt-8 flex flex-wrap gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full bg-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] px-5 py-3 font-semibold text-[var(--brand-btn-text,#fff)]">{primaryCta.label}<ArrowRight size={16} /></a>}
          {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-full border border-[var(--style-border-color,rgba(0,0,0,.15))] px-5 py-3 font-semibold text-[var(--style-text-primary,#111827)]">{secondaryCta.label}</a>}
        </div>
      </motion.div>
      <div className="rounded-xl bg-[var(--style-card-bg,#fff)] p-5 shadow-lg">
...
function Modern({ header, introText, image, formEnabled, submitLabel, formFields, infoCards, primaryCta, secondaryCta }: Props) {
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-6 grid gap-3">
          {infoCards.map((card, index) => (
            <div key={`${card.label}-${index}`} className="flex gap-4 border-t border-[var(--style-border-color,rgba(0,0,0,.1))] pt-4">
              <DynamicIcon name={card.icon || 'mail'} size={20} className="text-[var(--style-icon-color,var(--style-accent-color,var(--brand-primary)))]" />
              <div><p className="text-xs font-light text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{card.label || ''}</p><p className="font-light text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{card.value || ''}</p></div>
            </div>
          ))}
        </div>
        <div className="mt-8 flex flex-wrap gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 rounded-lg border border-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] bg-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] px-5 py-3 font-semibold text-[var(--brand-btn-text,#fff)]">{primaryCta.label}<ArrowRight size={16} /></a>}
      ...
      <div className="border border-[var(--style-border-color,rgba(0,0,0,.1))] bg-[var(--style-card-bg,#fff)] p-5">
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-6 grid gap-3">
          {infoCards.map((card, index) => (
            <div key={`${card.label}-${index}`} className="flex gap-4 border-t-2 border-[var(--style-border-color,var(--style-text-primary,#111827))] pt-4">
              <DynamicIcon name={card.icon || 'mail'} size={20} className="text-[var(--style-icon-color,var(--style-accent-color,var(--brand-primary)))]" />
              <div><p className="text-xs font-bold uppercase text-[var(--style-text-muted,var(--style-text-secondary,#4b5563))]">{card.label || ''}</p><p className="font-black text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{card.value || ''}</p></div>
            </div>
          ))}
        </div>
        <div className="mt-8 flex flex-wrap gap-3">
          {primaryCta.label && <a href={primaryCta.href || '#'} className="inline-flex items-center gap-2 border-2 border-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] bg-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] px-5 py-3 font-black uppercase text-[var(--brand-btn-text,#fff)]">{primaryCta.label}<ArrowRight size={16} /></a>}
          {secondaryCta.label && <a href={secondaryCta.href || '#'} className="inline-flex items-center gap-2 border-2 border-[var(--style-border-color,var(--style-text-primary,#111827))] px-5 py-3 font-black uppercase text-[var(--style-text-primary,#111827)]">{secondaryCta.label}</a>}
        </div>
      </div>
      <div className="border-2 border-[var(--style-border-color,var(--style-text-primary,#111827))] bg-[var(--style-card-bg,#fff)] p-5 shadow-[4px_4px_0_var(--style-border-color,var(--style-text-primary,#111827))]">
        {image && <div className="relative mb-5 aspect-[16/10] overflow-hidden"><Image src={image} alt="" fill className="object-cover" sizes="50vw" /></div>}
        {formEnabled && <DynamicContactForm fields={formFields} submitLabel={submitLabel} />}
      </div>
    </div>
