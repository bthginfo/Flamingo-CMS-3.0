    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <motion.article key={`${image.src}-${index}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.08 }} className="group overflow-hidden rounded-xl bg-white shadow-lg">
            ...
            <div className="p-4">
              ...
              {(image.caption || image.alt) && <h3 className="mt-1 font-bold text-gray-900">{image.caption || image.alt}</h3>}
            </div>
          </motion.article>
        ))}
      </div>
...
function Modern({ header, images }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <article key={`${image.src}-${index}`} className="group overflow-hidden border border-black/10 bg-white">
            ...
            <div className="p-4">
              ...
              {(image.caption || image.alt) && <h3 className="mt-1 font-light text-gray-900">{image.caption || image.alt}</h3>}
            </div>
          </article>
        ))}
      </div>
...
function Bold({ header, images }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <article key={`${image.src}-${index}`} className="group overflow-hidden border-2 border-[#111827] bg-white shadow-[4px_4px_0_#111827]">
            {image.src && <div className="relative aspect-[4/3] overflow-hidden"><Image src={image.src} alt={image.alt || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-4">
              {image.category && <p className="text-xs font-black uppercase tracking-widest text-orange-500">{image.category}</p>}
              {(image.caption || image.alt) && <h3 className="mt-1 font-black uppercase text-gray-900">{image.caption || image.alt}</h3>}
            </div>
          </article>
        ))}
      </div>
