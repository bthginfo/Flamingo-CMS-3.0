      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <motion.article key={`${image.src}-${index}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.08 }} className="group overflow-hidden rounded-xl bg-[var(--token-card-bg)] shadow-lg" data-edit-collection="images" data-edit-index={index}>
            ...
            <div className="p-4">
              {image.category && <p className="text-xs font-bold uppercase tracking-widest text-[var(--token-success,theme(colors.green.700))]" data-edit-path="category">{image.category}</p>}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <article key={`${image.src}-${index}`} className="group overflow-hidden border border-black/10 bg-[var(--token-card-bg)]" data-edit-collection="images" data-edit-index={index}>
            ...
            <div className="p-4">
              {image.category && <p className="text-xs font-light uppercase tracking-widest text-[var(--token-success,theme(colors.teal.600))]" data-edit-path="category">{image.category}</p>}
...
function Bold({ header, images }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <article key={`${image.src}-${index}`} className="group overflow-hidden border-2 border-[var(--token-card-border)] bg-[var(--token-card-bg)] shadow-[4px_4px_0_var(--token-card-border)]" data-edit-collection="images" data-edit-index={index}>
            {image.src && <div className="relative aspect-[4/3] overflow-hidden"><Image data-edit-image="src" src={image.src} alt={image.alt || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-4">
              {image.category && <p className="text-xs font-black uppercase tracking-widest text-[var(--token-accent,theme(colors.orange.500))]" data-edit-path="category">{image.category}</p>}
