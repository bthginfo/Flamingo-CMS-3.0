...
export function DownloadGuidesSection({ data }: SectionProps) {
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="rounded-[var(--style-card-radius)] border border-black/10 bg-[var(--style-card-bg)] p-5 shadow-[var(--style-card-shadow)]">
            <Download size={22} />
            {item.metaLabel && <p className="mt-4 text-xs font-bold uppercase tracking-widest text-[var(--style-text-secondary)]">{item.metaLabel}</p>}
            ...
            {item.fileLabel && <a href={item.fileHref || '#'} className="mt-5 inline-flex font-semibold text-[var(--style-text-primary)]">{item.fileLabel}</a>}
          </article>
        ))}
      </div>
    </div>
