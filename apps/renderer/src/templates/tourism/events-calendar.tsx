...
function EventMeta({ event }: { event: EventItem }) {
  return (
    <div className="flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-widest text-[var(--style-body-color,var(--style-text-secondary,#4b5563))]">
...
function Classic({ header, events, fallbackText }: Props) {
  return (
    <div>
      ...
      {events.length === 0 && fallbackText ? <p className="text-[var(--style-body-color,var(--style-text-secondary,#4b5563))]">{fallbackText}</p> : null}
        ...
        {events.map((event, index) => (
          <motion.article key={`${event.title}-${index}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} className="grid overflow-hidden rounded-xl bg-white shadow-lg sm:grid-cols-[180px_1fr]">
            ...
            <div className="p-5">
              ...
              <h3 className="mt-3 text-xl font-bold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{event.title || ''}</h3>
              {event.text && <div className="mt-3 text-sm leading-6 text-[var(--style-body-color,var(--style-text-secondary,#4b5563))] rt-content" dangerouslySetInnerHTML={{ __html: event.text }} />}
        ...
        {events.map((event, index) => (
          <article key={`${event.title}-${index}`} className="grid overflow-hidden border border-black/10 bg-white sm:grid-cols-[180px_1fr]">
            ...
            <div className="p-5">
              ...
              <h3 className="mt-3 text-xl font-light text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{event.title || ''}</h3>
              {event.text && <div className="mt-3 text-sm font-light leading-6 text-[var(--style-body-color,var(--style-text-secondary,#4b5563))] rt-content" dangerouslySetInnerHTML={{ __html: event.text }} />}
              {event.cta?.label && <a href={event.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 font-semibold text-[var(--style-accent-color,var(--brand-primary))]">{event.cta.label}<ArrowRight size={14} /></a>}
            </div>
          </article>
        ))}
...
function Bold({ header, events, fallbackText }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {events.map((event, index) => (
          <article key={`${event.title}-${index}`} className="grid overflow-hidden border-2 border-[var(--style-border-color,var(--style-text-primary,#111827))] bg-white shadow-[4px_4px_0_var(--style-border-color,var(--style-text-primary,#111827))] sm:grid-cols-[180px_1fr]">
            {event.image && <div className="relative min-h-48"><Image src={event.image} alt={event.title || ''} fill className="object-cover" sizes="220px" /></div>}
            <div className="p-5">
              ...
              <h3 className="mt-3 text-xl font-black uppercase text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{event.title || ''}</h3>
              {event.text && <div className="mt-3 text-sm leading-6 text-[var(--style-body-color,var(--style-text-secondary,#4b5563))] rt-content" dangerouslySetInnerHTML={{ __html: event.text }} />}
