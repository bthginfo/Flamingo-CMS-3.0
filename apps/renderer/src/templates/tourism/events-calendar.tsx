...
function Modern({ header, events, fallbackText }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {events.map((event, index) => (
          <article key={`${event.title || 'item'}-${index}`} className="grid overflow-hidden border border-[color:color-mix(in_srgb,var(--token-card-border)_60%,transparent)] bg-[var(--token-card-bg)] sm:grid-cols-[180px_1fr]" data-edit-collection="events" data-edit-index={index}>
            {event.image && <div className="relative min-h-48"><Image data-edit-image="image" src={event.image} alt={event.title || ''} fill className="object-cover" sizes="220px" /></div>}
            <div className="p-5">
              <div className="flex flex-wrap gap-3 text-xs font-light uppercase tracking-widest text-[color:var(--token-accent)]">
                {event.dateLabel && <span className="inline-flex items-center gap-1"><CalendarDays size={13} />{event.dateLabel}</span>}
                {event.timeLabel && <span>{event.timeLabel}</span>}
                {event.locationLabel && <span className="inline-flex items-center gap-1"><MapPin size={13} />{event.locationLabel}</span>}
                {event.category && <span data-edit-path="category">{event.category}</span>}
              ...
              <h3 className="mt-3 text-xl font-light text-[color:var(--token-heading)]" data-edit-path="title">{event.title || ''}</h3>
              {event.text && <div className="mt-3 text-sm font-light leading-6 text-[color:var(--token-body)] rt-content" data-edit-rich="text" dangerouslySetInnerHTML={{ __html: event.text }} />}
              {event.cta?.label && <a href={event.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 font-semibold text-[color:var(--token-accent)]"><span data-edit-path="label">{event.cta.label}</span><ArrowRight size={14} /></a>}
            </div>
          </article>
        ))}
  ...
  return (
    <div>
      <div className="mb-10 max-w-3xl">
        {header.badgeText && <p className="text-xs font-black uppercase tracking-widest text-[color:var(--token-badge-text,var(--token-accent))]" data-edit-path="badgeText">{header.badgeText}</p>}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {events.map((event, index) => (
          <article key={`${event.title || 'item'}-${index}`} className="grid overflow-hidden border-2 border-[var(--token-card-border)] bg-[var(--token-card-bg)] shadow-[4px_4px_0_var(--token-card-border)] sm:grid-cols-[180px_1fr]" data-edit-collection="events" data-edit-index={index}>
            {event.image && <div className="relative min-h-48"><Image data-edit-image="image" src={event.image} alt={event.title || ''} fill className="object-cover" sizes="220px" /></div>}
            <div className="p-5">
              <div className="flex flex-wrap gap-3 text-xs font-black uppercase tracking-widest text-[color:var(--token-accent)]">
                {event.dateLabel && <span className="inline-flex items-center gap-1"><CalendarDays size={13} />{event.dateLabel}</span>}
                {event.timeLabel && <span>{event.timeLabel}</span>}
                {event.locationLabel && <span className="inline-flex items-center gap-1"><MapPin size={13} />{event.locationLabel}</span>}
                {event.category && <span data-edit-path="category">{event.category}</span>}
              ...
              <h3 className="mt-3 text-xl font-black uppercase text-[color:var(--token-heading)]" data-edit-path="title">{event.title || ''}</h3>
              {event.text && <div className="mt-3 text-sm leading-6 text-[color:var(--token-body)] rt-content" data-edit-rich="text" dangerouslySetInnerHTML={{ __html: event.text }} />}
              {event.cta?.label && <a href={event.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 font-black uppercase text-[color:var(--token-accent)]"><span data-edit-path="label">{event.cta.label}</span><ArrowRight size={14} /></a>}
            </div>
          </article>
        ))}
      </div>
