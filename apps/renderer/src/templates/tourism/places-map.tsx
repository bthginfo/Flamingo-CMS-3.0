  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
      <div>
        ...
        <div className="grid gap-4">
          {places.map((place, index) => (
            <article key={`${place.title || 'item'}-${index}`} className="grid gap-4 border-t-2 border-[var(--token-card-border)] pt-4 sm:grid-cols-[120px_1fr]" data-edit-collection="places" data-edit-index={index}>
              {place.image && <div className="relative aspect-[4/3] overflow-hidden"><Image data-edit-image="image" src={place.image} alt={place.title || ''} fill className="object-cover" sizes="160px" /></div>}
              <div>
