...
function MapEmbed({ mapEmbedUrl, mapFallbackText, className }: { mapEmbedUrl: string; mapFallbackText: string; className: string }) {
  return (
    <div className={className}>
      {mapEmbedUrl ? <iframe src={mapEmbedUrl} className="h-full min-h-[420px] w-full" loading="lazy" /> : <div className="flex h-full min-h-[420px] items-center justify-center p-8 text-center text-[var(--style-text-secondary)]">{mapFallbackText}</div>}
    </div>
  );
}
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
      <div>
          ...
          {places.map((place, index) => (
            <article key={`${place.title}-${index}`} className="grid gap-4 border-t border-black/10 pt-4 sm:grid-cols-[120px_1fr]">
              ...
              <div>
                ...
                <h3 className="mt-1 font-light text-[var(--style-text-primary)]">{place.title || ''}</h3>
                {place.address && <p className="mt-1 inline-flex items-center gap-1 text-xs text-[var(--style-text-secondary)]"><MapPin size={13} />{place.address}</p>}
                {place.text && <p className="mt-2 text-sm font-light leading-6 text-[var(--style-text-secondary)]">{place.text}</p>}
        ...
        {ctaPrimary.label && <div className="mt-8"><a href={ctaPrimary.href || '#'} className="inline-flex items-center gap-2 rounded-[var(--style-button-radius)] border border-teal-600 bg-teal-600 px-5 py-3 font-semibold text-white">{ctaPrimary.label}<ArrowRight size={16} /></a></div>}
      </div>
      <MapEmbed mapEmbedUrl={mapEmbedUrl} mapFallbackText={mapFallbackText} className="min-h-[420px] overflow-hidden border border-black/10 bg-[var(--style-section-bg-alt)]" />
    </div>
  );
}
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
      <div>
        ...
        <div className="grid gap-4">
          {places.map((place, index) => (
            <article key={`${place.title}-${index}`} className="grid gap-4 border-t-2 border-[var(--style-text-primary)] pt-4 sm:grid-cols-[120px_1fr]">
              {place.image && <div className="relative aspect-[4/3] overflow-hidden"><Image src={place.image} alt={place.title || ''} fill className="object-cover" sizes="160px" /></div>}
              <div>
                ...
                <h3 className="mt-1 font-black uppercase text-[var(--style-text-primary)]">{place.title || ''}</h3>
                {place.address && <p className="mt-1 inline-flex items-center gap-1 text-xs text-[var(--style-text-secondary)]"><MapPin size={13} />{place.address}</p>}
                {place.text && <p className="mt-2 text-sm leading-6 text-[var(--style-text-secondary)]">{place.text}</p>}
      ...
      <MapEmbed mapEmbedUrl={mapEmbedUrl} mapFallbackText={mapFallbackText} className="min-h-[420px] overflow-hidden border-2 border-[var(--style-text-primary)] bg-[var(--style-section-bg-alt)] shadow-[4px_4px_0_var(--style-text-primary)]" />
    </div>
  );
}
