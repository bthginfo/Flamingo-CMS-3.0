      ...
      <div>
        ...
        <div className="grid gap-4">
          {places.map((place, index) => (
            <motion.article key={`${place.title || 'item'}-${index}`} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: index * 0.1 }} className="grid gap-4 border-t border-[var(--token-card-border)] pt-4 sm:grid-cols-[120px_1fr]" data-card data-edit-collection="places" data-edit-index={index}>
