    ...
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {items.map((item, index) => (
          <motion.article key={`${item.title}-${index}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} className="grid overflow-hidden rounded-xl bg-[var(--style-card-bg,#fff)] shadow-lg sm:grid-cols-[220px_1fr]">
            ...
            <div className="p-5">
              <ExperienceMeta item={item} className="text-[var(--style-badge-text,var(--style-accent-color,var(--brand-primary)))]" />
              <h3 className="mt-3 text-2xl font-bold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{item.title || ''}</h3>
              ...
              {item.cta?.label && <div className="mt-5"><a href={item.cta.href || '#'} className="inline-flex items-center gap-2 rounded-full bg-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] px-5 py-2.5 text-sm font-semibold text-[var(--brand-btn-text,#fff)]">{item.cta.label}<ArrowRight size={14} /></a></div>}
            </div>
          </motion.article>
        ))}
      </div>
...
function Modern({ header, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="grid overflow-hidden border border-[var(--style-border-color,rgba(0,0,0,.1))] bg-[var(--style-card-bg,#fff)] sm:grid-cols-[220px_1fr]">
            {item.image && <div className="relative min-h-56"><Image src={item.image} alt={item.title || ''} fill className="object-cover" sizes="260px" /></div>}
            <div className="p-5">
              <ExperienceMeta item={item} className="text-[var(--style-badge-text,var(--style-accent-color,var(--brand-primary)))] font-light" />
              <h3 className="mt-3 text-2xl font-light text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{item.title || ''}</h3>
              ...
              {item.cta?.label && <div className="mt-5"><a href={item.cta.href || '#'} className="inline-flex items-center gap-2 font-semibold text-[var(--style-accent-color,var(--brand-primary))]">{item.cta.label}<ArrowRight size={14} /></a></div>}
            </div>
          </article>
        ))}
      </div>
...
function Bold({ header, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="grid overflow-hidden border-2 border-[var(--style-border-color,var(--style-text-primary,#111827))] bg-[var(--style-card-bg,#fff)] shadow-[4px_4px_0_var(--style-border-color,var(--style-text-primary,#111827))] sm:grid-cols-[220px_1fr]">
            {item.image && <div className="relative min-h-56"><Image src={item.image} alt={item.title || ''} fill className="object-cover" sizes="260px" /></div>}
            <div className="p-5">
              <ExperienceMeta item={item} className="text-[var(--style-badge-text,var(--style-accent-color,var(--brand-primary)))] font-black" />
              <h3 className="mt-3 text-2xl font-black uppercase text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{item.title || ''}</h3>
              {item.text && <div className="mt-3 text-sm leading-6 text-[var(--style-body-color,var(--style-text-secondary,#4b5563))] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
              {item.cta?.label && <div className="mt-5"><a href={item.cta.href || '#'} className="inline-flex items-center gap-2 border-2 border-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] bg-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-primary)))] px-5 py-2.5 text-sm font-black uppercase text-[var(--brand-btn-text,#fff)]">{item.cta.label}<ArrowRight size={14} /></a></div>}
            </div>
          </article>
        ))}
      </div>
