...
function Modern({ header, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="grid overflow-hidden border border-black/10 bg-[var(--style-card-bg)] sm:grid-cols-[220px_1fr]">
            {item.image && <div className="relative min-h-56"><Image src={item.image} alt={item.title || ''} fill className="object-cover" sizes="260px" /></div>}
            <div className="p-5">
              <ExperienceMeta item={item} className="text-teal-600 font-light" />
              <h3 className="mt-3 text-2xl font-light text-[var(--style-text-primary)]">{item.title || ''}</h3>
...
function Bold({ header, items, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {items.map((item, index) => (
          <article key={`${item.title}-${index}`} className="grid overflow-hidden border-2 border-[var(--style-text-primary)] bg-[var(--style-card-bg)] shadow-[4px_4px_0_var(--style-text-primary)] sm:grid-cols-[220px_1fr]">
            {item.image && <div className="relative min-h-56"><Image src={item.image} alt={item.title || ''} fill className="object-cover" sizes="260px" /></div>}
            <div className="p-5">
              <ExperienceMeta item={item} className="text-orange-500 font-black" />
              <h3 className="mt-3 text-2xl font-black uppercase text-[var(--style-text-primary)]">{item.title || ''}</h3>
              {item.text && <p className="mt-3 text-sm leading-6 text-[var(--style-text-secondary)]">{item.text}</p>}
