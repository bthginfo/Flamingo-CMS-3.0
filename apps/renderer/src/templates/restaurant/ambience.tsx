          ...
          {highlights.map((highlight, index) => (
            <motion.div key={`${highlight.title}-${index}`} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="flex gap-4 rounded-2xl bg-[var(--style-card-bg)] p-4 shadow-sm">
              ...
              <div>
                ...
                {highlight.text && <p className="mt-1 text-sm leading-6 text-[var(--style-text-muted)]">{highlight.text}</p>}
              </div>
            </motion.div>
          ))}
        </div>
        {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-8 inline-flex items-center gap-2 rounded-full bg-[var(--style-brand)] px-6 py-3 font-semibold text-white shadow-md transition-shadow hover:shadow-lg">{ctaPrimary.label}<ArrowRight size={16} /></a>}
      </div>
    </motion.div>
  );
}
    ...
    <div className="grid gap-16 lg:grid-cols-2 lg:items-start">
      ...
      <div>
        ...
        <div className="mt-10 grid gap-6">
          {highlights.map((highlight, index) => (
            <div key={`${highlight.title}-${index}`} className="flex gap-4 border-l-2 border-[var(--style-accent)] pl-5">
              <div className="mt-0.5 shrink-0 text-[var(--style-text-muted)]"><DynamicIcon name={highlight.icon || 'star'} size={18} /></div>
              <div>
                <h3 className="font-medium text-[var(--style-text-primary)]">{highlight.title || ''}</h3>
                {highlight.text && <p className="mt-1 text-sm font-light leading-6 text-[var(--style-text-muted)]">{highlight.text}</p>}
              </div>
            </div>
          ))}
        </div>
        {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-10 inline-flex items-center gap-2 border-b-2 border-[var(--style-text-primary)] pb-1 font-medium text-[var(--style-text-primary)]">{ctaPrimary.label}<ArrowRight size={16} /></a>}
      </div>
    </div>
  );
}
    ...
    <div className="bg-[var(--style-text-primary)] p-6 text-white sm:p-10">
      <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
        ...
        <div>
          ...
          <div className="mt-8 grid gap-4">
            {highlights.map((highlight, index) => (
              <div key={`${highlight.title}-${index}`} className="flex gap-4 border-2 border-white/20 p-4 shadow-[4px_4px_0_rgba(255,255,255,0.15)]">
                <div className="mt-0.5 shrink-0 text-[var(--style-accent)]"><DynamicIcon name={highlight.icon || 'star'} size={20} /></div>
                <div>
          ...
          {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-8 inline-flex items-center gap-2 rounded-none border-2 border-white bg-white px-6 py-3 font-black uppercase text-[var(--style-text-primary)] shadow-[4px_4px_0_rgba(255,255,255,0.3)]">{ctaPrimary.label}<ArrowRight size={16} /></a>}
        </div>
      </div>
    </div>
  );
