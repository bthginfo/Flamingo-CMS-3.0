    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          {storyText && <div className="mt-6 whitespace-pre-line leading-7 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: storyText }} />}
          {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-8 inline-flex rounded-full bg-[var(--token-btn-bg,var(--brand-primary,#1a5276))] px-6 py-3 font-semibold text-[color:var(--token-on-dark-heading,#ffffff)] shadow-md">{ctaPrimary.label}</a>}
        </motion.div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {founderQuote && (
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="flex flex-col justify-center rounded-xl bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))/10] p-5">
              ...
              {founderRole && <p className="text-xs text-[color:var(--token-on-dark-muted,#71717a)]">{founderRole}</p>}
            </motion.div>
          )}
        </div>
      </div>
          ...
          {values.map((v, i) => (
            <motion.div key={`${v.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex gap-4 rounded-xl bg-[var(--token-card-bg,#ffffff)] p-5 shadow-sm">
              ...
              <div>
                ...
                {v.text && <div className="mt-1 text-sm leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: v.text }} />}
              </div>
            </motion.div>
          ))}
        </div>
          ...
          <div className="relative border-l-2 border-[var(--token-card-border,var(--brand-accent,#f39c12))/30] pl-8">
            {milestones.map((m, i) => (
              <motion.div key={`${m.year}-${i}`} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="relative mb-8 last:mb-0">
                ...
                {m.text && <div className="mt-1 text-sm leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: m.text }} />}
              </motion.div>
            ))}
          </div>
        </div>
    ...
    <div>
      <div className="grid gap-16 lg:grid-cols-2 lg:items-start">
        <div>
          ...
          {storyText && <div className="mt-6 whitespace-pre-line font-light leading-7 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: storyText }} />}
          {founderQuote && (
            <blockquote className="mt-8 border-l-2 border-[var(--token-card-border,var(--brand-accent,#f39c12))] pl-5">
              ...
              {founderRole && <p className="text-xs font-light text-[color:var(--token-on-dark-muted,#71717a)]">{founderRole}</p>}
            </blockquote>
          )}
        ...
        <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {values.map((v, i) => (
            <div key={`${v.title}-${i}`} className="border-l-2 border-[var(--token-card-border,var(--brand-accent,#f39c12))] pl-5">
              <DynamicIcon name={v.icon || 'heart'} size={18} className="text-[color:var(--token-on-dark-muted,#71717a)]" />
              <h3 className="mt-2 font-medium text-[color:var(--token-heading,#18181b)]">{v.title || ''}</h3>
              {v.text && <div className="mt-1 text-sm font-light leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: v.text }} />}
            </div>
          ))}
        </div>
      )}
      {milestones.length > 0 && (
        <div className="mt-16 grid gap-6 border-t border-black/10 pt-10 sm:grid-cols-2 lg:grid-cols-4">
          {milestones.map((m, i) => (
            <div key={`${m.year}-${i}`}>
              {m.year && <p className="text-xs font-light uppercase tracking-[0.2em] text-[color:var(--token-on-dark-muted,#71717a)]">{m.year}</p>}
              <h3 className="mt-1 font-medium text-[color:var(--token-heading,#18181b)]">{m.title || ''}</h3>
              {m.text && <div className="mt-1 text-sm font-light leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: m.text }} />}
            </div>
          ))}
        </div>
      )}
    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          {subline && <div className="mt-4 font-bold text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: subline }} />}
          {storyText && <div className="mt-6 whitespace-pre-line leading-7 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: storyText }} />}
          {founderQuote && (
            <div className="mt-8 border-l-4 border-[var(--token-card-border,var(--brand-accent,#f39c12))] bg-[var(--token-section-bg-alt,#000000)/5] p-5">
              ...
              {founderRole && <p className="text-xs font-bold text-[color:var(--token-on-dark-muted,#71717a)]">{founderRole}</p>}
            </div>
          )}
