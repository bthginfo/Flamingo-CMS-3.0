    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {founderQuote && (
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="flex flex-col justify-center rounded-xl bg-[var(--token-badge-bg)/10] p-5">
      ...
      {values.length > 0 && (
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {values.map((v, i) => (
            <motion.div key={`${v.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex gap-4 rounded-xl bg-[var(--token-card-bg)] p-5 shadow-sm" data-edit-collection="values" data-edit-index={i}>
      ...
      {milestones.length > 0 && (
        <div className="mt-16">
          <div className="relative border-l-2 border-[var(--token-card-border)/30] pl-8">
            {milestones.map((m, i) => (
    ...
    <div>
      ...
      {values.length > 0 && (
        <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {values.map((v, i) => (
            <div key={`${v.title}-${i}`} className="border-l-2 border-[var(--token-card-border)] pl-5" data-edit-collection="values" data-edit-index={i}>
    ...
    <div>
      ...
      {values.length > 0 && (
        <div className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {values.map((v, i) => (
            <div key={`${v.title}-${i}`} className="border-2 border-[#111827] p-5 shadow-[4px_4px_0_var(--token-eyebrow)]" data-edit-collection="values" data-edit-index={i}>
