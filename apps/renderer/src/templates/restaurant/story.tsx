    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-8 inline-flex rounded-full bg-[var(--token-btn-bg,var(--brand-primary,#1a5276))] px-6 py-3 font-semibold text-white shadow-md">{ctaPrimary.label}</a>}
        </motion.div>
          ...
          <div className="relative border-l-2 border-[var(--token-card-border,var(--brand-accent,#f39c12))/30] pl-8">
            {milestones.map((m, i) => (
              <motion.div key={`${m.year}-${i}`} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="relative mb-8 last:mb-0">
                <div className="absolute -left-[2.55rem] top-1 h-4 w-4 rounded-full border-2 border-[var(--token-card-border,var(--brand-accent,#f39c12))] bg-white" />
    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          {storyText && <div className="mt-6 whitespace-pre-line leading-7 text-gray-500 rt-content" dangerouslySetInnerHTML={{ __html: storyText }} />}
          {founderQuote && (
            <div className="mt-8 border-l-4 border-[var(--token-card-border,var(--brand-accent,#f39c12))] bg-black/5 p-5">
