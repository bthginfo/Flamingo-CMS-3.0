  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((img, i) => (
          <motion.figure key={`${img.src}-${i}`} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="group overflow-hidden rounded-xl border border-black/10 bg-white shadow-md">
            ...
            <figcaption className="p-4">
              {img.category && <p className="text-xs uppercase tracking-widest text-brand-accent">{img.category}</p>}
              {img.caption && <p className="mt-1 text-sm text-gray-900">{img.caption}</p>}
            </figcaption>
          </motion.figure>
        ))}
      </div>
      {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-8 inline-flex items-center gap-2 rounded-full bg-brand-primary px-6 py-3 font-semibold text-white shadow-md">{ctaPrimary.label}<ArrowRight size={16} /></a>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-px border border-black/5 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((img, i) => (
          <figure key={`${img.src}-${i}`} className="group overflow-hidden border border-black/5 bg-white">
            {img.src && <div className="relative aspect-[4/3]"><Image src={img.src} alt={img.alt || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <figcaption className="p-5">
              {img.category && <p className="text-xs font-light uppercase tracking-[0.2em] text-gray-500">{img.category}</p>}
              {img.caption && <p className="mt-1 text-sm font-light text-gray-900">{img.caption}</p>}
            </figcaption>
          </figure>
        ))}
      </div>
      {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-10 inline-flex items-center gap-2 border-b-2 border-[#111827] pb-1 font-medium text-gray-900">{ctaPrimary.label}<ArrowRight size={14} /></a>}
    </div>
  );
}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((img, i) => (
          <figure key={`${img.src}-${i}`} className="group overflow-hidden border-2 border-white/20 shadow-[4px_4px_0_rgba(255,255,255,0.15)]">
            ...
            <figcaption className="p-4">
              {img.category && <p className="text-xs font-bold uppercase tracking-widest text-brand-accent">{img.category}</p>}
              {img.caption && <p className="mt-1 text-sm font-bold">{img.caption}</p>}
            </figcaption>
          </figure>
        ))}
      </div>
      {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="mt-8 inline-flex items-center gap-2 border-2 border-white bg-white px-6 py-3 font-black uppercase text-gray-900 shadow-[4px_4px_0_rgba(255,255,255,0.3)]">{ctaPrimary.label}<ArrowRight size={16} /></a>}
    </div>
  );
}
