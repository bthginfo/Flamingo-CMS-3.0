    ...
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {events.map((event, index) => (
          <motion.article key={`${event.title}-${index}`} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }} className="grid overflow-hidden rounded-xl border border-black/10 bg-white shadow-md sm:grid-cols-[180px_1fr]">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        {events.map((event, index) => (
          <article key={`${event.title}-${index}`} className="grid border border-black/5 sm:grid-cols-[160px_1fr]">
            ...
            <div className="p-6">
              <div className="flex flex-wrap gap-4 text-xs font-light uppercase tracking-[0.15em] text-gray-500">
