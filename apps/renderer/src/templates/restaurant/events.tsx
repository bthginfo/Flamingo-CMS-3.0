...
function EventsClassic({ headline, subline, badgeText, events, fallbackText }: EventsViewProps) {
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
      ...
      {events.length === 0 && fallbackText ? <p className="text-center text-[color:var(--token-on-dark-muted,#71717a)]">{fallbackText}</p> : null}
      <div className="grid gap-6 lg:grid-cols-2">
        {events.map((event, index) => (
          <motion.article key={`${event.title}-${index}`} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }} className="grid overflow-hidden rounded-xl border border-black/10 bg-[var(--token-card-bg,#ffffff)] shadow-md sm:grid-cols-[180px_1fr]">
            ...
            <div className="p-5">
              <div className="flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-widest text-[color:var(--token-on-dark-muted,#71717a)]">
                ...
                {event.priceLabel && <span className="rounded-full bg-[var(--token-section-bg-alt,#000000)/5] px-3 py-1">{event.priceLabel}</span>}
              </div>
              <h3 className="mt-3 text-xl font-bold text-[color:var(--token-heading,#18181b)]">{event.title || ''}</h3>
              {event.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: event.description }} />}
              <div className="mt-5 flex flex-wrap gap-4">
                {event.cta?.label && <a href={event.cta.href || '#'} className="inline-flex items-center gap-2 rounded-full bg-[var(--token-btn-bg,var(--brand-primary,#1a5276))] px-4 py-2 text-sm font-semibold text-[color:var(--token-on-dark-heading,#ffffff)]">{event.cta.label}</a>}
                {event.detailHref && event.detailLabel && <a href={event.detailHref} className="inline-flex items-center gap-1 text-sm text-[color:var(--token-on-dark-muted,#71717a)]">{event.detailLabel}<ArrowRight size={14} /></a>}
              </div>
            </div>
          </motion.article>
        ))}
...
function EventsModern({ headline, subline, badgeText, events, fallbackText }: EventsViewProps) {
  return (
    <div>
      ...
      {events.length === 0 && fallbackText ? <p className="font-light text-[color:var(--token-on-dark-muted,#71717a)]">{fallbackText}</p> : null}
      <div className="grid gap-8 lg:grid-cols-2">
        {events.map((event, index) => (
          <article key={`${event.title}-${index}`} className="grid border border-black/5 sm:grid-cols-[160px_1fr]">
            ...
            <div className="p-6">
              <div className="flex flex-wrap gap-4 text-xs font-light uppercase tracking-[0.15em] text-[color:var(--token-on-dark-muted,#71717a)]">
              ...
              {event.description && <div className="mt-3 text-sm font-light leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: event.description }} />}
              <div className="mt-5 flex flex-wrap gap-4">
                {event.cta?.label && <a href={event.cta.href || '#'} className="border-b border-[#111827] pb-0.5 text-sm font-medium text-[color:var(--token-heading,#18181b)]">{event.cta.label}</a>}
                {event.detailHref && event.detailLabel && <a href={event.detailHref} className="inline-flex items-center gap-1 text-sm font-light text-[color:var(--token-on-dark-muted,#71717a)]">{event.detailLabel}<ArrowRight size={14} /></a>}
              </div>
            </div>
          </article>
        ))}
  ...
  return (
    <div className="bg-[#111827] p-6 text-[color:var(--token-on-dark-heading,#ffffff)] sm:p-10">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {events.map((event, index) => (
          <article key={`${event.title}-${index}`} className="grid overflow-hidden rounded-none border-2 border-[color:var(--token-card-border,#ffffff)/20] shadow-[4px_4px_0_rgba(255,255,255,0.15)] sm:grid-cols-[160px_1fr]">
            {event.image && <div className="relative min-h-48"><Image src={event.image} alt={event.title || ''} fill className="object-cover" sizes="200px" /></div>}
            <div className="p-5">
              ...
              <div className="mt-5 flex flex-wrap gap-4">
                {event.cta?.label && <a href={event.cta.href || '#'} className="rounded-none border-2 border-[color:var(--token-card-border,#ffffff)] bg-[var(--token-card-bg,#ffffff)] px-4 py-2 text-sm font-bold uppercase text-[color:var(--token-heading,#18181b)]">{event.cta.label}</a>}
