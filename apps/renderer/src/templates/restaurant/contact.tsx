    ...
    <div className="grid gap-3">
      {cards.map((card, i) => (
        <div key={`${card.label}-${i}`} className="flex gap-4 border-t border-[var(--token-icon, var(--brand-primary))]/20 pt-4" data-edit-collection="cards" data-edit-index={i}>
          ...
          <div>
            <p className="text-xs text-[color:var(--token-muted,#52525b)]" data-edit-path="label">{card.label || ''}</p>
            <p className="font-semibold text-[color:var(--token-heading,#18181b)]" data-edit-path="value">{card.value || ''}</p>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        {p.introText && <p className="text-[color:var(--token-muted,#52525b)]">{plain(p.introText)}</p>}
        <div className="mt-6"><InfoCards cards={p.infoCards} /></div>
        <div className="mt-8 flex flex-wrap gap-3">
          {p.primaryCta.label && <a href={p.primaryCta.href || '#'} className="inline-flex rounded-full bg-[#111827] px-5 py-3 font-semibold text-[color:var(--token-on-dark-heading,#ffffff)] shadow-md" data-edit-path="label">{p.primaryCta.label}</a>}
          {p.secondaryCta.label && <a href={p.secondaryCta.href || '#'} className="inline-flex rounded-full border border-[var(--token-icon, var(--brand-primary))]/30 px-5 py-3 font-semibold text-[color:var(--token-heading,#18181b)]" data-edit-path="label">{p.secondaryCta.label}</a>}
        </div>
      </div>
  ...
  return (
    <div className="grid gap-16 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-6">{p.infoCards.map((c, i) => <div key={`${c.label}-${i}`} className="flex gap-4 border-t border-black/10 pt-4" data-edit-collection="infoCards" data-edit-index={i}><DynamicIcon name={c.icon || 'mail'} size={18} className="shrink-0 text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]" /><div className="min-w-0 flex-1"><p className="text-xs font-light text-[color:var(--token-muted,#52525b)]" data-edit-path="label">{c.label || ''}</p><p className="break-words font-light text-[color:var(--token-heading,#18181b)]" data-edit-path="value">{c.value || ''}</p></div></div>)}</div>
        <div className="mt-8 flex flex-wrap gap-3">
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-6">{p.infoCards.map((c, i) => <div key={`${c.label}-${i}`} className="flex gap-4 border-t-2 border-[#111827] pt-4" data-edit-collection="infoCards" data-edit-index={i}><DynamicIcon name={c.icon || 'mail'} size={20} className="shrink-0 text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]" /><div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase text-[color:var(--token-muted,#52525b)]" data-edit-path="label">{c.label || ''}</p><p className="break-words font-black text-[color:var(--token-heading,#18181b)]" data-edit-path="value">{c.value || ''}</p></div></div>)}</div>
        <div className="mt-8 flex flex-wrap gap-3">
