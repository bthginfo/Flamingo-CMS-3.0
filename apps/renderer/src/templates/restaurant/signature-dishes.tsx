  ...
  return (
    <div>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <article key={`${dish.name || 'item'}-${index}`} className="group border border-black/5" data-edit-collection="dishes" data-edit-index={index}>
            ...
            <div className="p-6">
              ...
              <div className="mt-2 flex items-start justify-between gap-4">
                ...
                {dish.price && <p className="shrink-0 font-medium text-[color:var(--token-price)]" data-edit-path="price">{dish.price}</p>}
              </div>
              ...
              {dish.cta?.label && <a href={dish.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 border-b border-[var(--token-card-border)] pb-0.5 text-sm font-medium text-[color:var(--token-heading)]"><span data-edit-path="label">{dish.cta.label}</span><ArrowRight size={16} /></a>}
            </div>
          </article>
        ))}
      </div>
  ...
  return (
    <div className="bg-[var(--token-btn-bg)] p-6 text-[color:var(--token-on-dark-heading)] sm:p-10">
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <article key={`${dish.name || 'item'}-${index}`} className="group overflow-hidden rounded-none border-2 border-[color:color-mix(in_srgb,var(--token-card-border)_20%,transparent)] shadow-[4px_4px_0_rgba(255,255,255,0.15)]" data-edit-collection="dishes" data-edit-index={index}>
