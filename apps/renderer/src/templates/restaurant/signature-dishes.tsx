    ...
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <motion.article key={`${dish.name}-${index}`} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="group overflow-hidden rounded-xl border border-black/10 bg-[var(--token-card-bg,#ffffff)] shadow-md">
            ...
            <div className="p-5">
              {dish.label && <p className="inline-block rounded-full bg-[var(--token-badge-bg,var(--brand-accent,#f39c12))/10] px-3 py-1 text-xs font-bold uppercase tracking-widest text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]">{dish.label}</p>}
              <div className="mt-2 flex items-start justify-between gap-4">
                <h3 className="text-xl font-bold text-[color:var(--token-heading,#18181b)]">{dish.name || ''}</h3>
                {dish.price && <p className="shrink-0 font-bold text-[color:var(--token-heading,#18181b)]">{dish.price}</p>}
              </div>
              {dish.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: dish.description }} />}
              {asList<string>(dish.ingredients).length > 0 && <p className="mt-4 text-xs text-[color:var(--token-on-dark-muted,#71717a)]">{asList<string>(dish.ingredients).join(' / ')}</p>}
              {dish.cta?.label && <a href={dish.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 font-semibold text-[color:var(--token-icon,var(--brand-primary,#1a5276))]">{dish.cta.label}<ArrowRight size={16} /></a>}
            </div>
          </motion.article>
        ))}
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <article key={`${dish.name}-${index}`} className="group border border-black/5">
            ...
            <div className="p-6">
              {dish.label && <p className="text-xs font-light uppercase tracking-[0.15em] text-[color:var(--token-on-dark-muted,#71717a)]">{dish.label}</p>}
              <div className="mt-2 flex items-start justify-between gap-4">
                <h3 className="text-xl font-medium text-[color:var(--token-heading,#18181b)]">{dish.name || ''}</h3>
                {dish.price && <p className="shrink-0 font-medium text-[color:var(--token-heading,#18181b)]">{dish.price}</p>}
              </div>
              {dish.description && <div className="mt-3 text-sm font-light leading-6 text-[color:var(--token-on-dark-muted,#71717a)] rt-content" dangerouslySetInnerHTML={{ __html: dish.description }} />}
              {asList<string>(dish.ingredients).length > 0 && <p className="mt-4 text-xs font-light text-[color:var(--token-on-dark-muted,#71717a)]">{asList<string>(dish.ingredients).join(' / ')}</p>}
              {dish.cta?.label && <a href={dish.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 border-b border-[#111827] pb-0.5 text-sm font-medium text-[color:var(--token-heading,#18181b)]">{dish.cta.label}<ArrowRight size={16} /></a>}
            </div>
          </article>
        ))}
      </div>
  ...
  return (
    <div className="bg-[#111827] p-6 text-[color:var(--token-on-dark-heading,#ffffff)] sm:p-10">
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <article key={`${dish.name}-${index}`} className="group overflow-hidden rounded-none border-2 border-[color:var(--token-card-border,#ffffff)/20] shadow-[4px_4px_0_rgba(255,255,255,0.15)]">
            ...
            <div className="p-5">
              {dish.label && <p className="text-xs font-bold uppercase tracking-widest text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]">{dish.label}</p>}
              <div className="mt-2 flex items-start justify-between gap-4">
                <h3 className="text-xl font-bold uppercase">{dish.name || ''}</h3>
                {dish.price && <p className="shrink-0 font-bold text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]">{dish.price}</p>}
              </div>
              ...
              {dish.cta?.label && <a href={dish.cta.href || '#'} className="mt-5 inline-flex items-center gap-2 font-bold uppercase text-[color:var(--token-on-dark-heading,#ffffff)]">{dish.cta.label}<ArrowRight size={16} /></a>}
            </div>
          </article>
        ))}
      </div>
