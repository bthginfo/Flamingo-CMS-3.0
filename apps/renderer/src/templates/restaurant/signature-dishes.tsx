    ...
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <motion.article key={`${dish.name}-${index}`} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="group overflow-hidden rounded-xl border border-black/10 bg-[var(--token-card-bg)] shadow-md" data-edit-collection="dishes" data-edit-index={index}>
            ...
            <div className="p-5">
              {dish.label && <p className="inline-block rounded-full bg-[var(--token-badge-bg)/10] px-3 py-1 text-xs font-bold uppercase tracking-widest text-[color:var(--token-eyebrow)]" data-edit-path="label">{dish.label}</p>}
              ...
              {dish.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: dish.description }} />}
        ...
        {dishes.map((dish, index) => (
          <article key={`${dish.name}-${index}`} className="group border border-black/5" data-edit-collection="dishes" data-edit-index={index}>
            ...
            <div className="p-6">
              ...
              {dish.description && <div className="mt-3 text-sm font-light leading-6 text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: dish.description }} />}
  ...
  return (
    <div className="bg-[#111827] p-6 text-[color:var(--token-on-dark-heading)] sm:p-10">
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <article key={`${dish.name}-${index}`} className="group overflow-hidden rounded-none border-2 border-[color:var(--token-card-border)/20] shadow-[4px_4px_0_rgba(255,255,255,0.15)]" data-edit-collection="dishes" data-edit-index={index}>
            {dish.image && <div className="relative aspect-[4/3] overflow-hidden border-b-2 border-[color:var(--token-card-border)/20]"><Image src={dish.image} alt={dish.name || ''} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-5">
              ...
              {dish.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-heading)/60] rt-content" dangerouslySetInnerHTML={{ __html: dish.description }} />}
              {asList<string>(dish.ingredients).length > 0 && <p className="mt-4 text-xs text-[color:var(--token-on-dark-heading)/50]">{asList<string>(dish.ingredients).join(' / ')}</p>}
