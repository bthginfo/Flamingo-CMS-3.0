      ...
      <div className="grid gap-5 md:grid-cols-3">
        {dishes.map((dish, index) => (
          <article key={`$<span data-edit-path="name">{dish.name}</span>-${index}`} className="group overflow-hidden rounded-none border-2 border-[color:var(--token-card-border)/20] shadow-[4px_4px_0_rgba(255,255,255,0.15)]" data-edit-collection="dishes" data-edit-index={index}>
            {dish.image && <div className="relative aspect-[4/3] overflow-hidden border-b-2 border-[color:var(--token-card-border)/20]"><Image data-edit-image="image" src={dish.image} alt={dish.name || ''} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="33vw" /></div>}
            <div className="p-5">
              ...
              {dish.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-heading)/60] rt-content" data-edit-rich="description" dangerouslySetInnerHTML={{ __html: dish.description }} />}
