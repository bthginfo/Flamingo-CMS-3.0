...
function MenuClassic(props: MenuViewProps) {
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-[var(--style-card-radius)] border border-black/10 bg-[var(--style-card-bg)] shadow-[var(--style-card-shadow)] overflow-hidden">
...
function MenuBold(props: MenuViewProps) {
  return (
    <div className="bg-[var(--style-text-primary)] text-white p-6 sm:p-10 shadow-[var(--style-card-shadow)]">
...
function MenuHeader({ headline, subline, badgeText, introText, align, inverted }: MenuViewProps & { align: 'left' | 'center'; inverted?: boolean }) {
  return (
    <div className={`${align === 'center' ? 'text-center mx-auto' : ''} max-w-3xl p-6 sm:p-10`}>
      {badgeText && <p className={`text-xs font-bold uppercase tracking-widest ${inverted ? 'text-[var(--style-badge-text,#f8c15c)]' : 'text-[var(--style-text-secondary)]'}`}>{badgeText}</p>}
      <h2 className={`mt-3 text-3xl sm:text-5xl font-[var(--style-heading-weight)] ${inverted ? 'text-white' : 'text-[var(--style-text-primary)]'}`}>{headline}</h2>
      {subline && <p className={`mt-4 text-base sm:text-lg ${inverted ? 'text-white/70' : 'text-[var(--style-text-secondary)]'}`}>{subline}</p>}
      {introText && <p className={`mt-5 leading-7 ${inverted ? 'text-white/65' : 'text-[var(--style-text-secondary)]'}`}>{introText}</p>}
    </div>
  );
}
...
function MenuCategoryBlock({ category, layout }: { category: MenuCategory; layout: 'classic' | 'modern' | 'bold' }) {
  ...
  return (
    <div className={`${layout === 'classic' ? 'p-6 sm:p-10' : layout === 'bold' ? 'border-2 border-white/20 p-5' : 'border border-black/10 p-6'}`}>
      <div className="mb-6">
        <h3 className={`text-2xl font-bold ${layout === 'bold' ? 'text-white uppercase' : 'text-[var(--style-text-primary)]'}`}>{category.title || ''}</h3>
        {category.description && <p className={`mt-2 text-sm ${layout === 'bold' ? 'text-white/60' : 'text-[var(--style-text-secondary)]'}`}>{category.description}</p>}
      </div>
      <div className="space-y-5">
        {items.map((item, index) => (
          <article key={`${item.name}-${index}`} className={`grid gap-4 ${item.image ? 'sm:grid-cols-[96px_1fr]' : ''} ${item.highlighted ? 'bg-[var(--style-section-bg-alt)] p-4' : ''}`}>
            {item.image && (
              <div className="relative h-24 w-24 overflow-hidden rounded-[var(--style-radius-md)]">
                <Image src={item.image} alt={item.name || ''} fill className="object-cover" sizes="96px" />
              </div>
            )}
            <div>
              <div className="flex items-start justify-between gap-4">
                <h4 className={`font-semibold ${layout === 'bold' ? 'text-white' : 'text-[var(--style-text-primary)]'}`}>{item.name || ''}</h4>
                {item.price && <p className={`shrink-0 font-bold ${layout === 'bold' ? 'text-[var(--style-badge-text,#f8c15c)]' : 'text-[var(--style-text-primary)]'}`}>{item.price}</p>}
              </div>
              {item.description && <p className={`mt-1 text-sm leading-6 ${layout === 'bold' ? 'text-white/60' : 'text-[var(--style-text-secondary)]'}`}>{item.description}</p>}
...
function MenuFooter({ footnote, ctaPrimary, inverted }: MenuViewProps & { inverted?: boolean }) {
  ...
  return (
    <div className={`flex flex-col gap-4 p-6 sm:p-10 sm:flex-row sm:items-center sm:justify-between ${inverted ? 'text-white/70' : 'text-[var(--style-text-secondary)]'}`}>
      {footnote && <p className="text-sm">{footnote}</p>}
      {ctaPrimary.label && <a href={ctaPrimary.href || '#'} className="inline-flex items-center gap-2 font-semibold text-[var(--style-text-primary)] bg-[var(--style-badge-bg)] px-5 py-3 rounded-[var(--style-button-radius)]">{ctaPrimary.label}<ArrowRight size={16} /></a>}
    </div>
  );
}
