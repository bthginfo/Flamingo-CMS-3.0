            ...
            <AnimatePresence>
              {isOpen && item.answer && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }}>
                  <div className={`pb-5 text-sm leading-6 ${variant === 'modern' ? 'font-light text-[color:var(--token-on-dark-muted,#71717a)]' : 'text-[color:var(--token-on-dark-muted,#71717a)]'}`} dangerouslySetInnerHTML={{ __html: item.answer }} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
function FaqBold({ headline, subline, badgeText, items, ctaPrimary }: Props) {
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {ctaPrimary.label && <div className="mt-8"><a href={ctaPrimary.href || '#'} className="inline-flex border-2 border-[#111827] bg-[#111827] px-6 py-3 font-black uppercase text-[color:var(--token-on-dark-heading,#ffffff)] shadow-[4px_4px_0_var(--token-eyebrow, var(--brand-accent))]">{ctaPrimary.label}</a></div>}
    </div>
  );
}
