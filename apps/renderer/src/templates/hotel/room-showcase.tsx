    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <motion.article key={`${room.name}-${index}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className={`overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md ${room.highlighted ? 'ring-2 ring-[var(--token-icon, var(--brand-primary))]' : ''}`}>
            ...
            <div className="p-5">
              ...
              {room.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: room.description }} />}
              <div className="mt-4 grid gap-2 text-xs text-[color:var(--token-on-dark-muted,#52525b)]">
              ...
              {asList<string>(room.features).length > 0 && <p className="mt-4 text-xs text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(room.features).join(' / ')}</p>}
              {asList<string>(room.galleryImages).length > 0 && <p className="mt-2 text-xs text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(room.galleryImages).length} Bilder</p>}
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                {room.bookingCta?.label && <a href={room.bookingCta.href || '#'} className="font-semibold text-[color:var(--token-on-dark-muted,#52525b)]">{room.bookingCta.label}</a>}
              </div>
            </div>
          </motion.article>
        ))}
      </div>
      {footerText && <p className="mt-6 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">{footerText}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-px border border-black/10 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <article key={`${room.name}-${index}`} className={`overflow-hidden border border-black/10 bg-[var(--token-card-bg,#ffffff)] ${room.highlighted ? 'bg-[var(--token-section-bg-alt,#000000)]/[0.02]' : ''}`}>
            {room.image && <div className="relative aspect-[4/3]"><Image src={room.image} alt={room.name || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-6">
              <div className="flex items-start justify-between gap-3">
                <h3 className="text-lg font-light text-[color:var(--token-heading,#18181b)]">{room.name || ''}</h3>
                {room.priceLabel && <p className="text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">{room.priceLabel}</p>}
              </div>
              {room.description && <div className="mt-3 text-sm font-light leading-7 text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: room.description }} />}
              <div className="mt-4 grid gap-2 text-xs font-light text-[color:var(--token-on-dark-muted,#52525b)]">
                {room.sizeLabel && <span className="inline-flex items-center gap-2"><Maximize2 size={13} />{room.sizeLabel}</span>}
                {room.occupancyLabel && <span className="inline-flex items-center gap-2"><Users size={13} />{room.occupancyLabel}</span>}
                {room.bedLabel && <span className="inline-flex items-center gap-2"><BedDouble size={13} />{room.bedLabel}</span>}
              </div>
              {asList<string>(room.features).length > 0 && <p className="mt-4 text-xs font-light text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(room.features).join(' / ')}</p>}
              {asList<string>(room.galleryImages).length > 0 && <p className="mt-2 text-xs font-light text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(room.galleryImages).length} Bilder</p>}
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                {room.bookingCta?.label && <a href={room.bookingCta.href || '#'} className="font-light text-[color:var(--token-on-dark-muted,#52525b)] underline underline-offset-4">{room.bookingCta.label}</a>}
              </div>
            </div>
          </article>
        ))}
      </div>
      {footerText && <p className="mt-8 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">{footerText}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <article key={`${room.name}-${index}`} className={`overflow-hidden border-2 border-[#111827] bg-[var(--token-card-bg,#ffffff)] shadow-[4px_4px_0_#111827] ${room.highlighted ? 'ring-2 ring-[var(--token-icon, var(--brand-primary))]' : ''}`}>
            {room.image && <div className="relative aspect-[4/3]"><Image src={room.image} alt={room.name || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-5">
              <div className="flex items-start justify-between gap-3">
              ...
              {room.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: room.description }} />}
              <div className="mt-4 grid gap-2 text-xs font-bold text-[color:var(--token-on-dark-muted,#52525b)]">
                {room.sizeLabel && <span className="inline-flex items-center gap-2"><Maximize2 size={13} />{room.sizeLabel}</span>}
                {room.occupancyLabel && <span className="inline-flex items-center gap-2"><Users size={13} />{room.occupancyLabel}</span>}
                {room.bedLabel && <span className="inline-flex items-center gap-2"><BedDouble size={13} />{room.bedLabel}</span>}
              </div>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                {room.bookingCta?.label && <a href={room.bookingCta.href || '#'} className="font-bold uppercase text-[color:var(--token-on-dark-muted,#52525b)]">{room.bookingCta.label}</a>}
              </div>
            </div>
          </article>
        ))}
      </div>
      {footerText && <p className="mt-6 text-sm font-bold text-[color:var(--token-on-dark-muted,#52525b)]">{footerText}</p>}
    </div>
  );
}
