    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <motion.article key={`${room.name}-${index}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className={`overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md ${room.highlighted ? 'ring-2 ring-[var(--token-icon, var(--brand-primary))]' : ''}`} data-edit-collection="rooms" data-edit-index={index}>
            ...
            <div className="p-5">
              ...
              {asList<string>(room.features).length > 0 && <p className="mt-4 text-xs text-[color:var(--token-muted,#52525b)]">{asList<string>(room.features).join(' / ')}</p>}
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                {room.detailCta?.label && <a href={room.detailCta.href || '#'} className="inline-flex items-center gap-1 font-semibold text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]"><span data-edit-path="label">{room.detailCta.label}</span><ArrowRight size={15} /></a>}
      ...
      {footerText && <p className="mt-6 text-sm text-[color:var(--token-muted,#52525b)]">{footerText}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-px border border-black/10 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <article key={`${room.name}-${index}`} className={`overflow-hidden border border-black/10 bg-[var(--token-card-bg,#ffffff)] ${room.highlighted ? 'bg-[var(--token-section-bg-alt,#000000)]/[0.02]' : ''}`} data-edit-collection="rooms" data-edit-index={index}>
            {room.image && <div className="relative aspect-[4/3]"><Image src={room.image} alt={room.name || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-6">
              <div className="flex items-start justify-between gap-3">
              ...
              {asList<string>(room.features).length > 0 && <p className="mt-4 text-xs font-light text-[color:var(--token-muted,#52525b)]">{asList<string>(room.features).join(' / ')}</p>}
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                {room.detailCta?.label && <a href={room.detailCta.href || '#'} className="inline-flex items-center gap-1 font-light text-[color:var(--token-heading,#18181b)] underline underline-offset-4"><span data-edit-path="label">{room.detailCta.label}</span><ArrowRight size={14} /></a>}
      ...
      {footerText && <p className="mt-8 text-sm font-light text-[color:var(--token-muted,#52525b)]">{footerText}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <article key={`${room.name}-${index}`} className={`overflow-hidden border-2 border-[#111827] bg-[var(--token-card-bg,#ffffff)] shadow-[4px_4px_0_#111827] ${room.highlighted ? 'ring-2 ring-[var(--token-icon, var(--brand-primary))]' : ''}`} data-edit-collection="rooms" data-edit-index={index}>
            {room.image && <div className="relative aspect-[4/3]"><Image src={room.image} alt={room.name || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-5">
              <div className="flex items-start justify-between gap-3">
              ...
              {asList<string>(room.galleryImages).length > 0 && <p className="mt-2 text-xs text-[color:var(--token-muted,#52525b)]">{asList<string>(room.galleryImages).length} Bilder</p>}
              <div className="mt-5 flex flex-wrap gap-3">
                {room.detailCta?.label && <a href={room.detailCta.href || '#'} className="inline-flex items-center gap-1 font-black uppercase text-[color:var(--token-eyebrow,var(--brand-accent,#f39c12))]"><span data-edit-path="label">{room.detailCta.label}</span><ArrowRight size={15} /></a>}
      ...
      {footerText && <p className="mt-6 text-sm font-bold text-[color:var(--token-muted,#52525b)]">{footerText}</p>}
    </div>
  );
}
