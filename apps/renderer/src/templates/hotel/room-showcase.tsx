  ...
  return (
    <div>
      ...
      <div className="grid gap-px border border-black/10 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <article key={`${room.name}-${index}`} className={`overflow-hidden border border-black/10 bg-[var(--style-card-bg)] ${room.highlighted ? 'bg-black/[0.02]' : ''}`}>
            {room.image && <div className="relative aspect-[4/3]"><Image src={room.image} alt={room.name || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-6">
              <div className="flex items-start justify-between gap-3">
                <h3 className="text-lg font-light text-[var(--style-text-primary)]">{room.name || ''}</h3>
                {room.priceLabel && <p className="text-sm font-light text-[var(--style-text-secondary)]">{room.priceLabel}</p>}
              </div>
              {room.description && <p className="mt-3 text-sm font-light leading-7 text-[var(--style-text-secondary)]">{room.description}</p>}
              <div className="mt-4 grid gap-2 text-xs font-light text-[var(--style-text-secondary)]">
                {room.sizeLabel && <span className="inline-flex items-center gap-2"><Maximize2 size={13} />{room.sizeLabel}</span>}
                {room.occupancyLabel && <span className="inline-flex items-center gap-2"><Users size={13} />{room.occupancyLabel}</span>}
                {room.bedLabel && <span className="inline-flex items-center gap-2"><BedDouble size={13} />{room.bedLabel}</span>}
              </div>
              {asList<string>(room.features).length > 0 && <p className="mt-4 text-xs font-light text-[var(--style-text-secondary)]">{asList<string>(room.features).join(' / ')}</p>}
              {asList<string>(room.galleryImages).length > 0 && <p className="mt-2 text-xs font-light text-[var(--style-text-secondary)]">{asList<string>(room.galleryImages).length} Bilder</p>}
              <div className="mt-5 flex flex-wrap gap-3">
                {room.detailCta?.label && <a href={room.detailCta.href || '#'} className="inline-flex items-center gap-1 font-light text-[var(--style-text-primary)] underline underline-offset-4">{room.detailCta.label}<ArrowRight size={14} /></a>}
                {room.bookingCta?.label && <a href={room.bookingCta.href || '#'} className="font-light text-[var(--style-text-secondary)] underline underline-offset-4">{room.bookingCta.label}</a>}
              </div>
            </div>
          </article>
        ))}
      </div>
      {footerText && <p className="mt-8 text-sm font-light text-[var(--style-text-secondary)]">{footerText}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {rooms.map((room, index) => (
          <article key={`${room.name}-${index}`} className={`overflow-hidden border-2 border-[var(--style-text-primary)] bg-[var(--style-card-bg)] shadow-[4px_4px_0_var(--style-text-primary)] ${room.highlighted ? 'ring-2 ring-[var(--style-badge-bg)]' : ''}`}>
            {room.image && <div className="relative aspect-[4/3]"><Image src={room.image} alt={room.name || ''} fill className="object-cover" sizes="33vw" /></div>}
            <div className="p-5">
              <div className="flex items-start justify-between gap-3">
      ...
      {footerText && <p className="mt-6 text-sm font-bold text-[var(--style-text-secondary)]">{footerText}</p>}
    </div>
  );
}
