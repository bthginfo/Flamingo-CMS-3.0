...
function BookingClassic({ headline, subline, badgeText, submitCta, secondaryCta, bookingNote, trustItems }: Props) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] p-6 shadow-lg text-center">
      {badgeText && <p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[color:var(--token-on-dark-muted,#52525b)]"><Star size={12} className="text-[color:var(--token-icon,var(--brand-primary,#1a5276))]" />{badgeText}</p>}
      <h2 className="mt-3 text-2xl font-[700] text-[color:var(--token-heading,#18181b)] sm:text-3xl">{headline}</h2>
      {subline && <div className="mx-auto mt-2 max-w-lg text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: subline }} />}
      {trustItems.length > 0 && (
        <div className="mt-5 flex flex-wrap items-center justify-center gap-4">
          {trustItems.map((item, i) => (
            <span key={i} className="inline-flex items-center gap-2 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">
      ...
      {bookingNote && <p className="mt-4 text-sm text-[color:var(--token-on-dark-muted,#71717a)]">{bookingNote}</p>}
    </motion.div>
  );
}
...
function BookingModern({ headline, subline, badgeText, submitCta, secondaryCta, bookingNote, trustItems }: Props) {
  return (
    <div className="border border-black/10 bg-[var(--token-card-bg,#ffffff)] p-8 text-center">
      ...
      {subline && <div className="mx-auto mt-2 max-w-lg text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: subline }} />}
      {trustItems.length > 0 && (
        <div className="mt-5 flex flex-wrap items-center justify-center gap-4">
          {trustItems.map((item, i) => (
            <span key={i} className="inline-flex items-center gap-2 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)]">
              <DynamicIcon name={item.icon || 'check'} size={16} className="text-[color:var(--token-icon,var(--brand-primary,#1a5276))]" />
              {plain(item.text)}
            </span>
          ))}
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-4">
        ...
        {secondaryCta.label && <a href={secondaryCta.href || '#'} target="_blank" rel="noopener noreferrer" className="font-light text-[color:var(--token-on-dark-muted,#52525b)] underline underline-offset-4">{secondaryCta.label}</a>}
      </div>
      {bookingNote && <p className="mt-4 text-sm font-light text-[color:var(--token-on-dark-muted,#71717a)]">{bookingNote}</p>}
    </div>
  );
}
...
function BookingBold({ headline, subline, badgeText, submitCta, secondaryCta, bookingNote, trustItems }: Props) {
  return (
    <div className="border-2 border-[#111827] bg-[var(--token-card-bg,#ffffff)] p-6 shadow-[4px_4px_0_#111827] text-center">
      ...
      {subline && <div className="mx-auto mt-2 max-w-lg text-sm text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: subline }} />}
      {trustItems.length > 0 && (
        <div className="mt-5 flex flex-wrap items-center justify-center gap-4">
          {trustItems.map((item, i) => (
            <span key={i} className="inline-flex items-center gap-2 text-sm font-bold text-[color:var(--token-on-dark-muted,#3f3f46)]">
      ...
      {bookingNote && <p className="mt-4 text-sm font-bold text-[color:var(--token-on-dark-muted,#71717a)]">{bookingNote}</p>}
    </div>
  );
}
