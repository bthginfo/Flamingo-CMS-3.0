    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <motion.figure key={`${image.src}-${index}`} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.06 }} className="group overflow-hidden rounded-xl border border-[var(--brand-primary)]/20 bg-white shadow-md">
            ...
            <figcaption className="p-4">
              {image.category && <p className="text-xs uppercase tracking-widest text-brand-primary">{image.category}</p>}
              {image.caption && <p className="mt-1 text-sm text-gray-900">{image.caption}</p>}
            </figcaption>
          </motion.figure>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div className="grid gap-px border border-black/10 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <figure key={`${image.src}-${index}`} className="group overflow-hidden border border-black/10 bg-white">
            {image.src && <div className="relative aspect-[4/3]"><Image src={image.src} alt={image.alt || ''} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" /></div>}
            <figcaption className="p-5">
              {image.category && <p className="text-xs font-light uppercase tracking-[0.3em] text-gray-600">{image.category}</p>}
              {image.caption && <p className="mt-1 text-sm font-light text-gray-900">{image.caption}</p>}
            </figcaption>
          </figure>
        ))}
      </div>
...
function GalleryBold({ headline, subline, badgeText, images, ctaPrimary }: Props) {
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((image, index) => (
          <figure key={`${image.src}-${index}`} className="group overflow-hidden border-2 border-[#111827] bg-white shadow-[4px_4px_0_#111827]">
            {image.src && <div className="relative aspect-[4/3]"><Image src={image.src} alt={image.alt || ''} fill className="object-cover" sizes="33vw" /></div>}
            <figcaption className="p-4">
              {image.category && <p className="text-xs font-bold uppercase tracking-widest text-brand-primary">{image.category}</p>}
              {image.caption && <p className="mt-1 text-sm font-bold text-gray-900">{image.caption}</p>}
            </figcaption>
          </figure>
        ))}
      </div>
