...
function OffersClassic({ headline, subline, badgeText, offers, fallbackText }: Props) {
  return (
    <div>
      ...
      {offers.length === 0 && fallbackText ? <p className="text-[color:var(--token-on-dark-muted,#52525b)]">{fallbackText}</p> : null}
      <div className="grid gap-6 md:grid-cols-2">
        {offers.map((offer, index) => (
          <motion.article key={`${offer.title}-${index}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className={`grid overflow-hidden rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] shadow-md sm:grid-cols-[180px_1fr] ${offer.highlighted ? 'ring-2 ring-[var(--token-icon, var(--brand-primary))]' : ''}`}>
            ...
            <div className="p-5">
              <div className="flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-widest text-[color:var(--token-on-dark-muted,#52525b)]">
              ...
              {offer.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: offer.description }} />}
              {asList<string>(offer.includes).length > 0 && <p className="mt-3 text-xs text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(offer.includes).join(' / ')}</p>}
              <div className="mt-5 flex flex-wrap gap-4">
                ...
                {offer.detailHref && offer.detailLabel && <a href={offer.detailHref} className="inline-flex items-center gap-1 text-sm text-[color:var(--token-on-dark-muted,#52525b)]">{offer.detailLabel}<ArrowRight size={14} /></a>}
              </div>
            </div>
          </motion.article>
        ))}
...
function OffersModern({ headline, subline, badgeText, offers, fallbackText }: Props) {
  return (
    <div>
      ...
      {offers.length === 0 && fallbackText ? <p className="font-light text-[color:var(--token-on-dark-muted,#52525b)]">{fallbackText}</p> : null}
      <div className="grid gap-px border border-black/10 md:grid-cols-2">
        {offers.map((offer, index) => (
          <article key={`${offer.title}-${index}`} className={`grid overflow-hidden border border-black/10 bg-[var(--token-card-bg,#ffffff)] sm:grid-cols-[180px_1fr] ${offer.highlighted ? 'bg-[var(--token-section-bg-alt,#000000)]/[0.02]' : ''}`}>
            {offer.image && <div className="relative min-h-52"><Image src={offer.image} alt={offer.title || ''} fill className="object-cover" sizes="240px" /></div>}
            <div className="p-6">
              <div className="flex flex-wrap gap-3 text-xs font-light uppercase tracking-[0.3em] text-[color:var(--token-on-dark-muted,#52525b)]">
                {offer.durationLabel && <span>{offer.durationLabel}</span>}
                {offer.priceLabel && <span>{offer.priceLabel}</span>}
                {offer.validUntilLabel && <span>{offer.validUntilLabel}</span>}
              </div>
              <h3 className="mt-3 text-xl font-light text-[color:var(--token-heading,#18181b)]">{offer.title || ''}</h3>
              {offer.description && <div className="mt-3 text-sm font-light leading-7 text-[color:var(--token-on-dark-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: offer.description }} />}
              {asList<string>(offer.includes).length > 0 && <p className="mt-3 text-xs font-light text-[color:var(--token-on-dark-muted,#52525b)]">{asList<string>(offer.includes).join(' / ')}</p>}
              <div className="mt-5 flex flex-wrap gap-4">
                ...
                {offer.detailHref && offer.detailLabel && <a href={offer.detailHref} className="inline-flex items-center gap-1 text-sm font-light text-[color:var(--token-on-dark-muted,#52525b)] underline underline-offset-4">{offer.detailLabel}<ArrowRight size={14} /></a>}
              </div>
            </div>
          </article>
        ))}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {offers.map((offer, index) => (
          <article key={`${offer.title}-${index}`} className={`grid overflow-hidden border-2 border-[#111827] bg-[var(--token-card-bg,#ffffff)] shadow-[4px_4px_0_#111827] sm:grid-cols-[180px_1fr] ${offer.highlighted ? 'ring-2 ring-[var(--token-icon, var(--brand-primary))]' : ''}`}>
            {offer.image && <div className="relative min-h-52"><Image src={offer.image} alt={offer.title || ''} fill className="object-cover" sizes="240px" /></div>}
            <div className="p-5">
              <div className="flex flex-wrap gap-3 text-xs font-bold uppercase tracking-widest text-[color:var(--token-on-dark-muted,#52525b)]">
                {offer.durationLabel && <span>{offer.durationLabel}</span>}
                {offer.priceLabel && <span>{offer.priceLabel}</span>}
                {offer.validUntilLabel && <span>{offer.validUntilLabel}</span>}
              </div>
              ...
              <div className="mt-5 flex flex-wrap gap-4">
                ...
                {offer.detailHref && offer.detailLabel && <a href={offer.detailHref} className="inline-flex items-center gap-1 text-sm font-bold text-[color:var(--token-on-dark-muted,#52525b)]">{offer.detailLabel}<ArrowRight size={14} /></a>}
              </div>
            </div>
          </article>
        ))}
