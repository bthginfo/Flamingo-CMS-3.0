        ...
        {offers.map((offer, index) => (
          <motion.article key={`${offer.title}-${index}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className={`grid overflow-hidden rounded-xl border border-[var(--token-icon)]/20 bg-[var(--token-card-bg)] shadow-md sm:grid-cols-[180px_1fr] ${offer.highlighted ? 'ring-2 ring-[var(--token-icon)]' : ''}`} data-edit-collection="offers" data-edit-index={index}>
            ...
            <div className="p-5">
              ...
              {offer.description && <div className="mt-3 text-sm leading-6 text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: offer.description }} />}
  ...
  return (
    <div>
      ...
      <div className="grid gap-px border border-black/10 md:grid-cols-2">
        {offers.map((offer, index) => (
          <article key={`${offer.title}-${index}`} className={`grid overflow-hidden border border-black/10 bg-[var(--token-card-bg)] sm:grid-cols-[180px_1fr] ${offer.highlighted ? 'bg-[var(--token-section-bg-alt)]/[0.02]' : ''}`} data-edit-collection="offers" data-edit-index={index}>
            {offer.image && <div className="relative min-h-52"><Image src={offer.image} alt={offer.title || ''} fill className="object-cover" sizes="240px" /></div>}
            <div className="p-6">
              ...
              {offer.description && <div className="mt-3 text-sm font-light leading-7 text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: offer.description }} />}
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {offers.map((offer, index) => (
          <article key={`${offer.title}-${index}`} className={`grid overflow-hidden border-2 border-[#111827] bg-[var(--token-card-bg)] shadow-[4px_4px_0_#111827] sm:grid-cols-[180px_1fr] ${offer.highlighted ? 'ring-2 ring-[var(--token-icon)]' : ''}`} data-edit-collection="offers" data-edit-index={index}>
            {offer.image && <div className="relative min-h-52"><Image src={offer.image} alt={offer.title || ''} fill className="object-cover" sizes="240px" /></div>}
            <div className="p-5">
