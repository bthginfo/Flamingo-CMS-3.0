      ...
      <div>
        ...
        <div className="mt-6 grid gap-3">
          {transportItems.map((item, index) => (
            <motion.div key={`${item.label}-${index}`} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="flex gap-4 border-t border-[var(--token-icon, var(--brand-primary))]/20 pt-4" data-edit-collection="transportItems" data-edit-index={index}>
              <div className="text-[color:var(--token-icon,var(--brand-primary,#1a5276))]"><DynamicIcon name={item.icon || 'map-pin'} size={20} /></div>
              <div><h3 className="font-semibold text-[color:var(--token-heading,#18181b)]"><span data-edit-path="label">{item.label || ''}</span> <span data-edit-path="value">{item.value || ''}</span></h3>{item.text && <div className="text-sm text-[color:var(--token-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}</div>
            </motion.div>
          ))}
        </div>
      ...
      <div className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {nearbyItems.map((item, index) => (
            <motion.article key={`${item.title}-${index}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-[var(--token-card-bg,#ffffff)] p-4 shadow-md" data-edit-collection="nearbyItems" data-edit-index={index}>
    ...
    <div className="grid gap-12 lg:grid-cols-2">
        ...
        <div className="mt-8 grid gap-4">
          {transportItems.map((item, index) => (
            <div key={`${item.label}-${index}`} className="flex gap-4 border-t border-black/10 pt-4" data-edit-collection="transportItems" data-edit-index={index}>
              ...
              <div><h3 className="font-light text-[color:var(--token-heading,#18181b)]"><span data-edit-path="label">{item.label || ''}</span> <span data-edit-path="value">{item.value || ''}</span></h3>{item.text && <div className="text-sm font-light text-[color:var(--token-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}</div>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-5">
        ...
        <div className="grid gap-px border border-black/10 sm:grid-cols-2">
          {nearbyItems.map((item, index) => (
            <article key={`${item.title}-${index}`} className="border border-black/10 bg-[var(--token-card-bg,#ffffff)] p-5" data-edit-collection="nearbyItems" data-edit-index={index}>
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
        ...
        <div className="mt-6 grid gap-3">
          {transportItems.map((item, index) => (
            <div key={`${item.label}-${index}`} className="flex gap-4 border-t-2 border-[#111827] pt-4" data-edit-collection="transportItems" data-edit-index={index}>
              <div className="text-[color:var(--token-icon,var(--brand-primary,#1a5276))]"><DynamicIcon name={item.icon || 'map-pin'} size={20} /></div>
              <div><h3 className="font-black text-[color:var(--token-heading,#18181b)]"><span data-edit-path="label">{item.label || ''}</span> <span data-edit-path="value">{item.value || ''}</span></h3>{item.text && <div className="text-sm text-[color:var(--token-muted,#52525b)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}</div>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {nearbyItems.map((item, index) => (
            <article key={`${item.title}-${index}`} className="border-2 border-[#111827] bg-[var(--token-card-bg,#ffffff)] p-4 shadow-[4px_4px_0_#111827]" data-edit-collection="nearbyItems" data-edit-index={index}>
