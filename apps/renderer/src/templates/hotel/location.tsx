    ...
    <div className="grid gap-12 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-8 grid gap-4">
          {transportItems.map((item, index) => (
            <div key={`${item.label || 'item'}-${index}`} className="flex gap-4 border-t border-black/10 pt-4" data-edit-collection="transportItems" data-edit-index={index}>
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-6 grid gap-3">
          {transportItems.map((item, index) => (
            <div key={`${item.label || 'item'}-${index}`} className="flex gap-4 border-t-2 border-[var(--token-card-border)] pt-4" data-edit-collection="transportItems" data-edit-index={index}>
              <div className="text-[color:var(--token-icon)]"><DynamicIcon editPath="icon" name={item.icon || 'map-pin'} size={20} /></div>
