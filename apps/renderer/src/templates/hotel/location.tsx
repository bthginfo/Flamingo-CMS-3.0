      ...
      <div>
        ...
        <div className="mt-6 grid gap-3">
          {transportItems.map((item, index) => (
            <motion.div key={`${item.label}-${index}`} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="flex gap-4 border-t border-[var(--token-icon)]/20 pt-4" data-edit-collection="transportItems" data-edit-index={index}>
              <div className="text-[color:var(--token-icon)]"><DynamicIcon name={item.icon || 'map-pin'} size={20} /></div>
              <div><h3 className="font-semibold text-[color:var(--token-heading)]"><span data-edit-path="label">{item.label || ''}</span> <span data-edit-path="value">{item.value || ''}</span></h3>{item.text && <div className="text-sm text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}</div>
            </motion.div>
          ))}
        </div>
        {routeCta.label && <a href={routeCta.href || '#'} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-[#111827] px-5 py-3 font-semibold text-[color:var(--token-on-dark-heading)] shadow-lg"><span data-edit-path="label">{routeCta.label}</span><ArrowRight size={16} /></a>}
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {nearbyItems.map((item, index) => (
            <motion.article key={`${item.title}-${index}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="rounded-xl border border-[var(--token-icon)]/20 bg-[var(--token-card-bg)] p-4 shadow-md" data-edit-collection="nearbyItems" data-edit-index={index}>
              ...
              {item.text && <div className="mt-2 text-sm text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
            </motion.article>
          ))}
        </div>
      </div>
    ...
    <div className="grid gap-12 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-8 grid gap-4">
          {transportItems.map((item, index) => (
            <div key={`${item.label}-${index}`} className="flex gap-4 border-t border-black/10 pt-4" data-edit-collection="transportItems" data-edit-index={index}>
              <DynamicIcon name={item.icon || 'map-pin'} size={18} className="text-[color:var(--token-muted)]" />
              <div><h3 className="font-light text-[color:var(--token-heading)]"><span data-edit-path="label">{item.label || ''}</span> <span data-edit-path="value">{item.value || ''}</span></h3>{item.text && <div className="text-sm font-light text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}</div>
            </div>
          ))}
        </div>
        {routeCta.label && <a href={routeCta.href || '#'} className="mt-10 inline-flex items-center gap-2 font-light text-[color:var(--token-heading)] underline underline-offset-4"><span data-edit-path="label">{routeCta.label}</span><ArrowRight size={14} /></a>}
      </div>
      <div className="space-y-5">
        ...
        <div className="grid gap-px border border-black/10 sm:grid-cols-2">
          {nearbyItems.map((item, index) => (
            <article key={`${item.title}-${index}`} className="border border-black/10 bg-[var(--token-card-bg)] p-5" data-edit-collection="nearbyItems" data-edit-index={index}>
              ...
              {item.text && <div className="mt-2 text-sm font-light text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}
            </article>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-6 grid gap-3">
          {transportItems.map((item, index) => (
            <div key={`${item.label}-${index}`} className="flex gap-4 border-t-2 border-[#111827] pt-4" data-edit-collection="transportItems" data-edit-index={index}>
              <div className="text-[color:var(--token-icon)]"><DynamicIcon name={item.icon || 'map-pin'} size={20} /></div>
              <div><h3 className="font-black text-[color:var(--token-heading)]"><span data-edit-path="label">{item.label || ''}</span> <span data-edit-path="value">{item.value || ''}</span></h3>{item.text && <div className="text-sm text-[color:var(--token-muted)] rt-content" dangerouslySetInnerHTML={{ __html: item.text }} />}</div>
            </div>
          ))}
        </div>
        {routeCta.label && <a href={routeCta.href || '#'} className="mt-8 inline-flex items-center gap-2 border-2 border-[#111827] bg-[#111827] px-5 py-3 font-black uppercase text-[color:var(--token-on-dark-heading)] shadow-[4px_4px_0_var(--token-icon)]"><span data-edit-path="label">{routeCta.label}</span><ArrowRight size={16} /></a>}
      </div>
      <div className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {nearbyItems.map((item, index) => (
            <article key={`${item.title}-${index}`} className="border-2 border-[#111827] bg-[var(--token-card-bg)] p-4 shadow-[4px_4px_0_#111827]" data-edit-collection="nearbyItems" data-edit-index={index}>
