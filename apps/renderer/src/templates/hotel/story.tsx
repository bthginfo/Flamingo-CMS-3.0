    ...
    <div>
      <div className="grid gap-16 lg:grid-cols-2 lg:items-start">
        ...
        <div className="grid gap-3">
          {p.imagePrimary && <div className="relative aspect-[16/10] overflow-hidden border border-black/10"><Image data-edit-image="imagePrimary" src={p.imagePrimary} alt="" fill className="object-cover" sizes="50vw" /></div>}
          {p.imageSecondary && <div className="relative aspect-[16/9] overflow-hidden border border-black/10"><Image data-edit-image="imageSecondary" src={p.imageSecondary} alt="" fill className="object-cover" sizes="50vw" /></div>}
        </div>
      </div>
      {p.stats.length > 0 && (
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 gap-px border border-black/10 md:grid-cols-4">
          {p.stats.map((s, i) => (
            <div key={`${s.label || 'item'}-${i}`} className="border border-black/10 bg-[var(--token-card-bg)] p-6 text-center" data-edit-collection="stats" data-edit-index={i}>
...
function StoryBold(p: Props) {
  return (
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <div>
          {p.badgeText && <p className="inline-block bg-[var(--token-badge-bg)] px-3 py-1 text-xs font-bold uppercase tracking-widest text-[color:var(--token-icon)]" data-edit-path="badgeText">{p.badgeText}</p>}
        ...
        <div className="grid gap-2">
          {p.imagePrimary && <div className="relative aspect-[16/10] overflow-hidden border-2 border-[var(--token-card-border)] shadow-[4px_4px_0_var(--token-card-border)]"><Image data-edit-image="imagePrimary" src={p.imagePrimary} alt="" fill className="object-cover" sizes="50vw" /></div>}
      ...
      {p.stats.length > 0 && (
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 gap-4 md:grid-cols-4">
          {p.stats.map((s, i) => (
            <div key={`${s.label || 'item'}-${i}`} className="border-2 border-[var(--token-card-border)] p-5 text-center shadow-[4px_4px_0_var(--token-icon)]" data-edit-collection="stats" data-edit-index={i}>
      ...
      {p.values.length > 0 && (
        <div className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {p.values.map((v, i) => (
            <div key={`${v.title || 'item'}-${i}`} className="border-2 border-[var(--token-card-border)] p-5 shadow-[4px_4px_0_var(--token-card-border)]" data-edit-collection="values" data-edit-index={i}>
              <DynamicIcon editPath="icon" name={v.icon || 'heart'} size={20} className="text-[color:var(--token-icon)]" />
