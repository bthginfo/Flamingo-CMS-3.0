    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          {p.badgeText && <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-600"><Star size={12} className="text-[color:var(--token-icon,var(--brand-primary,#1a5276))]" />{p.badgeText}</p>}
          ...
          {p.founderQuote && (
            <blockquote className="mt-8 rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-white p-5 shadow-sm">
      ...
      {p.stats.length > 0 && (
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 gap-5 md:grid-cols-4">
          {p.stats.map((s, i) => (
            <motion.div key={`${s.label}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-white p-5 text-center shadow-sm">
      ...
      {p.values.length > 0 && (
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {p.values.map((v, i) => (
            <motion.div key={`${v.title}-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex gap-4 rounded-xl border border-[var(--token-icon, var(--brand-primary))]/20 bg-white p-5 shadow-sm">
        ...
        <div className="mt-16 relative border-l-2 border-[var(--token-icon, var(--brand-primary))]/30 pl-8">
          {p.milestones.map((m, i) => (
            <motion.div key={`${m.year}-${i}`} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="relative mb-8 last:mb-0">
              <div className="absolute -left-[2.55rem] top-1 h-4 w-4 rounded-full border-2 border-[var(--token-icon, var(--brand-primary))] bg-white" />
    ...
    <div>
      <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          {p.storyText && <p className="mt-6 whitespace-pre-line leading-7 text-gray-600">{plain(p.storyText)}</p>}
          {p.founderQuote && (
            <div className="mt-8 border-l-4 border-[var(--token-icon, var(--brand-primary))] bg-black/5 p-5">
