            ...
            <div className="flex-1">
              ...
              {item.features && item.features.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {item.features.map((f, fi) => <span key={fi} className="text-xs font-bold uppercase bg-[color-mix(in_srgb,var(--token-card-bg, var(--style-card-bg,#fff))_70%,var(--style-accent,var(--token-eyebrow, var(--brand-accent)))_30%)] text-[var(--style-text-primary,#111827)] px-2 py-1">{f}</span>)}
                </div>
              )}
              {item.ctaLabel && item.ctaHref && (
