          ...
          <motion.div key={i} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.15 }}
            ...
            <div className="w-full lg:w-1/2">
              {item.mediaType === 'image' && item.image ? (
                ...
                <div className="w-24 h-24 rounded-2xl bg-[color-mix(in_srgb,var(--token-icon, var(--style-accent,var(--token-icon, var(--brand-primary))))_10%,transparent)] flex items-center justify-center mx-auto lg:mx-0">
                  <DynamicIcon name={item.icon} size={40} className="text-[var(--token-icon, var(--style-accent,var(--token-icon, var(--brand-primary))))]" />
                </div>
              ) : null}
            </div>
            <div className="w-full lg:w-1/2">
              <h3 className="font-display font-bold text-2xl lg:text-3xl mb-4 text-[var(--style-text-primary,#111827)]" data-edit-path="title">{item.title}</h3>
  ...
  return (
    <div ref={ref}>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-12 md:mb-20">
        {badgeText && <div className="flex items-center gap-3 text-sm text-[var(--token-muted, var(--style-muted,var(--style-text-secondary,#9ca3af)))] mb-4 tracking-wide uppercase"><span className="w-8 h-px bg-[var(--token-card-border, var(--style-border,#d1d5db))]" />{badgeText}</div>}
        {headline && <h2 className="text-4xl lg:text-3xl md:text-5xl font-light text-[var(--style-text-primary,#111827)] tracking-tight" data-edit-path="headline">{headline}</h2>}
        ...
        {items.map((item, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.1 }} data-edit-collection="items" data-edit-index={i}>
            ...
            <div className="max-w-2xl">
              <h3 className="text-2xl font-medium text-[var(--style-text-primary,#111827)] mb-3" data-edit-path="title">{item.title}</h3>
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: i * 0.1 }}
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-3">
                <span className="w-8 h-8 bg-[var(--token-icon, var(--style-accent,var(--token-eyebrow, var(--brand-accent))))] text-[var(--style-text-primary,var(--token-section-bg-alt, var(--brand-dark)))] font-black text-sm flex items-center justify-center">{i + 1}</span>
                <h3 className="font-bold uppercase tracking-wide text-lg text-[var(--style-text-primary,#111827)]" data-edit-path="title">{item.title}</h3>
              </div>
