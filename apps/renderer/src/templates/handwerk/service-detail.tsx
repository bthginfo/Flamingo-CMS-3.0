          ...
          <motion.div key={i} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.15 }}
            ...
            <div className="w-full lg:w-1/2">
              {item.mediaType === 'image' && item.image ? (
                ...
                <div className="w-24 h-24 rounded-2xl bg-[color-mix(in_srgb,var(--style-accent,var(--token-icon, var(--brand-primary)))_10%,transparent)] flex items-center justify-center mx-auto lg:mx-0">
                  <DynamicIcon name={item.icon} size={40} className="text-[var(--style-accent,var(--token-icon, var(--brand-primary)))]" />
                </div>
              ) : null}
            </div>
            <div className="w-full lg:w-1/2">
              ...
              {item.features && item.features.length > 0 && (
                <ul className="space-y-2 mb-6">
                  {item.features.map((f, fi) => <li key={fi} className="flex items-center gap-2 text-sm text-[var(--style-muted,var(--style-text-secondary,#4b5563))]"><CheckCircle size={16} className="text-[var(--style-accent,var(--token-eyebrow, var(--brand-accent)))] shrink-0" />{f}</li>)}
                </ul>
              )}
              {item.ctaLabel && item.ctaHref && (
                <a href={item.ctaHref} className="inline-flex items-center gap-2 text-[var(--style-accent,var(--token-icon, var(--brand-primary)))] font-medium hover:gap-3 transition-all">
  ...
  return (
    <div ref={ref}>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-12">
        {badgeText && <span className="inline-block bg-[var(--style-accent,var(--token-eyebrow, var(--brand-accent)))] text-[var(--style-text-primary,var(--token-section-bg-alt, var(--brand-dark)))] font-bold text-xs uppercase tracking-widest px-3 py-1.5 mb-4">{badgeText}</span>}
      ...
      <div className="space-y-6">
        {items.map((item, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: i * 0.1 }}
            className="flex flex-col lg:flex-row gap-6 p-6 border-3 border-[var(--style-border,var(--style-text-primary,#111827))] shadow-[4px_4px_0_var(--style-text-primary,#0d2137)] bg-[var(--token-card-bg, var(--token-card-bg, var(--style-card-bg,transparent)))]">
            {item.mediaType === 'image' && item.image && (
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-3">
                <span className="w-8 h-8 bg-[var(--style-accent,var(--token-eyebrow, var(--brand-accent)))] text-[var(--style-text-primary,var(--token-section-bg-alt, var(--brand-dark)))] font-black text-sm flex items-center justify-center">{i + 1}</span>
              ...
              {item.features && item.features.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {item.features.map((f, fi) => <span key={fi} className="text-xs font-bold uppercase bg-[color-mix(in_srgb,var(--token-card-bg, var(--token-card-bg, var(--style-card-bg,#fff)))_70%,var(--style-accent,var(--token-eyebrow, var(--brand-accent)))_30%)] text-[var(--style-text-primary,#111827)] px-2 py-1">{f}</span>)}
                </div>
              )}
              {item.ctaLabel && item.ctaHref && (
                <a href={item.ctaHref} className="inline-flex items-center gap-2 mt-4 font-bold uppercase text-sm text-[var(--style-text-primary,var(--token-section-bg-alt, var(--brand-dark)))] hover:text-[var(--style-accent,var(--token-eyebrow, var(--brand-accent)))] transition-colors">
                  {item.ctaLabel}{item.icon && <DynamicIcon name={item.icon} size={14} />}
                </a>
              )}
            </div>
