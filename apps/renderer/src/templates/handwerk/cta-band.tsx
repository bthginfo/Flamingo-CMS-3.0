...
function CtaClassic({ headline, subline, badgeText, cta, colors }: CProps) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, scale: 0.98 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.7 }} className="relative overflow-hidden rounded-4xl" style={wrapStyle}>
      {!colors?.bgColor && <motion.div style={{ y: bgY }} className="absolute inset-0 scale-110 bg-[var(--style-section-bg,var(--brand-dark,#0d2137))]" />}
      ...
      <div className="relative z-10 px-6 py-12 text-center text-[var(--style-heading-color,#ffffff)] sm:py-16 md:py-24 lg:py-32">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2 }}
          className="mb-8 inline-flex items-center gap-2 rounded-full border border-[var(--style-border-color,rgba(255,255,255,0.20))] bg-[var(--style-badge-bg,rgba(255,255,255,0.10))] px-5 py-2 text-sm text-[var(--style-badge-text,#ffffff)] backdrop-blur-sm">
        ...
        {subline && <motion.p initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.4 }} className="rt-content mx-auto mb-10 max-w-2xl text-lg leading-8 text-[var(--style-body-color,rgba(255,255,255,0.78))] sm:text-xl" dangerouslySetInnerHTML={{ __html: subline }} />}
        {cta?.label && (
...
function CtaModern({ headline, subline, cta, colors }: CProps) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.8 }}
      ...
      {subline && <div className="rt-content mx-auto mt-6 max-w-2xl text-lg text-[var(--style-body-color,#52525b)]" dangerouslySetInnerHTML={{ __html: subline }} />}
      {cta?.label && (
...
function CtaBold({ headline, subline, badgeText, cta, colors }: CProps) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}
      className={`flex flex-col items-start justify-between gap-8 p-10 text-[var(--style-heading-color,#ffffff)] lg:flex-row lg:items-center lg:p-16 ${!colors?.bgColor ? 'bg-[var(--style-section-bg,var(--brand-dark,#0d2137))]' : ''}`} style={wrapStyle}>
      <div>
        {badgeText && <span className="mb-4 inline-block bg-[var(--style-badge-bg,var(--style-accent-color,var(--brand-accent)))] px-3 py-1.5 text-xs font-bold uppercase tracking-widest text-[var(--style-badge-text,#ffffff)]">{badgeText}</span>}
        <h2 className="text-2xl lg:text-4xl font-black uppercase tracking-tight">{headline}</h2>
        {subline && <div className="rt-content mt-3 max-w-xl font-medium text-[var(--style-body-color,rgba(255,255,255,0.72))]" dangerouslySetInnerHTML={{ __html: subline }} />}
      </div>
      {cta?.label && (
        <a href={cta.href} className="inline-flex w-full shrink-0 items-center justify-between bg-[var(--brand-btn-bg,var(--style-accent-color,var(--brand-accent)))] px-8 py-4 text-base font-bold uppercase tracking-wider text-[var(--brand-btn-text,#111827)] shadow-[4px_4px_0_rgba(255,255,255,0.2)] transition-transform hover:translate-x-1 lg:w-auto lg:justify-center lg:gap-3">
