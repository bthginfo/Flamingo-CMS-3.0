...
function FaqItemClassic({ question, answer, defaultOpen }: { question: string; answer: string; defaultOpen: boolean }) {
  ...
  return (
    <div className={cn('overflow-hidden rounded-2xl border transition-all duration-300', open ? 'border-[var(--style-border-color,rgba(0,0,0,0.14))] bg-[var(--style-card-bg,#ffffff)] shadow-lg' : 'border-[var(--style-border-color,rgba(0,0,0,0.08))] bg-[var(--style-card-bg,rgba(255,255,255,0.50))] hover:shadow-sm')}>
      <button onClick={() => setOpen(!open)} className="font-display flex w-full items-center justify-between gap-4 px-7 py-6 text-left text-[16px] font-semibold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">
        {question}
        <ChevronDown size={18} className={cn('shrink-0 text-[var(--style-text-muted,var(--style-text-secondary,#9ca3af))] transition-transform', open && 'rotate-180')} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }}>
            <div className="px-7 pb-6 leading-relaxed text-[var(--style-body-color,var(--style-text-secondary,#6b7280))]">{plain(answer)}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function FaqModern({ headline, badgeText, items, expandFirst }: FProps) {
  ...
  return (
    <div ref={ref} className="max-w-3xl mx-auto">
      ...
      <div className="divide-y divide-[var(--style-border-color,rgba(0,0,0,0.08))]">
        {items.map((item, i) => (
...
function FaqItemModern({ question, answer, defaultOpen }: { question: string; answer: string; defaultOpen: boolean }) {
  ...
  return (
    <div className="py-6">
      <button onClick={() => setOpen(!open)} className="flex w-full items-center justify-between gap-4 text-left font-medium text-[var(--style-heading-color,var(--style-text-primary,#111827))] transition-colors hover:text-[var(--style-accent-color,var(--brand-primary))]">
        {question}
        <Plus size={16} className={cn('shrink-0 text-[var(--style-text-muted,var(--style-text-secondary,#d1d5db))] transition-transform', open && 'rotate-45')} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }}>
            <p className="mt-4 text-[15px] leading-relaxed text-[var(--style-body-color,var(--style-text-secondary,#9ca3af))]">{plain(answer)}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function FaqItemBold({ question, answer, defaultOpen, num }: { question: string; answer: string; defaultOpen: boolean; num: number }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className={cn('border-3 border-[var(--style-border-color,var(--style-text-primary,#111827))] transition-all', open ? 'shadow-[4px_4px_0_var(--style-accent-color,#f39c12)]' : 'shadow-[4px_4px_0_var(--style-text-primary,#0d2137)]')}>
      <button onClick={() => setOpen(!open)} className="flex w-full items-center gap-4 px-6 py-5 text-left text-sm font-bold uppercase tracking-wide text-[var(--style-heading-color,var(--style-text-primary,#111827))]">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center bg-[var(--style-text-primary,var(--brand-dark))] text-xs font-black text-[var(--brand-btn-text,#ffffff)]">{num}</span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }}>
            <div className="border-t-2 border-[var(--style-border-color,rgba(0,0,0,0.12))] px-6 pb-5 pt-4 leading-relaxed text-[var(--style-body-color,var(--style-text-secondary,#4b5563))]">{plain(answer)}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
