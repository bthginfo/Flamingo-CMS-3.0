    ...
    <div ref={ref}>
      ...
      {(storyHeadline || storyText) && (
        <motion.div initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 }} className="flex flex-col lg:flex-row gap-12 lg:gap-8 lg:gap-16 items-center mb-12 md:mb-24">
          ...
          <div className={storyImage ? 'w-full lg:w-1/2' : 'w-full max-w-3xl mx-auto'}>
            {storyHeadline && <h3 className="font-display mb-4 text-2xl font-bold text-[var(--style-heading-color,var(--style-text-primary,#111827))] lg:text-3xl">{storyHeadline}</h3>}
            {storyText && <div className="rt-content whitespace-pre-line text-lg leading-relaxed text-[var(--style-body-color,var(--style-text-secondary,#6b7280))]" dangerouslySetInnerHTML={{ __html: storyText }} />}
          </div>
        </motion.div>
      )}
      {stats.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12 md:mb-24">
          {stats.map((s, i) => (
            <div key={i} className="rounded-2xl border border-[var(--style-border-color,rgba(0,0,0,0.08))] bg-[var(--style-card-bg,rgba(0,0,0,0.03))] p-6 text-center">
              <div className="font-display mb-1 text-3xl font-bold text-[var(--style-accent-color,var(--brand-primary))] lg:text-4xl">{s.value}</div>
              <div className="text-sm text-[var(--style-text-secondary,var(--style-text-muted,#6b7280))]">{s.label}</div>
            </div>
          ))}
        </div>
      )}
      {values.length > 0 && (
        <div className="mb-12 md:mb-24">
          <h3 className="font-display mb-12 text-center text-2xl font-bold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{valuesHeadline}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {values.map((v, i) => (
              <div key={i} className="group rounded-2xl border border-[var(--style-border-color,rgba(0,0,0,0.08))] bg-[var(--style-card-bg,#ffffff)] p-8 shadow-sm transition-all hover:shadow-lg">
                {v.icon && <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-xl bg-[var(--style-accent-color,var(--brand-primary))]/10 transition-transform group-hover:scale-110"><DynamicIcon name={v.icon} size={28} className="text-[var(--style-icon-color,var(--style-accent-color,var(--brand-primary)))]" /></div>}
                <h4 className="mb-2 text-lg font-semibold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{v.title}</h4>
                <div className="rt-content text-sm leading-relaxed text-[var(--style-body-color,var(--style-text-secondary,#6b7280))]" dangerouslySetInnerHTML={{ __html: v.text }} />
              </div>
            ))}
          </div>
        </div>
      ...
      {members.length > 0 && (
        <div>
          <h3 className="font-display mb-12 text-center text-2xl font-bold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{membersHeadline}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {members.map((m, i) => (
              <div key={i} className="text-center group">
                ...
                <h4 className="text-lg font-semibold text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{m.name}</h4>
                <p className="mb-2 text-sm font-medium text-[var(--style-accent-color,var(--brand-primary))]">{m.role}</p>
                {m.bio && <div className="rt-content mx-auto max-w-xs text-sm text-[var(--style-body-color,var(--style-text-secondary,#6b7280))]" dangerouslySetInnerHTML={{ __html: m.bio }} />}
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div ref={ref}>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-12 md:mb-20">
        {badgeText && <div className="mb-4 flex items-center gap-3 text-sm uppercase tracking-wide text-[var(--style-text-muted,var(--style-text-secondary,#9ca3af))]"><span className="h-px w-8 bg-[var(--style-border-color,#d1d5db)]" />{badgeText}</div>}
      ...
      {(storyHeadline || storyText) && (
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-center mb-12 md:mb-24">
          ...
          <div className={storyImage ? 'w-full lg:w-1/2' : 'max-w-2xl'}>
            {storyHeadline && <h3 className="mb-4 text-2xl font-light text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{storyHeadline}</h3>}
            {storyText && <div className="rt-content whitespace-pre-line leading-loose text-[var(--style-body-color,var(--style-text-secondary,#9ca3af))]" dangerouslySetInnerHTML={{ __html: storyText }} />}
          </div>
        </div>
      )}
      {stats.length > 0 && (
        <div className="flex flex-wrap gap-8 md:gap-12 mb-24 justify-center">
          {stats.map((s, i) => <div key={i} className="text-center"><div className="text-4xl font-light text-[var(--style-accent-color,var(--style-heading-color,#111827))]">{s.value}</div><div className="mt-1 text-xs uppercase tracking-wider text-[var(--style-text-muted,var(--style-text-secondary,#9ca3af))]">{s.label}</div></div>)}
        </div>
      )}
      {members.length > 0 && (
        <div>
          <h3 className="mb-10 text-xl font-light text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{membersHeadline}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {members.map((m, i) => (
              <div key={i} className="group">
                <div className="relative aspect-[3/4] rounded-lg overflow-hidden mb-4">
                  {m.image ? <Image src={m.image} alt={m.name} fill className="object-cover grayscale transition-all duration-500 group-hover:grayscale-0" sizes="300px" /> : <div className="h-full w-full bg-[var(--style-card-bg,#f3f4f6)]" />}
                </div>
                <h4 className="font-medium text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{m.name}</h4>
                <p className="text-sm text-[var(--style-text-muted,var(--style-text-secondary,#9ca3af))]">{m.role}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    <div ref={ref}>
      ...
      {(storyHeadline || storyText) && (
        <div className="flex flex-col lg:flex-row gap-8 items-start mb-10 md:mb-16">
          {storyImage && <div className="w-full lg:w-1/2"><div className="relative aspect-[4/3] overflow-hidden border-3 border-[var(--style-border-color,var(--style-text-primary,#111827))] shadow-[6px_6px_0_var(--style-text-primary,#0d2137)]"><Image src={storyImage} alt="" fill className="object-cover" sizes="50vw" /></div></div>}
          <div className={storyImage ? 'w-full lg:w-1/2' : 'max-w-2xl'}>
            {storyHeadline && <h3 className="mb-3 text-xl font-bold uppercase text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{storyHeadline}</h3>}
            {storyText && <div className="rt-content whitespace-pre-line leading-relaxed text-[var(--style-body-color,var(--style-text-secondary,#4b5563))]" dangerouslySetInnerHTML={{ __html: storyText }} />}
          </div>
        </div>
      )}
      {stats.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 md:mb-16">
          {stats.map((s, i) => (
            <div key={i} className="bg-[var(--style-card-bg,var(--brand-dark))] p-5 text-center text-[var(--style-text-primary,#ffffff)]">
              <div className="text-2xl font-black text-[var(--style-accent-color,var(--brand-accent))]">{s.value}</div>
              <div className="mt-1 text-xs uppercase tracking-wider text-[var(--style-text-muted,rgba(255,255,255,0.60))]">{s.label}</div>
            </div>
          ))}
        </div>
      )}
      {members.length > 0 && (
        <div>
          <h3 className="mb-8 text-xl font-bold uppercase text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{membersHeadline}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {members.map((m, i) => (
              <div key={i} className="flex items-start gap-4 border-3 border-[var(--style-border-color,var(--style-text-primary,#111827))] bg-[var(--style-card-bg,transparent)] p-4 shadow-[3px_3px_0_var(--style-accent-color,#f39c12)]">
                <div className="relative w-20 h-20 shrink-0 overflow-hidden">
                  {m.image ? <Image src={m.image} alt={m.name} fill className="object-cover" sizes="80px" /> : <div className="h-full w-full bg-[var(--style-text-muted,#e5e7eb)]" />}
                </div>
                <div>
                  <h4 className="text-sm font-bold uppercase text-[var(--style-heading-color,var(--style-text-primary,#111827))]">{m.name}</h4>
                  <p className="text-xs font-bold uppercase text-[var(--style-accent-color,var(--brand-accent))]">{m.role}</p>
                  {m.bio && <div className="rt-content mt-1 text-xs text-[var(--style-body-color,var(--style-text-secondary,#6b7280))]" dangerouslySetInnerHTML={{ __html: m.bio }} />}
                </div>
              </div>
            ))}
          </div>
