      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className="text-center p-6 rounded-2xl bg-[var(--token-card-bg,var(--token-card-bg, var(--style-card-bg,#fff)))] border border-[var(--token-card-border,var(--token-card-border, var(--style-border-color,rgba(0,0,0,.08))))] shadow-sm"
          >
            {stat.icon && (
              <div className="w-10 h-10 mx-auto mb-3 rounded-lg bg-[color-mix(in_srgb,var(--token-icon,var(--token-icon, var(--style-icon-color,var(--style-accent-color,var(--brand-primary)))))_14%,transparent)] flex items-center justify-center text-[var(--token-icon,var(--token-icon, var(--style-icon-color,var(--style-accent-color,var(--brand-primary)))))]">
