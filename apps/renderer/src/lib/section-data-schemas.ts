export function getSectionSchemas(industry: string): Record<string, object> {
  const schemas: Record<string, object> = {
    ...
    cameraExplodeScroll: { fields: { badge: 'string?', headline: 'string', subline: 'string?', brandImage: 'url?', parts: '{ id?: stable-string, label: string, text: string, offsetX: number -180..180, offsetY: number -180..180, color?: hex-color }[] (4–6 Teile)', cta: '{ label: string, href: safe-url }?' }, note: 'Advanced 2.5D Section. No GLTF/Three.js required; provide concise camera/production layers and safe offsets.' },
  };
