export function getSectionSchemas(industry: string): Record<string, object> {
  const schemas: Record<string, object> = {
    ...
    statsCounter: { fields: { badge: 'string?', headline: 'string', subline: 'string?', layout: '"default"|"projectDossier"? (projectDossier = kompaktes redaktionelles Projekt-Dossier)', stats: '{ value: number|string, suffix?: string, prefix?: string, label: string }[] (4 items recommended; numbers animate, text such as "Website · Kampagne" renders as keywords)' } },
    ...
    customForm: { fields: { formKey: 'lowercase-slug string', eyebrow: 'string?', title: 'string', description: 'string?', submitLabel: 'string', successTitle: 'string', successMessage: 'string', deliveryPolicy: '"dry-run"|"live" (server-authoritative)', groups: '{ id: string, title: string, description?: string, kind?: "default"|"matrix"|"consent", fields: { id: string, label: string, type: "text"|"email"|"tel"|"textarea"|"select"|"radio"|"checkbox"|"date"|"number"|"boolean-details", required?: boolean, placeholder?: string, helpText?: string, options?: { value: string, label: string }[], condition?: { field: string, equals?: string|boolean|number, includes?: string }, autocomplete?: string, width?: "full"|"half"|"third", maxLength?: number, min?: number, max?: number, detailsLabel?: string, detailsPlaceholder?: string, detailsRequired?: boolean }[] }[]', privacyHref: 'string', privacyLabel: 'string', pdfTitle: 'string', pdfFilename: 'string', practiceSubject: 'string', confirmationSubject: 'string', confirmationText: 'string', emailField: 'field id', firstNameField: 'field id?', lastNameField: 'field id?' }, note: 'Reusable, server-validated form. Delivery policy is resolved from the published section; never accepted from the browser request.' },
    ...
    verticalReelShowcase: { fields: { badge: 'string?', headline: 'string', subline: 'string?', aspectRatio: '"9/16"|"4/5"|"1/1"|"16/9"?', reels: '{ eyebrow?: string, title: string, text?: string, videoSrc?: https-url, poster?: url, meta?: string, ctaLabel?: string, ctaHref?: safe-url, autoplay?: boolean }[] (2–5 Reels)', cta: '{ label: string, href: safe-url }?' }, note: 'Advanced Reel Section. Preserve vertical aspect ratio. Use direct public MP4/WebM URLs for videoSrc; use poster when video is unavailable.' },
    aiWorkflowReel: { fields: { badge: 'string?', headline: 'string', subline: 'string?', media: '{ videoSrc?: https-url, poster?: url, caption?: string }', steps: '{ kicker?: string, title: string, text: string, proof?: string }[] (3–6 Phasen)', cta: '{ label: string, href: safe-url }?' }, note: 'Advanced Workflow Section. Explain real production steps. Avoid generic AI hype; every phase must be concrete and understandable.' },
    cameraExplodeScroll: { fields: { badge: 'string?', headline: 'string', subline: 'string?', modelUrl: 'https-url to .glb/.gltf?', brandImage: 'url?', parts: '{ id?: stable-string, label: string, text: string, offsetX: number -260..260, offsetY: number -260..260, offsetZ?: number -260..260, color?: hex-color }[] (4–7 Teile)', cta: '{ label: string, href: safe-url }?' }, note: 'Advanced WebGL Section. If modelUrl is a public GLB/GLTF, the model is used; otherwise Flamingo renders a lightweight procedural 3D camera. Never provide scripts, raw HTML or custom JS.' },
  };
