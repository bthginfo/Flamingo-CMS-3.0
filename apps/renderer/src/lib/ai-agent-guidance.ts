...
export function buildAiAgentContract(input: {
  ...
  return {
    ...
    qualityBar: [
      ...
      'Within a card or step section, give each item its own specific description. Reusing the same paragraph under different titles fails plan validation.',
      'For image and video cards, check captions and controls against the actual media on desktop and mobile; a palette-only contrast check cannot assess a bright frame.',
    ...
    advancedExperienceGuide: {
      ...
      assetRules: {
        ...
        verticalReelShowcase: '2–5 reels. Match the aspect ratio to the footage and leave room for the caption and play control. Use direct HTTPS video URLs; provide a representative poster from the same project for every video so blank first frames do not render. Each reel needs a distinct title and short context.',
