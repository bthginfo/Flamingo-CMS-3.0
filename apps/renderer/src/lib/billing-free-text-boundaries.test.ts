...
test('free-text reads, writes and downloads are tenant scoped and add-on gated', () => {
  ...
  const workspace = source('apps/renderer/src/app/admin/billing/free-text-workspace.tsx');
  ...
  assert.match(finalization, /eq\(billingFreeTextDocuments\.status, 'draft'\)/, 'only draft documents may be claimed for finalization');
  const save = actions.slice(actions.indexOf('export async function saveFreeTextDocumentAction'), actions.indexOf('export async function finalizeFreeTextDocumentAction'));
  assert.match(save, /existing\.status === 'finalized'/, 'finalized documents must be rejected before an update is attempted');
  assert.match(save, /eq\(billingFreeTextDocuments\.status, 'draft'\)/, 'the update must remain draft-only if a finalization races with the save');
  const remove = actions.slice(actions.indexOf('export async function deleteFreeTextDocumentAction'));
  assert.match(remove, /existing\.status === 'finalized'/, 'finalized documents must be rejected before deletion');
  assert.match(remove, /eq\(billingFreeTextDocuments\.status, 'draft'\)/, 'only drafts may be deleted');
  assert.match(workspace, /const editable = detail\.status === 'draft'/);
  assert.match(workspace, /status: detail\.status/, 'finalized previews must resolve from immutable snapshots');
  assert.match(workspace, /\{editable \? <button type="button" disabled=\{pending\} onClick=\{save\}/);
  assert.match(workspace, /\{editable \? <button type="button" onClick=\{remove\}/);
  assert.match(workspace, /locked \? <button type="button" disabled=\{pending\} onClick=\{finalize\}/, 'finalizing letters keep their status-check action');
  assert.ok(workspace.includes('Danach ist das Schreiben unver\\u00e4nderlich'));
  assert.ok(workspace.includes('Entw\\u00fcrfe k\\u00f6nnen Sie bearbeiten; festgeschriebene Schreiben stehen als PDF bereit'));
  assert.doesNotMatch(workspace, /PDF aktualisieren|Schreiben bleibt bearbeitbar/);
