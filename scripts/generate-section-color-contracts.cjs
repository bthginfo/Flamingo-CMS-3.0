...
function loadTemplateRegistry() {
  ...
  // Also parse SHARED_TEMPLATES to ensure all shared sections get contracts
  const sharedRe = /^const SHARED_TEMPLATES[^{]*\{([\s\S]*?)^\};/m;
  const sharedMatch = src.match(sharedRe);
  if (sharedMatch) {
    const sharedBody = sharedMatch[1];
    const sharedEntryRe = /^\s*([a-zA-Z][\w]*)\s*:\s*([A-Z][A-Za-z0-9]+),?\s*$/gm;
    let sm;
    while ((sm = sharedEntryRe.exec(sharedBody)) !== null) {
      const alreadyCovered = industryTypeComponent.some(e => e.type === sm[1]);
      if (!alreadyCovered) {
        industryTypeComponent.push({ industry: "shared", type: sm[1], componentName: sm[2] });
      }
    }
  }
