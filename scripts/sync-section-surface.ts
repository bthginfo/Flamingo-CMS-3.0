...
const oldBlock = extractBalanced(colorSource, start);
const entries = [...sectionColorFields.entries()]
  .filter(([, fields]) => fields.length > 0)
  .sort(([a], [b]) => a.localeCompare(b))
  .map(([type, fields]) => `  ${type}: ${quoteArray(fields)},`)
  .join('\n');
const newBlock = `{\n${entries}\n}`;
const updatedColorSource = colorSource.slice(0, start) + colorSource.slice(start).replace(oldBlock, newBlock).replace(
  'return SECTION_FIELDS[sectionType] ?? [\'sectionBg\', \'headingColor\', \'bodyColor\'];',
  'return SECTION_FIELDS[sectionType] ?? [];'
);
fs.writeFileSync(colorEditorPath, updatedColorSource);
...
console.log(JSON.stringify({
  ...
  colorEditorPath: path.relative(root, colorEditorPath),
